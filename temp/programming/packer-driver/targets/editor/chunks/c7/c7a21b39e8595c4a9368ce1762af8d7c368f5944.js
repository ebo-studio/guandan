      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          const remaining = cards.map((c, idx) => ({
            ...
            idx
          }) => !usedIndex.has(idx)); // ✅ 对未用牌按点数分组（保留 index）
          ...
          for (const indexes of rankMap.values()) {
            ...
            if (hasSelected) {
              moveToBack.push(group); // 放末尾
            } else {
              result.push(group); // 保持原顺序
            }

            indexes.forEach(i => usedIndex.add(i)); // 标记为已处理
          } // 最后返回结果，炸弹已经在最前面
...
//# sourceMappingURL=c7a21b39e8595c4a9368ce1762af8d7c368f5944.js.map