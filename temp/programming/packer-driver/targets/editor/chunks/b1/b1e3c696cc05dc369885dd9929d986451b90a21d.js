    ...
    execute: function () {
      ...
      _export("CardLayer", CardLayer = (_dec = ccclass('CardLayer'), _dec2 = property(Prefab), _dec3 = property(SpriteAtlas), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(_crd && GameTimer === void 0 ? (_reportPossibleCrUseOfGameTimer({
        ...
        updateSelectedCards_V() {
          ...
          let isRecover = this.isRecoverGroupPartialMatch(this.handCardsValue, this.sorthandCardsValue, 1, this.selectCardValue);
        ...
        isRecoverGroupPartialMatch(cards, // 当前完整手牌（含重复）
        grouped, // 理牌拍扁的数组
        groupSizes, // 每组牌型的长度
        selected) {
          // Step1: 找出 selected 中的 index
          ...
          for (let i = 0; i < cards.length; i++) {
            ...
            if ((selectedCount.get(c) || 0) > 0) {
              selectedIndexes.push(i);
              selectedCount.set(c, selectedCount.get(c) - 1);
              usedSelected.add(i);
            }
          } // Step2: 从 grouped 构建每一组的 index 映射（映射到 cards）
          ...
          for (const size of groupSizes) {
            const group = grouped.slice(groupedOffset, groupedOffset + size);
            groupedOffset += size;
            const groupCount = new Map();

            for (const val of group) {
              groupCount.set(val, (groupCount.get(val) || 0) + 1);
            }
            ...
            for (let i = 0; i < cards.length; i++) {
              ...
              if ((groupCount.get(c) || 0) > 0) {
                groupedIndexes.add(i);
                groupCount.set(c, groupCount.get(c) - 1);
                usedGlobal.add(i);
              }
            }
          } // Step3: 判断是否选中牌中包含 grouped 的任意 index


          return selectedIndexes.some(idx => groupedIndexes.has(idx));
        } //连续点击两次 双击
...
//# sourceMappingURL=b1e3c696cc05dc369885dd9929d986451b90a21d.js.map