    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function smartSortCards(cards) {
          ...
          const randomPart = []; // 杂牌型：飞机、连对、顺子、三带二

          const scatteredPart = []; // 未组成牌型的单张、对子、三张

          const usedIndex = new Set();

          const getUnused = () => cards.filter((_, i) => !usedIndex.has(i));

          const pushAndMark = (group, target) => {
            const realGroup = [];
          ...
          const remain = getUnused();

          for (let i = 0; i < remain.length; i++) {
            const point = remain[i] % 16;
          ...
          for (const [rank, list] of countMap.entries()) {
            const group = list.slice(); // 所有未用的相同点数牌

            while (group.length >= 3) remainGroups.push(group.splice(0, 3));

            while (group.length >= 2) remainGroups.push(group.splice(0, 2));

            while (group.length >= 1) remainGroups.push(group.splice(0, 1));
          }
...
//# sourceMappingURL=0a64cf13fdecf0bbf8f45e6b67d47d31c58b2a55.js.map