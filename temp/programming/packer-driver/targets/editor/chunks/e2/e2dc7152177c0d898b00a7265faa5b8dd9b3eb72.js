    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          const countMap = new Map(); // 分组：按点数归类
          ...
          const triples = Array.from(countMap.entries()).filter(([rank, list]) => list.length >= 3).sort((a, b) => a[0] - b[0]); // 所有 >=2 的对子（按点数升序）

          const pairs = Array.from(countMap.entries()).filter(([rank, list]) => list.length >= 2).sort((a, b) => a[0] - b[0]); // 尝试匹配最小三张 + 最小对子（不同点数）

          for (const [tripleRank, tripleCards] of triples) {
            for (const [pairRank, pairCards] of pairs) {
              if (tripleRank !== pairRank) {
                const result = [...tripleCards.slice(0, 3), ...pairCards.slice(0, 2)];
...
//# sourceMappingURL=e2dc7152177c0d898b00a7265faa5b8dd9b3eb72.js.map