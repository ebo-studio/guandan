    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findBombsByCount(cards, count) {
          const map = new Map();
          const result = []; // 找红桃级牌（逢人配牌）

          const frpCards = cards.filter(card => {
            const rank = card % 16;
            const color = Math.floor(card / 16);
          ...
          const remainCards = cards.filter(card => !frpCards.includes(card));

          for (const card of remainCards) {
            const rank = card % 16;
...
//# sourceMappingURL=6b3cad381dbd023c7ceee2ad741b66814b3633f6.js.map