      ...
      (function (_GameLogic) {
        ...
        function getTripleWithPairPreferBig(cards) {
          ...
          for (const rank of ranksSorted) {
            const base = rankMap.get(rank); // if (!base) continue;

            const need = 3 - base.length;

            if (base.length >= 1 && heartCards.length >= need) {
              const triple = [...base, ...heartCards.slice(0, need)];
              const used = new Set(triple); // 查找对子部分
              ...
              for (const red of redLeft) {
                for (const [r, list] of pairMap.entries()) {
                  if (list.length === 1) {
                    list.push(red);
                    break;
                  }
...
//# sourceMappingURL=6bd2e5f581da03dcfbd083dd220bd1b13418dc89.js.map