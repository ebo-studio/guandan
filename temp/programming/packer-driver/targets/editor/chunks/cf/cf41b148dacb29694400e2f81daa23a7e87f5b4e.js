    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function autoSortCards(cards) {
          const result = [];
          const used = new Set();
          const sortCards = sortCardsBySizeDown(cards, cards.length); // ✅ 一键理牌：先识别固定顺序牌型
          ...
          const randomTypes = shuffleArray([findFeiji, findStraightByCard, findLiandui, findThreeWithTwo]);

          for (const func of randomTypes) {
            const groups = func(cards.filter(c => !used.has(c)));
            addGroup(groups);
          } // ✅ 剩余未处理的牌分组（对子、单牌）


          const remain = cards.filter(c => !used.has(c));
          const sortValue = sortCardsBySizeDown(remain, remain.length);
          const uniqueList = getUniqueCard(sortValue);
          let idx = 0;
        ...
        function findWangzha(cards) {
          const result = []; // 大王和小王，一般用牌值区分，比如 0x4f 和 0x4e 或者 rank = 15/16 等

          const joker1 = cards.find(c => getCardSize(c) === 15); // 小王

          const joker2 = cards.find(c => getCardSize(c) === 16); // 大王
...
//# sourceMappingURL=cf41b148dacb29694400e2f81daa23a7e87f5b4e.js.map