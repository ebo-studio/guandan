    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          const prevSet = new Set(prevSelected); // 找出这次新选中的牌（不在上一次中的）

          const newSelected = selected.filter(card => !prevSet.has(card)); // 剩余未选中的牌

          const remain = cards.filter(card => !selectedSet.has(card)); // 保持每次理牌顺序追加：上次的 + 新的

          const result = [];
          if (prevSelected.length > 0) result.push(prevSelected);
          if (newSelected.length > 0) result.push(newSelected); // 把剩余的加在最前面（作为主手牌）
...
//# sourceMappingURL=53cd2d73c33b7a0f099557e958c49b48903056ca.js.map