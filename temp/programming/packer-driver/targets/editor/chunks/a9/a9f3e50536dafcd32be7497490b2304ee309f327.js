    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          const remain = []; // 把已理牌的牌从原数组中剔除

          for (let card of cards) {
            const idx = selectedCopy.indexOf(card);
          ...
          for (let card of remain) {
            if (temp.length === 0 || GameLogic.getCardSize(temp[0]) === GameLogic.getCardSize(card)) {
              temp.push(card);
            } else {
              groupList.push(temp);
              temp = [card];
          ...
          if (temp.length > 0) groupList.push(temp); // ② 将选中理牌组追加到末尾
...
//# sourceMappingURL=a9f3e50536dafcd32be7497490b2304ee309f327.js.map