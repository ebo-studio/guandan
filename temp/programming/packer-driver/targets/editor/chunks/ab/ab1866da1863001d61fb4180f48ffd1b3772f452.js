    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function autoSortCards(cards) {
          ...
          const randomGroups = [shuffleArray(findThreeWithTwo(getRemain(true))), // 三带二
        ...
        function findThreeWithTwo(cards) {
          ...
          const pairMap = new Map(); // 拆分点数映射表（排除大小王）
          ...
          for (const triple of triples) {
            const tripleCardSet = new Set(triple); // 找出一个未冲突的对子

            const pair = pairs.find(p => !p.some(c => tripleCardSet.has(c)) && !p.some(c => used.has(c)));
        ...
        function smartSortCards(cards) {
          const mainResult = []; // 主牌型区（固定优先级）

          const randomPart = []; // 飞机、连对、顺子、三带二等

          const used = new Set(); // const pushAndMark = (group: number[], toMain: boolean = true) => {
          ...
          const mainResult1 = getSameCardSizeList(mainResult.flat());
...
//# sourceMappingURL=ab1866da1863001d61fb4180f48ffd1b3772f452.js.map