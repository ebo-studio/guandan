    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          const selectedCardCount = new Map();
          ...
          for (const group of prevGrouped) {
            ...
            for (const c of group) {
              groupCount.set(c, (groupCount.get(c) || 0) + 1);
            } // 统计 selected 中包含的数量


            let isGroupSelected = false;

            for (const [card, count] of groupCount.entries()) {
              const selectedCount = selected.filter(c => c === card).length;
          ...
          const sortValue = cards.filter((_, idx) => !usedIndex.has(idx)); // // const remaining = cards.filter(card => !usedCards.has(card));
...
//# sourceMappingURL=ff1fbc982b101a6590461805e794ef3cc0919a92.js.map