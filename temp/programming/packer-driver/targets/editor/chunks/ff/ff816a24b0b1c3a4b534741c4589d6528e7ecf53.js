    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          if (isThreeWithTwo) {
            [...triple, ...pair].forEach(c => threeWithTwoSet.add(c));
          } // 3. 将牌型组成的 group 移到后面（如果是三带二，则整体移动该组合）
          ...
          for (let group of tmpList) {
            if (isThreeWithTwo && group.every(card => threeWithTwoSet.has(card))) {
              moveToBack.push(group);
            } else if (!isThreeWithTwo && group.some(card => selected.includes(card))) {
              moveToBack.push(group);
            } else {
              result.push(group);
            }
...
//# sourceMappingURL=ff816a24b0b1c3a4b534741c4589d6528e7ecf53.js.map