    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          const newSelected = selected.filter(card => !prevSet.has(card)); // 3. 追加到选中历史
          ...
          if (triple.length === 3 && pair.length === 2) {
            for (let c of [...triple, ...pair]) {
              threeWithTwoSet.add(c);
            }
          } // 5. 遍历分组，识别三带二整体移到后面，其它按点数落位
          ...
          for (let group of tmpList) {
            if (group.every(card => threeWithTwoSet.has(card))) {
              // 整体三带二
              moveToBack.push(group);
            } else if (group.some(card => selectedHistory.includes(card))) {
              // 其它选中的散牌
              moveToBack.push(group);
            } else {
              result.push(group);
...
//# sourceMappingURL=e191db0fc822d5c00dc50e9538ffb28989e3093c.js.map