    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function autoSortCards(cards) {
          ...
          const used = new Set(); // 用于标记已使用的牌（不论是否被组合）

          const usedForRandom = new Set(); // 用于标记已用于随机牌型的牌

          const usedForCombination = new Set(); // 用于标记参与组合的牌（避免在剩余牌时被误判为已使用）
          // 辅助方法：将已使用的牌加入结果
          ...
          const randomGroups = [shuffleArray(findThreeWithTwo(getRemain(false))), // 三带二
          shuffleArray(findFeiji(getRemain(false))), // 飞机
          shuffleArray(findLiandui(getRemain(false))), // 连对
          shuffleArray(findStraightByCard(getRemain(false))) // 顺子
          ]; // 只选择一个有效的随机牌型
          ...
          addGroup(findPair(getRemain(true)));
          addGroup(findAllSingles(getRemain(true))); // ✅ 最终拼接并返回
        ...
        function findPair(cards) {
          ...
          for (const [rank, group] of map.entries()) {
            if (rank == 15 || rank == 16 || rank == 17) continue; // 过滤 2、小王、大王（15，16，17）
...
//# sourceMappingURL=28977ac314a3cff6d79fc48aa774c08ddb688f12.js.map