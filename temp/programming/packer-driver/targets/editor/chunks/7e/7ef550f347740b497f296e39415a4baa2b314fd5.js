    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          const restoreSet = new Set(); // ✅ 找出需要“恢复”的理牌组（只要有1张被选中，就整组恢复）

          for (const group of prevGrouped) {
            const isRestored = group.some(card => selectedSet.has(card));
          ...
          const thisSelected = selected.filter(c => !restoreSet.has(c));
          const thisSelectedSet = new Set(thisSelected); // ✅ 三带二识别

          let triple = [];
          let pair = [];
          const map = getCardCountMap(thisSelected);
          ...
          const feijiList = findFeiji(thisSelected.filter(c => !usedCards.has(c)));

          for (let group of feijiList) {
            moveToBack.push(group);
            group.forEach(c => usedCards.add(c));
          } // ✅ 顺子识别


          const straights = findStraightByCard(thisSelected.filter(c => !usedCards.has(c)));
          ...
          const lianduiList = findLiandui(thisSelected.filter(c => !usedCards.has(c)));

          for (let group of lianduiList) {
            moveToBack.push(group);
            group.forEach(c => usedCards.add(c));
          } // ✅ 剩余牌：打散并插入 result


          const remainCards = cards.filter(c => !usedCards.has(c) && !restoreSet.has(c));
          const sortValue = sortCardsBySizeDown(remainCards, remainCards.length);
          const uniqueList = getUniqueCard(sortValue);
          let idx = 0;
...
//# sourceMappingURL=7ef550f347740b497f296e39415a4baa2b314fd5.js.map