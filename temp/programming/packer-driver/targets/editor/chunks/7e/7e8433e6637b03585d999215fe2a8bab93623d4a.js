    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function sortSelectedCardsToBack(cardList, selected) {
          // 复制原始数组以防修改原数组
          const remainCards = [];
          const selectedCopy = (_crd && utils === void 0 ? (_reportPossibleCrUseOfutils({
            error: Error()
          }), utils) : utils).deepCopy(selected); // 用于最终排序后的牌组

          const finalList = [];
          const pickedSet = new Set(); // 选中部分按理牌优先级识别（例如炸弹 > 顺子 > 对子）

          const sortedSelected = GameLogic.getSortCard(selectedCopy); // 将识别出的组合提取

          const matchedGroups = []; // 依次识别复杂牌型（优先炸弹）

          const bombList = GameLogic.getSameCardSizeList(sortedSelected).filter(g => g.length >= 4);

          for (let group of bombList) {
            matchedGroups.push(group);
            group.forEach(c => pickedSet.add(c));
          } // 继续识别顺子（需要过滤掉已用的）


          const leftForStraight = sortedSelected.filter(c => !pickedSet.has(c));
          const straights = (GameLogic.findStraight == null ? void 0 : GameLogic.findStraight(leftForStraight)) || [];
        ...
        function moveSelectedCardsToBack(cards, selected) {
          ...
          const remain = []; // 从原牌组中剔除选中牌（按值逐个移除）

          for (let i = 0; i < cards.length; i++) {
            const v = cards[i];
            const idx = selectedCopy.indexOf(v);
...
//# sourceMappingURL=7e8433e6637b03585d999215fe2a8bab93623d4a.js.map