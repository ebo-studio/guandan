    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          } // 2. 记录本轮新增选中的牌（和 prevSelected 做差集）


          const prevSet = new Set(prevSelected);
          const newSelected = selected.filter(card => !prevSet.has(card)); // 3. 追加新增牌到历史记录中（避免重复）

          for (const card of newSelected) {
            if (!selectedHistory.includes(card)) {
              selectedHistory.push(card);
            }
          } // 4. 遍历 selectedHistory，把对应组从 tmpList 中提取出来放后面
          ...
          for (let group of tmpList) {
            if (group.some(card => selectedHistory.includes(card))) {
...
//# sourceMappingURL=89a43bd700c5a54c20e3138c9bb6b5f3a9da150e.js.map