        ...
        onBtnHint() {
          ...
          if (this.outCardList.length == 1 && ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            for (let i = 0; i < totalHints; i++) {
              ...
              if (!found) {
                const targetSize = hint[0] % 16; // 获取 hint 目标点数

                for (let group of grouped) {
                  if (group.length === 2) {
                    const groupSize = group[0] % 16;

                    if (groupSize >= targetSize) {
                      this.selectCardValue = [group[0]]; // 拆出一张
...
//# sourceMappingURL=89dbbc9e956e1b0333f1572b8333e8815b1392ca.js.map