    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function autoSortCards(cards) {
          const fixedGroups = [];
          const randomGroups = [];
          const used = new Set(); // 1. 王炸

          const jokers = cards.filter(c => getCardSize(c) >= 16);
          ...
          for (const list of map.values()) {
            if (list.length === 2 && !used.has(list[0])) {
              fixedGroups.push(list);
          ...
          for (const list of map.values()) {
            const remain = list.filter(c => !used.has(c));
          ...
          const remainCards = cards.filter(c => !used.has(c));
          const planes = findFeiji(remainCards);
          const lianduis = findLiandui(remainCards);
          const shunzis = findStraightByCard(remainCards);
          const threeWithTwo = findThreeWithTwo(remainCards);
          const randomPool = [...planes, ...lianduis, ...shunzis, ...threeWithTwo]; // 标记这些牌不再参与后续处理
        ...
        function findThreeWithTwo(cards) {
          ...
          for (let triple of triples) {
            for (let pair of pairs) {
              const all = [...triple, ...pair];
              const allIds = all.map(c => c);
...
//# sourceMappingURL=f72dec7e28ef23a9fbbc3bc57d8b72ef3fba95bf.js.map