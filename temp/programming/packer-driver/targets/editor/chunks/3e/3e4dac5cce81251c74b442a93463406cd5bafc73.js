    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          const getRank = c => c % 16;

          const getColor = c => Math.floor(c / 16);
          ...
          const results = [];
          const usedIndex = new Set(); // 创建 rank -> [index] 映射

          const rankMap = new Map();

          for (let i = 0; i < cards.length; i++) {
            const rank = getSize(cards[i]);
          ...
          const allRanks = Array.from(rankMap.keys()).sort((a, b) => a - b); // 识别顺子（长度必须 == 5，不能复用）
          ...
          for (let i = 0; i <= allRanks.length - 5; i++) {
            ...
            for (let j = 0; j < 5; j++) {
              const rank = allRanks[i + j];
              const candidates = rankMap.get(rank);
              ...
              const idx = candidates.find(idx => !usedIndex.has(idx));
        ...
        function findBombsWithHeartCard(cards) {
          ...
          for (const [rank, list] of countMap) {
            const len = list.length; // 获取还可用的红心牌
...
//# sourceMappingURL=3e4dac5cce81251c74b442a93463406cd5bafc73.js.map