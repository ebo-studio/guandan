    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findFeiji(cards) {
          ...
          const sortedRanks = Object.keys(rankMap).map(Number).sort((a, b) => a - b);
          ...
          for (let i = 0; i < sortedRanks.length; i++) {
            ...
            if (list.length >= 3) {
              rankToTriplet[rank] = list.slice(0, 3); // 原生三张
            } else if (list.length === 2 && redLeft.length >= 1) {
              rankToTriplet[rank] = [...list, redLeft.shift()];
            } else if (list.length === 1 && redLeft.length >= 2) {
              rankToTriplet[rank] = [...list, redLeft.shift(), redLeft.shift()];
            }
          }

          const tripletRanks = Object.keys(rankToTriplet).map(Number).sort((a, b) => a - b);

          for (let i = 0; i < tripletRanks.length - 1; i++) {
            const a = tripletRanks[i];
            const b = tripletRanks[i + 1];

            if (b === a + 1) {
              const triplet1 = rankToTriplet[a];
              const triplet2 = rankToTriplet[b];
        ...
        function isFengRenPeiSorted(cards) {
          ...
          const ranks = Array.from(new Set(cards.filter(c => !isHeartCard(c)).map(c => c % 16).filter(r => r >= 3 && r <= 13) // 排除 2、大小王
          )).sort((a, b) => a - b);
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          for (let s of straights) {
            ...
            if (isSameColorStraight) {
              // 检查是否为同花顺
              const firstCardColor = s[0] % 4;
              const isSameColor = s.every(card => card % 4 === firstCardColor);
        ...
        function getCardTypeByFeiji(cards) {
          ...
          const tripletRanks = [];
          let redLeft = redCount; // 遍历点数，尝试补出三张
...
//# sourceMappingURL=bd9809ef8a4afb3854f1c3cdd06cd849c1f41c87.js.map