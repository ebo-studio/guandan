    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex, maxCount = 1) {
          ...
          // return []; // 找不到合法三带二
          const result = [];
          const countMap = new Map(); // 分组：按点数归类（排除特殊点数）

          for (let i = 0; i < cards.length; i++) {
            const card = cards[i];
            const rank = card % 16;
            const color = Math.floor(card / 16); // ❌ 排除非红桃的级牌2、大王、小王、参谋
          ...
          const triples = Array.from(countMap.entries()).filter(([_, list]) => list.length >= 3).sort((a, b) => a[0] - b[0]); // 所有对子（按点数升序）

          const pairs = Array.from(countMap.entries()).filter(([_, list]) => list.length >= 2).sort((a, b) => a[0] - b[0]);
          const usedCardIndex = new Set(usedIndex);
          ...
          for (const [tripleRank, tripleList] of triples) {
            ...
            for (const [pairRank, pairList] of pairs) {
              ...
              const group = [triple[0].card, triple[1].card, triple[2].card, pair[0].card, pair[1].card]; // 标记为已使用
...
//# sourceMappingURL=c3f542c0b222fec2fb892b0da0eafacb48c390f8.js.map