    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          const results = []; // 找所有三张（点数升序）

          const triples = Array.from(map.entries()).filter(([_, list]) => list.length >= 3).sort((a, b) => a[0] - b[0]);

          for (const [rank, tripleList] of triples) {
            const three = tripleList.slice(0, 3); // 寻找任意一个不同于三张点数的两张（作为对子）

            const pair = Array.from(map.entries()).filter(([r, list]) => r !== rank && list.length >= 2).sort((a, b) => a[0] - b[0]) // 优先较小对子
            .map(([_, list]) => list.slice(0, 2))[0];
...
//# sourceMappingURL=4fed0691ded506973d6e14030a6337b095cfb6cb.js.map