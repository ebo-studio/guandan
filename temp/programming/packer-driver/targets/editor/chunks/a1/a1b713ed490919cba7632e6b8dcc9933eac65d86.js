    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findStraightWithHongtao2_Safe(cards) {
          ...
          const isHeartLevelCard = card => getRank(card) === 2 && getColor(card) === HONGTAO; // 拆分红心级牌与其他牌
        ...
        function isFengRenPeiSorted(cards) {
          ...
          const heartCount = heartCards.length;
          if (heartCount > 2) return false; // 最多允许2张红心级牌

          const ranks = Array.from(new Set(cards.filter(c => !isHeartCard(c)).map(c => c % 16).filter(r => r >= 3 && r <= 13))).sort((a, b) => a - b);
          if (ranks.length + heartCount < 5) return false; // AKQJ10 特殊顺子

          const akqj10 = [10, 11, 12, 13, 1];
          const akqj10Count = akqj10.filter(r => ranks.includes(r)).length;
...
//# sourceMappingURL=a1b713ed490919cba7632e6b8dcc9933eac65d86.js.map