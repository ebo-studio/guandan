      ...
      (function (_GameLogic) {
        ...
        function findBombsWithHeartCard(cards) {
          ...
          for (const [rank, list] of countMap) {
            const len = list.length; // 普通炸弹（4张或以上）
        ...
        function findThreeWithTwoBy(cards, usedIndex = new Set(), maxCount = 2) {
          ...
          const pairs = Array.from(countMap.entries()).filter(([_, list]) => list.length === 2).sort((a, b) => a[0] - b[0]);
        ...
        function countThreeWithTwoAndStraight(unused) {
          const usedIndex = new Set(); // 标记用过的牌索引

          const rankMap = new Map(); // 点数 -> 索引列表
          // 分组：点数 -> 所有索引
          ...
          const sortedRanks = Array.from(rankMap.keys()).sort((a, b) => b - a); // 大点优先
          // ✅ Step 1: 找三带二
          ...
          for (const mainRank of sortedRanks) {
            ...
            if (mainIdxList.length >= 3) {
              const three = mainIdxList.slice(0, 3); // 找对子（不能是同点数）
...
//# sourceMappingURL=795517ec1dde053ff8f5a145d8091ccc48dc553b.js.map