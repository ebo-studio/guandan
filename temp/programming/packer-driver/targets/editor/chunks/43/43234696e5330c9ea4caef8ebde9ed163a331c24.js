    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          const map = getCardCountMap(cards);
          const results = []; // 所有三张牌（按牌点从小到大）

          const triples = Array.from(map.entries()).filter(([rank, group]) => group.length >= 3).sort((a, b) => getCardSize(a[0]) - getCardSize(b[0]));

          for (const [rank, group] of triples) {
            // 找3张还没用过的
            const threeCards = [];
            ...
            for (const [r, g] of map.entries()) {
              if (r === rank || g.length < 2) continue;
              const tempPair = [];

              for (let i = 0; i < cards.length; i++) {
                if (!usedIndex.has(i) && getCardSize(cards[i]) === r) {
                  tempPair.push(cards[i]);
            ...
            const full = [...threeCards, ...pair];

            for (let i = 0; i < cards.length; i++) {
              if (full.includes(cards[i]) && !usedIndex.has(i)) {
                usedIndex.add(i);
                full[full.indexOf(cards[i])] = NaN; // 避免重复标记
              }
            }
...
//# sourceMappingURL=43234696e5330c9ea4caef8ebde9ed163a331c24.js.map