    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findStraightByCard(cards) {
          ...
          const rankMap = new Map(map); // 深拷贝用于消耗

          const results = []; // 所有合法点数范围：3 ~ A(14)

          const legalRanks = Array.from({
            length: 12
          }, (_, i) => i + 3); // [3, 4, ..., 14]

          for (let start = 0; start <= legalRanks.length - 5; start++) {
            for (let len = 5; start + len <= legalRanks.length; len++) {
              const seq = legalRanks.slice(start, start + len); // 取出一段连续 rank
              // 检查每个 rank 是否存在一张牌
...
//# sourceMappingURL=74046695e917244fc54a0b09754aebc9ed0e89b2.js.map