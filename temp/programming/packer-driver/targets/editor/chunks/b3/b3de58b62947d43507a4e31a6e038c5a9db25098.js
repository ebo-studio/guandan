    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          // 基础分组
          const result = [];
          const moveToBack = []; // 找出新选中的牌（这一次比上一次多出来的）

          const prevSet = new Set(prevSelected);
          const newlySelected = selected.filter(c => !prevSet.has(c)); // 更新历史选中牌

          for (const card of newlySelected) {
            if (!selectedHistory.includes(card)) {
              selectedHistory.push(card);
            }
          }

          const selectedSet = new Set(selectedHistory); // 分组并处理
          ...
          for (let j = 0; j < uniqueList.length; j++) {
            ...
            for (let i = idx; i < sortValue.length; i++) {
              if (getCardSize(d) === getCardSize(sortValue[i])) {
                ...
                if (i === sortValue.length - 1) {
                  if (list1.length) {
                    if (list1.some(card => selectedSet.has(card))) {
...
//# sourceMappingURL=b3de58b62947d43507a4e31a6e038c5a9db25098.js.map