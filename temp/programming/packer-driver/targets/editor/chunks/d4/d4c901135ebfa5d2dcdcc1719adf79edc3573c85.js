    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function checkIfSameColorOrFengRenPei(straight) {
          ...
          const isSameColor = straight.every(card => getCardColor(card) === firstCardColor); // const firstCardColor = straight[0] % 4;
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          const cardsWithIndex = cards.map((card, index) => ({
            card,
            index
          })); // ✅ 保留旧分组中未重新选中的牌（按 index 判断）

          for (const group of prevGrouped) {
            if (!group.some(c => selectedSet.has(c))) {
              result.push(group);
              group.forEach(c => {
                const idx = cards.findIndex((x, i) => x === c && !usedIndex.has(i));
                if (idx !== -1) usedIndex.add(idx);
              });
            }
          }

          const getUnusedCards = () => cardsWithIndex.filter(({
            index
          }) => !usedIndex.has(index)).map(({
            card
          }) => card);

          const markUsed = group => {
            for (const card of group) {
              const idx = cards.findIndex((c, i) => c === card && !usedIndex.has(i));
              if (idx !== -1) usedIndex.add(idx);
            }
          }; // ✅ 王炸


          const bomsKing = findRocket(getUnusedCards());
          ...
          const bombs = findBombsWithHeartCard(getUnusedCards());
          ...
          const triplets = findFeiji(getUnusedCards());
          ...
          if (selected.length === 5) {
            const map = getTripleWithPairPreferBig(getUnusedCards());
          ...
          const straights = findStraightByCard(getUnusedCards());

          for (const s of straights) {
            const isSameColor = (() => {
              const first = s.find(c => !isHeartCard(c));
              if (!first) return false;
              const color = getCardColor(first);
              return s.every(c => isHeartCard(c) || getCardColor(c) === color);
            })();
          ...
          const lianduiGroups = findLiandui(getUnusedCards());

          for (const group of lianduiGroups) {
            moveToBack.push(group);
            markUsed(group);
          } // ✅ 对子 / 三条


          const countMap = getCardCountMap_I(getUnusedCards());

          for (const list of countMap.values()) {
          ...
          const finalRemain = cardsWithIndex.filter(({
            index
          }) => !usedIndex.has(index));
          const sortValue = sortCardsBySizeDown(finalRemain.map(obj => obj.card), finalRemain.length);
          const grouped = [];

          while (sortValue.length > 0) {
            const first = sortValue[0];
            const rank = getCardSize(first);
            const group = sortValue.filter(c => getCardSize(c) === rank);
            grouped.push(group);
            sortValue.splice(0, group.length);
          }
...
//# sourceMappingURL=d4c901135ebfa5d2dcdcc1719adf79edc3573c85.js.map