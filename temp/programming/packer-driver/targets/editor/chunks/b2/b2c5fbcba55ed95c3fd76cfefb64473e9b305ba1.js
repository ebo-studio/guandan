    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          const map = getCardCountMap(cards);
          const rankGroups = []; // 先收集所有三张及以上的组合
          ...
          for (const triple of rankGroups) {
            // 查找未被使用的对子（不能用其他三张或炸弹里的）
            const usedRanks = new Set(rankGroups.map(g => g.rank));
            const pairs = [];

            for (const [rank, group] of map.entries()) {
              if (rank === triple.rank || usedRanks.has(rank)) continue;
              ...
              if (group.length >= 2) {
                ...
                for (let i = 0; i < cards.length; i++) {
                  ...
                  if (!usedIndex.has(i) && getCardSize(card) === rank) {
                    pair.push(card);
            ...
            if (pairs.length > 0) {
              ...
              for (let i = 0; i < cards.length; i++) {
                if (!usedIndex.has(i) && getCardSize(cards[i]) === triple.rank) {
              ...
              if (threeCards.length === 3) {
                const combo = [...threeCards, ...pairs[0]]; // 标记使用索引

                for (let i = 0; i < cards.length; i++) {
                  if (combo.includes(cards[i])) {
                    usedIndex.add(i); // 避免重复添加同一张

                    combo[combo.indexOf(cards[i])] = NaN;
                  }
                }
...
//# sourceMappingURL=b2c5fbcba55ed95c3fd76cfefb64473e9b305ba1.js.map