    ...
    execute: function () {
      ...
      _export("CardLayer", CardLayer = (_dec = ccclass('CardLayer'), _dec2 = property(Prefab), _dec3 = property(SpriteAtlas), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(_crd && GameTimer === void 0 ? (_reportPossibleCrUseOfGameTimer({
        ...
        updateSelectedCards_V() {
          ...
          let isRecover = this.isSelectedFromSortedGroup(this.handCardsValue, this.sorthandCardsValue, this.selectCardValue);
        ...
        isSelectedFromSortedGroup(cards, sorthandCardsValue, selected) {
          // 标记 sorthandCardsValue 中使用了哪些 index（在 cards 里的）
          const used = new Set();
          const count = new Map();

          for (const val of sorthandCardsValue) {
          ...
          for (let i = cards.length - 1; i >= 0; i--) {
            ...
            if ((count.get(c) || 0) > 0) {
              used.add(i); // 记录这个 index 被理牌用了

              count.set(c, count.get(c) - 1);
            }
          } // 现在找到 selected 在 cards 中的 index，如果它在 used 中就返回 true
          ...
          for (let i = cards.length - 1; i >= 0; i--) {
            ...
            if ((selectedCount.get(c) || 0) > 0) {
              if (used.has(i)) return true; // 命中了理牌中那张牌
...
//# sourceMappingURL=100b35a71e7e3726857a9b6e4bc171b39319434c.js.map