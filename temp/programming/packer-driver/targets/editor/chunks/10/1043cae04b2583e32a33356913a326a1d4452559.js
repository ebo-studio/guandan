    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          for (const [rank, same] of map.entries()) {
            if (same.length >= 3) {
              triples.push({
                rank,
          ...
          for (const triple of triples) {
            ...
            for (const tCard of triple.cards) {
              const idx = cards.findIndex((c, i) => c === tCard && !usedIndex.has(i));
            ...
            if (tripleIdx.length !== 3) continue; // 查找对子

            const remains = cards.map((c, i) => ({
              c,
              i
            })).filter(({
              c,
              i
            }) => !usedIndex.has(i) && !triple.cards.includes(c));
            ...
            for (const [_, idxList] of pairMap) {
              if (idxList.length >= 2) {
                const group = [];
                tripleIdx.forEach(i => {
                  usedIndex.add(i);
                ...
                const pairIdx = idxList.slice(0, 2);
                pairIdx.forEach(i => {
                  usedIndex.add(i);
                  group.push(cards[i]);
                });
...
//# sourceMappingURL=1043cae04b2583e32a33356913a326a1d4452559.js.map