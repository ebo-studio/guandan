      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          for (let s of straights) {
            ...
            markUsedByCardValues(cards, s, usedIndex); // s.forEach(c => usedCards.add(c));
          } // ✅ 步骤6：连对识别
          ...
          // // 最后返回结果，炸弹已经在最前面
          // return [...result, ...moveToBack];
          // ✅ 步骤9：处理未使用牌（保持顺序、按点数分组）


          const remaining = cards.filter((_, idx) => !usedIndex.has(idx));
          const sortValue = sortCardsBySizeDown(remaining, remaining.length); // 分组逻辑替换掉 uniqueList + idx 遍历方式
...
//# sourceMappingURL=10d3ffa0cbab555e37c3f63e774956affaea5d54.js.map