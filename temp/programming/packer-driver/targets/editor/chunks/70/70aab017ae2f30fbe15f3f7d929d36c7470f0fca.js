    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          const getCardSize = card => card % 16;

          const cardCountMap = new Map(); // 分组：点数 => 对应所有牌（不区分花色）
          ...
          const triples = []; // 收集所有三张

          for (const [rank, indices] of cardCountMap.entries()) {
            if (indices.length >= 3) {
          ...
          for (const three of triples) {
            // 标记这三张的点数
            const tripleRank = getCardSize(cards[three[0]]); // 查找未使用、且不是同 rank 的对子

            let pair = null;

            for (const [rank, indices] of cardCountMap.entries()) {
              if (rank === tripleRank || indices.length < 2) continue;
              const [i1, i2] = indices;
            ...
            if (pair) {
              // 合并索引并标记
              const allIndices = [...three, ...pair];
              const group = allIndices.map(idx => cards[idx]);
...
//# sourceMappingURL=70aab017ae2f30fbe15f3f7d929d36c7470f0fca.js.map