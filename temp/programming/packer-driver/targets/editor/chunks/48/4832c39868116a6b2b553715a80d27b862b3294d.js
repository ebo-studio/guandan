    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function getOneCardSizeList(value) {
          ...
          const triples = tmpList.filter(g => g.length === 3);
          const pairs = tmpList.filter(g => g.length === 2);
          const singles = tmpList.filter(g => g.length === 1);
          const others = tmpList.filter(g => g.length !== 1 && g.length !== 2 && g.length !== 3);
          const used = new Set(); // 三带二

          const sanDaiErList = [];

          for (let i = 0; i < triples.length; i++) {
            for (let j = 0; j < pairs.length; j++) {
              if (getCardSize(triples[i][0]) !== getCardSize(pairs[j][0]) && !used.has(i) && !used.has(1000 + j)) {
                sanDaiErList.push([...triples[i], ...pairs[j]]);
                used.add(i);
                used.add(1000 + j);
                break;
              }
          ...
          for (let i = 0; i < triples.length; i++) {
            ...
            for (let j = 0; j < singles.length; j++) {
              if (getCardSize(triples[i][0]) !== getCardSize(singles[j][0]) && !used.has(2000 + j)) {
                sanDaiYiList.push([...triples[i], ...singles[j]]);
                used.add(i);
                used.add(2000 + j);
                break;
              }
          ...
          const remainingPairs = pairs.filter((_, i) => !used.has(1000 + i));
          const lianDuiList = [];
          let temp = [];

          for (let i = 0; i < remainingPairs.length; i++) {
            if (temp.length === 0) {
              temp.push(remainingPairs[i]);
            } else {
              const last = getCardSize(temp[temp.length - 1][0]);
              const curr = getCardSize(remainingPairs[i][0]);

              if (curr === last + 1 && curr < 15) {
                temp.push(remainingPairs[i]);
              } else {
                if (temp.length >= 2) lianDuiList.push(temp.flat());
                temp = [remainingPairs[i]];
              }
            }
          }

          if (temp.length >= 2) lianDuiList.push(temp.flat());
          const usedPairCards = new Set(lianDuiList.flat());
          const finalPairs = remainingPairs.filter(pair => !usedPairCards.has(pair[0]));
          const finalTriples = triples.filter((_, i) => !used.has(i));
          const finalSingles = singles.filter((_, i) => !used.has(2000 + i)); // 同花顺/顺子识别

          const remainingForStraight = [...finalSingles.flat(), ...finalTriples.flat(), ...finalPairs.flat(), ...others.flat()];
          const sortedRemain = sortCardsBySizeUp(remainingForStraight, remainingForStraight.length);
          ...
          const findStraightGroups = cards => {
            ...
            for (let i = 0; i < unique.length; i++) {
              ...
              if (temp.length === 0) {
                ...
                const prev = getCardSize(temp[temp.length - 1]);
                const curr = getCardSize(cur);
          ...
          const straightGroups = findStraightGroups(sortedRemain);
          const tongHuaShunList = [];
          const shunziList = [];
          ...
          const usedStraight = new Set([...tongHuaShunList.flat(), ...shunziList.flat()]);
          const remainAfterStraight = sortedRemain.filter(c => !usedStraight.has(c)); // 最终组合并排序

          const tmpAll = [...tongHuaShunList, ...sanDaiErList, ...sanDaiYiList, ...lianDuiList, ...finalPairs, ...finalTriples, ...shunziList, ...remainAfterStraight.map(c => [c])];
        ...
        function getGroupPower(cards) {
          ...
          const allSame = isAllSameSize(cards);
        ...
        function isLianDui(cards) {
          ...
          for (let i = 0; i < cards.length; i += 2) {
            ...
            if (i >= 2) {
              const prev = getCardSize(cards[i - 2]);
              const curr = getCardSize(cards[i]);
...
//# sourceMappingURL=4832c39868116a6b2b553715a80d27b862b3294d.js.map