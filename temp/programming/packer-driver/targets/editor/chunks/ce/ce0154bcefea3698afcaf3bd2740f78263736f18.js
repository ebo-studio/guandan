    ...
    execute: function () {
      ...
      _export("CardLayer", CardLayer = (_dec = ccclass('CardLayer'), _dec2 = property(Prefab), _dec3 = property(SpriteAtlas), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(_crd && GameTimer === void 0 ? (_reportPossibleCrUseOfGameTimer({
        ...
        updateSelectedCards_V() {
          ...
          let isRecover = this.isRecoverGroupFromFlatGrouped(this.handCardsValue, this.sorthandCardsValue, this.selectCardValue);
        ...
        isRecoverGroupFromFlatGrouped(cards, // 当前手牌（有重复）
        grouped, // 理牌后展开数组（number[]）
        selected) {
          const selectedCount = new Map();

          for (const val of selected) {
            selectedCount.set(val, (selectedCount.get(val) || 0) + 1);
          } // 获取 selected 在 cards 中的 index 序列
          ...
          for (let i = 0; i < cards.length; i++) {
            const c = cards[i];
            const count = selectedCount.get(c) || 0;

            if (count > 0) {
              selectedIndexes.push(i);
              selectedCount.set(c, count - 1);
              used.add(i);
            }
          } // 获取 grouped 中的每组组合（通过 cards 做匹配）


          const groupedUsed = new Set();
          const groupedIndexGroups = [];
          let i = 0;

          while (i < grouped.length) {
            const group = [];
            group.push(grouped[i]); // 简单逻辑：一组牌用完就断（这里你可换成真实逻辑判断是否是一个有效牌组）

            let j = i + 1;

            while (j < grouped.length && (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).getCardSize(grouped[j]) >= (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).getCardSize(grouped[j - 1])) {
              group.push(grouped[j]);
              j++;
            } // 在 cards 中找对应 index
            ...
            for (let k = 0; k < cards.length; k++) {
              if (groupedUsed.has(k)) continue;
              const c = cards[k];

              if ((countMap.get(c) || 0) > 0) {
                idxGroup.push(k);
            ...
            groupedIndexGroups.push(idxGroup);
            i = j;
          }

          const sortedSel = [...selectedIndexes].sort((a, b) => a - b);
          return groupedIndexGroups.some(g => {
            const sortedGroup = [...g].sort((a, b) => a - b);
            return sortedGroup.length === sortedSel.length && sortedGroup.every((v, i) => v === sortedSel[i]);
          });
        } //连续点击两次 双击
        ...
        onBtnCardDir() {
          ...
          const newHandCards = [];
          const used = new Array(this.handCards.length).fill(false);
...
//# sourceMappingURL=ce0154bcefea3698afcaf3bd2740f78263736f18.js.map