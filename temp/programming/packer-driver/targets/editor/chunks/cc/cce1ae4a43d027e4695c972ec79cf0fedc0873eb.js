    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards) {
          ...
          const selectedThree = threeList[0];
          const threeRank = getCardSize(selectedThree[0]); // 找一个不与三张重复的对子

          const selectedPair = pairList.find(p => getCardSize(p[0]) !== threeRank);
        ...
        function smartSortCards(cards) {
          ...
          const mainResult = []; // 主牌型：王炸、炸弹、同花顺、大王、小王、级牌

          const scatteredPart = []; // 散牌（单张、对子、三张）

          const randomPart = []; // 杂牌型（顺子、连对、飞机、三带二）
          ...
          const scatteredGrouped = groupBySizeDown(remain);
...
//# sourceMappingURL=cce1ae4a43d027e4695c972ec79cf0fedc0873eb.js.map