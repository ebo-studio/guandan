    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, groupedCards) {
          ...
          const moveToBack = [];
          const selectedSet = new Set(selected);
          const prevSet = new Set(prevSelected); // 选中历史（按顺序追加）
          ...
          for (const group of groupedCards) {
            const overlap = group.filter(c => selectedSet.has(c));
          ...
          const regroupList = cards.filter(c => !groupedCards.flat().includes(c) || restoreSet.has(c)); // 先识别牌型组合

          const usedSet = new Set(); // ✅ 三带二识别
          ...
          const straightList = findStraightByCard(selected.filter(c => !usedSet.has(c)));

          for (const group of straightList) {
            group.forEach(c => usedSet.add(c));
            moveToBack.push(group);
          } // ✅ 连对识别


          const lianduiList = findLiandui(selected.filter(c => !usedSet.has(c)));

          for (const group of lianduiList) {
            group.forEach(c => usedSet.add(c));
            moveToBack.push(group);
          } // ✅ 飞机识别


          const feijiList = findFeiji(selected.filter(c => !usedSet.has(c)));

          for (const group of feijiList) {
            group.forEach(c => usedSet.add(c));
            moveToBack.push(group);
          } // ✅ 将剩余牌进行普通分组


          const sortValue = sortCardsBySizeDown(regroupList, regroupList.length);
          const uniqueList = getUniqueCard(sortValue);
          let idx = 0;
        ...
        function findPlane(cards) {
          const map = GameLogic.getCardCountMap(cards);
...
//# sourceMappingURL=fe23f993263203980be044bb1581fe32bb7c3e9b.js.map