    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards) {
          ...
          const getSize = card => card % 16; // 获取张数映射
          ...
          const usedSize = new Set(); // 获取所有 >=3 的点数（按牌点从小到大排序）

          const threeRanks = Array.from(map.entries()).filter(([_, list]) => list.length >= 3).sort((a, b) => getSize(a[0]) - getSize(b[0]));

          for (const [rank, list3] of threeRanks) {
            const three = list3.slice(0, 3);
            usedSize.add(rank); // 找带的对子（不能是同一 rank，不能是王或级牌）

            const pairRank = Array.from(map.entries()).filter(([r, list]) => list.length >= 2 && r !== rank && !usedSize.has(r) && getSize(r) < 14 && // 排除大王(14)、小王(15)
            getSize(r) !== (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.levelCard // 排除级牌 2
            ).sort((a, b) => getSize(a[0]) - getSize(b[0])) // 小点优先
            .map(([r, list]) => list.slice(0, 2))[0]; // 取第一个合法的对子
...
//# sourceMappingURL=d8c6a2b2a792fdbfca3205ad250d2e7142dd07bf.js.map