    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          const pairList = []; // for (const [rank, list] of countMap.entries()) {
          //     if (list.length === 2) {
          //         pairList.push(list);
          //     }
          // }
        ...
        function getCardType(cards) {
          ...
          if (cards.length === 5) {
            ...
            const nonHeart = cards.filter(c => !isHeartCard(c)); // 查找三张相同点数的非红心牌
        ...
        function getCardCountMap_I(cards) {
          ...
          for (const card of heartCards) {
            ...
            for (const [rank, list] of map.entries()) {
              if (list.length < 4 && list.every(c => c % 16 === rank)) {
                // 仅补到点数一致的组
            ...
            if (targetRank !== null) {
              map.get(targetRank).push(card);
            } else {
              // ❗️如果没有合适的目标，就保留为单独 key（防止遗漏）
              map.set(card % 16, [card]);
            }
          }
...
//# sourceMappingURL=d6c2049e3e1a422a2cba907b2493fd2c57aa1891.js.map