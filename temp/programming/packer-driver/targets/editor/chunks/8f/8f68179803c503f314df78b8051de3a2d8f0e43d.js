    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          const result = []; // 找所有 >=3 的点数，按点数升序（越小的在前）

          const tripleRanks = Array.from(map.keys()).filter(rank => map.get(rank).length >= 3).sort((a, b) => a - b);

          for (const rank of tripleRanks) {
            const three = map.get(rank).slice(0, 3); // 找其他任意一个不同点数的对子（最小的）

            const pairRank = Array.from(map.keys()).filter(r => r !== rank && map.get(r).length >= 2).sort((a, b) => a - b)[0];

            if (pairRank !== undefined) {
              const pair = map.get(pairRank).slice(0, 2);
...
//# sourceMappingURL=8f68179803c503f314df78b8051de3a2d8f0e43d.js.map