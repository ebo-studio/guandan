    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          const map = new Map(); // 构建 rank -> cards 映射
          ...
          const tripleRanks = Array.from(map.keys()).filter(rank => map.get(rank).length >= 3).sort((a, b) => a - b); // 从小到大，优先出小的

          for (const rank of tripleRanks) {
            const three = map.get(rank).slice(0, 3); // 找不同点数的最小对子
...
//# sourceMappingURL=4b1b2ad3d452644c6c25f4677872ffb440fd6c2c.js.map