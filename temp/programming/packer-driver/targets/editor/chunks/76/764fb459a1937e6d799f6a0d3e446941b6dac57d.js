    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          const prevSet = new Set(prevSelected); // 获取本次新增选中的牌（diff）

          const newSelected = selected.filter(card => !prevSet.has(card));
          const newSelectedCopy = (_crd && utils === void 0 ? (_reportPossibleCrUseOfutils({
            error: Error()
          }), utils) : utils).deepCopy(newSelected);
          const remain = []; // 从原始手牌中剔除这次新增的牌

          for (let i = 0; i < cards.length; i++) {
            const v = cards[i];
            const idx = newSelectedCopy.indexOf(v);
          ...
          const orderedNewGroup = [];
          const tempNew = [...newSelected];

          for (let card of cards) {
            const idx = tempNew.indexOf(card);
...
//# sourceMappingURL=764fb459a1937e6d799f6a0d3e446941b6dac57d.js.map