    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findStraightByCard(cards) {
          ...
          const uniqueRanks = [...rankToIndexes.keys()].sort((a, b) => a - b);

          for (let start = 0; start < uniqueRanks.length; start++) {
            let group = [];
            let usedThisGroup = new Set();
            let heartUsed = 0;
            let expected = uniqueRanks[start];
            let current = expected;
            let index = start;

            while (true) {
              const hasRank = rankToIndexes.has(current);

              if (hasRank) {
                const i = rankToIndexes.get(current).find(i => !usedIndexes.has(i) && !usedThisGroup.has(i));
                ...
                if (i !== undefined) {
                  ...
                  // 无可用牌，尝试补红心
                  const red = redIndexes.find(i => !usedIndexes.has(i) && !usedThisGroup.has(i));
...
//# sourceMappingURL=a734cea21b5c5279990a4979d6e373334b8639f8.js.map