    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          const triples = Array.from(map.entries()).filter(([rank, group]) => group.length >= 3).map(([rank, group]) => ({
            rank,
            group: group.slice(0, 3)
          })).sort((a, b) => a.rank - b.rank); // 小牌在前
          // 找出所有可用对子（排除三张的点数）

          const pairs = Array.from(map.entries()).filter(([rank, group]) => group.length >= 2).map(([rank, group]) => ({
            rank,
            group: group.slice(0, 2)
          })).sort((a, b) => a.rank - b.rank); // 小牌在前

          for (const triple of triples) {
            const pair = pairs.find(p => p.rank !== triple.rank);

            if (pair) {
              return [[...triple.group, ...pair.group]];
            }
          }
...
//# sourceMappingURL=632fc64a9dc045b6492bb1718f5df6be21e0d438.js.map