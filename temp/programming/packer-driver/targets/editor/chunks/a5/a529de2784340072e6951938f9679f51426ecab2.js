    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findFeiji(cards) {
          const map = getCardCountMap(cards);
          const rankToCards = new Map(); // 收集所有三张牌的 rank 和对应的牌值
          ...
          const sortedRanks = Array.from(rankToCards.keys()).sort((a, b) => a - b); // 寻找连续的牌点 rank 组成的飞机

          const feiji = [];

          for (let i = 0; i < sortedRanks.length - 1; i++) {
            const start = sortedRanks[i];
            let group = [rankToCards.get(start)];
...
//# sourceMappingURL=a529de2784340072e6951938f9679f51426ecab2.js.map