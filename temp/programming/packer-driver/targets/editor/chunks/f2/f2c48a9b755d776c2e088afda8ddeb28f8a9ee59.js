    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          const remaining = selected.filter(card => !usedCards.has(card)); // ✅ 只处理选中的未用牌

          const sortValue = sortCardsBySizeDown(remaining, remaining.length); // 用 Map 按点数分组

          const rankMap = new Map();

          for (const card of sortValue) {
            const rank = getCardSize(card); // 点数
          ...
          for (const list of rankMap.values()) {
            if (list.some(c => selectedSet.has(c))) {
              moveToBack.push(list);
            } else {
              result.push(list);
            }
...
//# sourceMappingURL=f2c48a9b755d776c2e088afda8ddeb28f8a9ee59.js.map