    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findBombsWithHeartSupport(groups) {
          const result = [];
          const heartGroup = groups.find(g => g.length === 1 && isHeartCard(g[0]));
          if (!heartGroup) return result;
          const heartCard = heartGroup[0];

          for (const group of groups) {
            if (group === heartGroup) continue;

            if (group.length === 3) {
              const merged = [...group, heartCard];
              const bombs = GameLogic.newfindBombs(merged);
...
//# sourceMappingURL=94e1a20fac296b669698d37975486b73eb0494df.js.map