    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findBombsWithHeartCard(cards) {
          ...
          const bombs = [];
          const heartCards = cards.filter(card => isHeartCard(card)); // 查找三张及以上相同点数的牌，尝试组合炸弹（含逢人配）
          ...
          for (const [rank, list] of countMap) {
            ...
            if (list.length >= 3 && list.length < 6) {
              // 查找可用的红心级牌（未出现在该list中）
              const usedSet = new Set(list);
              const availableHeartCards = heartCards.filter(card => !usedSet.has(card)); // 可组合成炸弹（最多补成6炸）

              for (let i = 0; i < Math.min(6 - list.length, availableHeartCards.length); i++) {
                const bomb = [...list, ...availableHeartCards.slice(0, i + 1)];
                bombs.push(bomb);
              }
            }
          }
        ...
        function getHintCards(targetCards, groupedCards) {
          ...
          switch (type) {
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              if (hintList.length === 0) {
                for (let i = groupedCards.length - 1; i >= 0; i--) {
                  const group = groupedCards[i]; // 只判断是否为炸弹（四张及以上，同点数）
...
//# sourceMappingURL=47477fd32dba5c9ebbaca5e77a758b6846e39738.js.map