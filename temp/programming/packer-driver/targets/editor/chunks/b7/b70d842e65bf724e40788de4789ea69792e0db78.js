    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          const getCardSize = card => card % 16; // 先从 cards 中过滤出未被使用的牌


          const unusedCards = cards.map((c, i) => ({
            card: c,
            idx: i
          })).filter(({
            idx
          }) => !usedIndex.has(idx)); // 构建 size -> card[] 映射

          const sizeMap = new Map();

          for (const item of unusedCards) {
            const size = getCardSize(item.card);
          ...
          for (const [size, group] of sizeMap.entries()) {
            const candidates = group.filter(x => !usedTemp.has(x.idx));
          ...
          for (const {
            ...
            const pair = pairs.find(p => !usedFinal.has(p[0].idx) && !usedFinal.has(p[1].idx));

            if (pair) {
              const fullGroup = [...main, ...pair];
...
//# sourceMappingURL=b70d842e65bf724e40788de4789ea69792e0db78.js.map