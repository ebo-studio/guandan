    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards) {
          const map = getCardCountMap(cards);
          const results = [];
          const used = new Set(); // 获取所有三张的组合（排除2和大小王）
          ...
          for (const triple of triples) {
            const tripleRank = getCardSize(triple[0]);
            const tripleSet = new Set(triple); // 尝试从剩下的牌中找两个点数一样的（对），或两个不同单张

            const others = cards.filter(c => !tripleSet.has(c));
            const pairMap = getCardCountMap(others);
            let pair = []; // 找对子

            for (const list of pairMap.values()) {
              const rank = getCardSize(list[0]);
        ...
        function smartSortCards(cards) {
          ...
          const getUnused = () => cards.filter(c => !used.has(c)); // 0. 王炸
        ...
        function findChainPairs(cards) {
          const map = getCardCountMap(cards); // const ranks = Array.from(map.keys()).filter(r => map.get(r)!.length >= 2).sort((a, b) => a - b);
...
//# sourceMappingURL=b74d2d1a6653430bc4eced24e3bb6b881dcee982.js.map