    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function getOneCardSizeList(value) {
          ...
          const others = tmpList.filter(g => g.length !== 2); // 连对识别
          ...
          for (let i = 0; i < pairs.length; i++) {
            if (temp.length === 0) {
              ...
              const curr = getCardSize(pairs[i][0]);
          ...
          const remainingPairs = pairs.filter(pair => !usedPairCards.has(pair[0])); // 顺子 & 同花顺识别

          const remainCards = [...remainingPairs.flat(), ...others.flat()];
          ...
          const remainAfterStraight = sortedRemain.filter(c => !usedStraight.has(c)); // 单张转二维

          const remainSingleList = remainAfterStraight.map(c => [c]);
          const tmpAll = [...tongHuaShunList, ...lianDuiList, ...shunziList, ...remainingPairs, ...others, ...remainSingleList];
...
//# sourceMappingURL=2aa66d6a604948c68feb6fc8bcec5582f9d0df0f.js.map