    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          for (const indexes of rankMap.values()) {
            const group = indexes.map(i => cards[i]); // 当前组

            const hasSelected = indexes.some(i => selectedIndexSet.has(i)); // 判断是否被选中
            // 将选中的组移到末尾，未选中的保留在前面

            (hasSelected ? moveToBack : result).push(group); // 标记这些牌已使用
...
//# sourceMappingURL=99789e7a186295a5005f94142d9838f60a868557.js.map