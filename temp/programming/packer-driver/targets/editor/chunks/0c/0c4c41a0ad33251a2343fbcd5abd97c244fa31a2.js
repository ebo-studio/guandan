      ...
      (function (_GameLogic) {
        ...
        function autoSortCards(cards) {
          ...
          const getRemain = (isRandom = false) => {
            return cards.filter(c => isRandom ? !used.has(c) : !used.has(c)); // 判断是否已经用过
          }; // ✅ 固定牌型优先处理
          ...
          addGroup(findAllSingles(getRemain())); // ✅ 随机牌型放最后：顺子、连对、飞机、三带二（随机打乱顺序，选择一组）

          const randomGroups = [shuffleArray(findFeiji(getRemain(true))), // 飞机
...
//# sourceMappingURL=0c4c41a0ad33251a2343fbcd5abd97c244fa31a2.js.map