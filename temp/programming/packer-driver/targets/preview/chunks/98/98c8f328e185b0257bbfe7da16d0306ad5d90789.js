    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findStraightByCard(cards) {
          ...
          var allRanks = Array.from(map.keys()).filter(r => r >= 3 && r <= 14) // 排除 2 和大小王
          .sort((a, b) => a - b);
          var results = [];
          var temp = [];

          for (var i = 0; i < allRanks.length; i++) {
            var rank = allRanks[i]; // 连续则继续，不连续则重置 temp

            if (temp.length === 0 || rank === temp[temp.length - 1] + 1) {
              temp.push(rank);
            } else {
              temp = [rank];
            } // 当达到5连时，构建顺子组
            ...
            if (temp.length >= 5) {
              ...
              if (success) {
                results.push([...group]); // shift 掉每张已用牌（只用1张）
...
//# sourceMappingURL=98c8f328e185b0257bbfe7da16d0306ad5d90789.js.map