    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          var _loop = function _loop(tripleList, _rank2) {
            var three = tripleList.slice(0, 3); // 寻找任意一个不同于三张点数的两张（作为对子）

            var pair = Array.from(map.entries()).filter(_ref2 => {
              var [r, list] = _ref2;
              return r !== _rank2 && list.length >= 2;
            }).sort((a, b) => a[0] - b[0]) // 优先较小对子
            .map(_ref3 => {
              var [_, list] = _ref3;
            ...
            if (pair) {
              results.push([...three, ...pair]);
              return "break"; // 只取一个
            }
          };

          for (var [_rank2, tripleList] of triples) {
            var _ret = _loop(tripleList, _rank2);
...
//# sourceMappingURL=3b2e02e5a5aeb8c491c04a9a60ee00e032bd744d.js.map