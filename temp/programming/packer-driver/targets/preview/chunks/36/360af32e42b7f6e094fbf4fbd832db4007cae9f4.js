    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function autoSortCards(cards) {
          ...
          var used = new Set(); // 用于标记已使用的牌（不论是否被组合）

          var usedForRandom = new Set(); // 用于标记已用于随机牌型的牌

          var usedForCombination = new Set(); // 用于标记参与组合的牌（避免在剩余牌时被误判为已使用）
          // 辅助方法：将已使用的牌加入结果
          ...
          var randomGroups = [shuffleArray(findThreeWithTwo(getRemain(false))), // 三带二
          shuffleArray(findFeiji(getRemain(false))), // 飞机
          shuffleArray(findLiandui(getRemain(false))), // 连对
          shuffleArray(findStraightByCard(getRemain(false))) // 顺子
          ]; // 只选择一个有效的随机牌型

          var randomGroup = randomGroups.find(group => group.length > 0); // 选择第一个有效的随机组
...
//# sourceMappingURL=360af32e42b7f6e094fbf4fbd832db4007cae9f4.js.map