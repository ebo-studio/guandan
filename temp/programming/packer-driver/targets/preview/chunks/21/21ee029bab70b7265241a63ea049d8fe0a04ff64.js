    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function autoSortCards(cards) {
          var result = [];
          var used = new Set();
          var sortCards = sortCardsBySizeDown(cards, cards.length); // ✅ 一键理牌：先识别固定顺序牌型
          ...
          }; // 固定优先级牌型：王炸 > 六炸 > 同花顺 > 五炸 > 四炸 > 对子 > 单牌


          addGroup(findWangzha(cards.filter(c => !used.has(c))));
          addGroup(findZ(cards.filter(c => !used.has(c)), 6)); // 六炸

          addGroup(findTonghuashun(cards.filter(c => !used.has(c))));
          addGroup(findZ(cards.filter(c => !used.has(c)), 5)); // 五炸

          addGroup(findZ(cards.filter(c => !used.has(c)), 4)); // 四炸
          // ✅ 随机排列的牌型（飞机、顺子、连对、三带二）——每次打乱顺序

          var randomTypes = shuffleArray([findFeiji, findStraightByCard, findLiandui, findThreeWithTwo]);

          for (var func of randomTypes) {
            var groups = func(cards.filter(c => !used.has(c)));
            addGroup(groups);
          } // ✅ 剩余未处理的牌分组（对子、单牌）


          var remain = cards.filter(c => !used.has(c));
          var sortValue = sortCardsBySizeDown(remain, remain.length);
          var uniqueList = getUniqueCard(sortValue);
          var idx = 0;
          ...
          return fixedGroups.concat(result);
        }
...
//# sourceMappingURL=21ee029bab70b7265241a63ea049d8fe0a04ff64.js.map