    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function getCardCountMap_I(cards) {
          ...
          } // for (const card of heartCards) {
          //     let targetKey: number | null = null;
          //     let minLen = Infinity;
          //     // 找一个 <4张的组，补进去（牌数越少越优先）
          ...
          //     // 有目标组就加进去
          //     if (targetKey !== null) {
          //         map.get(targetKey)!.push(card);
          //     } else {
          //         // 所有组都满了（极少见），补到任意组（比如最大长度组）
          //         const fallback = [...map.entries()].sort((a, b) => b[1].length - a[1].length)[0];
          //         fallback[1].push(card);
          //     }
          // }
        ...
        function findFlushStraight(cards) {
          ...
          for (var [_color, colorCards] of colorMap.entries()) {
            ...
            for (var _card4 of colorCards) {
              var _rank10 = _card4 % 16;

              if (!rankMap.has(_rank10)) rankMap.set(_rank10, []);
              rankMap.get(_rank10).push(_card4);
            }
        ...
        function countThreeWithTwoAndStraight(unused) {
          ...
          for (var _i13 = 0; _i13 < unused.length; _i13++) {
            var _rank13 = getCardSize(unused[_i13]);

            if (_rank13 >= 3 && _rank13 <= 14 && !usedIndex.has(_i13)) {
              if (!rankToIndexes.has(_rank13)) rankToIndexes.set(_rank13, []);
              rankToIndexes.get(_rank13).push(_i13);
            }
          }
...
//# sourceMappingURL=65dcdaab2c6afcb5aef5e5cba78b244ec884a637.js.map