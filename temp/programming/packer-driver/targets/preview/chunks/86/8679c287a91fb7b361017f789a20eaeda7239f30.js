    ...
    execute: function () {
      ...
      _export("CardLayer", CardLayer = (_dec = ccclass('CardLayer'), _dec2 = property(Prefab), _dec3 = property(SpriteAtlas), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(_crd && GameTimer === void 0 ? (_reportPossibleCrUseOfGameTimer({
        ...
        setHandCards(value, ani, isUp, isOneCard) {
          ...
          if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            var _loop = function _loop(i) {
              ...
              var posX = _this.getHandCardPosX(i);

              card.node.scale = new Vec3(_this.handScale, _this.handScale, _this.handScale); //手牌位置

              var cardSps = _this.getCardsSprite(_this.handCardsValue[i]);

              if (cardSps) {
                card.setValue(_this.handCardsValue[i], cardSps);
              } //有发牌动画,最后一张显示背面


              if (ani) {
                if (i == _this.handCardsValue.length - 1) {
                  card.setBack(true);
                }
              } //动画
            ...
            if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              ...
              (function () {
                ...
                _this.initHandStartPosX_V(value.length);
                ...
                var _loop2 = function _loop2(_i) {
                  ...
                  var posX = _this.getHandCardPosX_V(_i);

                  var posY = _this.getHandCardPosY_V(0); // 每列纵向，只显示一张


                  card.node.scale = new Vec3(_this.handScale, _this.handScale, _this.handScale);

                  var cardSps = _this.getCardsSprite(value1);

                  if (cardSps) {
                    card.setValue(value1, cardSps); // 是否显示炸弹标志

                    var list = [value1];
                    if ((_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                      error: Error()
                    }), GameLogic) : GameLogic).isNBomb(list, 6)) card.showBomb(6);else if ((_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                      error: Error()
                    }), GameLogic) : GameLogic).isNBomb(list, 5)) card.showBomb(5);else if ((_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                      error: Error()
                    }), GameLogic) : GameLogic).isSameSuitStraight(list)) card.showTonghua(true);else if ((_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                      error: Error()
                    }), GameLogic) : GameLogic).isNBomb(list, 4)) card.showBomb(4);
                  }

                  if (ani && _i == value.length - 1) {
                    card.setBack(true);
                  }
                  ...
                  card.setIndex(idx);
                  card.setBottom(true); // 每列只有一张牌

                  card.setLastLine(_i == value.length - 1);
              ...
              (function () {
                _this.handCardsValue = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                  error: Error()
                }), GameLogic) : GameLogic).sortCardsBySizeDown(_this.handCardsValue, _this.handCardsValue.length); // console.log("len---> ",value.length);
                ...
                _this.initHandStartPosX_V(sameSizeList.length);

                var idx = 0;
                var tmpIdx = 0; // console.log("---> ",sameSizeList[0][0]);

                var _loop3 = function _loop3(_i2) {
                  var list = sameSizeList[_i2];

                  var posX = _this.getHandCardPosX_V(_i2);
                  ...
                  var _loop4 = function _loop4(j) {
                    ...
                    if (ani) {
                      if (_i2 == sameSizeList.length - 1 && j == 0) {
                        card.setBack(true);
                        tmpIdx = idx;
                      }
                    }
                    ...
                    var moveFunc = function moveFunc(posX, posY, tmpIdx) {
                      if (ani) {
                        card.node.setPosition(v3(0, 0, 0));
                        tween(card.node).delay(_i2 * delayTime).call(() => {
                          if (tmpIdx == idx - 1) {
                            card.setBack(false);
                            that.setCanTouch(true);
                          }
                    ...
                    card.setIndex(idx);
                    card.setBottom(j == 0);
                    card.setLastLine(_i2 == sameSizeList.length - 1);
                    idx++;
                ...
                for (var _i2 = 0; _i2 < sameSizeList.length; _i2++) {
                  _loop3(_i2);
                }
              })();
            }
        ...
        dealOutCards(viewid, cardList, count, isAuto) {
          ...
          if (viewid == (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              ...
              (function () {
                //整理手牌
                var posY = _this2.getHandCardPosY(); //实际起始位置


                _this2.initHandStartPosX();

                var _loop5 = function _loop5(_j) {
                  var card = _this2.handCards[_j];
                  card.node.scale = v3(_this2.handScale, _this2.handScale, _this2.handScale);
                  card.setIndex(_j); //手牌位置

                  var posX = _this2.getHandCardPosX(_j);
                ...
                for (var _j = 0; _j < _this2.handCards.length; _j++) {
                  _loop5(_j);
                }
              })();
            } else {
              (function () {
                ...
                if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                  ...
                  sameSizeList = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).getOneCardSizeList(_this2.handCardsValue);
                } else {
                  sameSizeList = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).getSameCardSizeList(_this2.handCardsValue);
                }
                  ...
                  var _loop7 = function _loop7(_j2) {
                    ...
                    var _loop8 = function _loop8(n) {
                      ...
                      if (card.getValue() == valueSize && useIdxList.indexOf(card.getIndex()) == -1) {
                        ...
                        var posY = _this2.getHandCardPosY_V(_j2);
        ...
        onScreenTouchMove(event) {
          ...
          if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            for (var _i5 = this.handCards.length - 1; _i5 >= 0; _i5--) {
              var _item = this.handCards[_i5].node;
              ...
              if (_item != null) {
                ...
                var tmpPosXR = this.handCards[_i5].getLastLine() ? posXR : posXL + this.handDistance_V; //右边界点

                var tmpPosY = this.handCards[_i5].getBottom() ? _posyYB : _posY - this.cardDistance_V; //下边界点
                ...
                if (leftUp || leftDown || rightUp || rightDown) {
                  this.handCards[_i5].setMask(true);
                } else {
                  this.handCards[_i5].setMask(false);
                }
              }
            } //第一张
            ...
            for (var _i7 = this.handCards.length - 1; _i7 >= 0; _i7--) {
              var _card8 = this.handCards[_i7];
        ...
        onBtnCardDir() {
          ...
          var remain = (_crd && utils === void 0 ? (_reportPossibleCrUseOfutils({
            error: Error()
          }), utils) : utils).deepCopy(this.handCardsValue);
          var selectCopy = (_crd && utils === void 0 ? (_reportPossibleCrUseOfutils({
            error: Error()
          }), utils) : utils).deepCopy(selected);
          var newHand = [];

          var _loop9 = function _loop9(i) {
            var card = remain[i];
            var idx = selectCopy.findIndex(v => v === card);
          ...
          newHand = newHand.concat((_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
            error: Error()
          }), GameLogic) : GameLogic).getSortCard(selected)); // 更新显示
        ...
        smartSetHandCards(sortedCards) {
          ...
          for (var _i8 = 0; _i8 < sameSizeList.length; _i8++) {
            var col = sameSizeList[_i8]; // 一列内的牌

            var posX = this.getHandCardPosX_V(_i8);
            ...
            for (var _j5 = col.length - 1; _j5 >= 0; _j5--) {
              ...
              card.setLastLine(_i8 === sameSizeList.length - 1);
...
//# sourceMappingURL=8679c287a91fb7b361017f789a20eaeda7239f30.js.map