  ...
  return {
    ...
    execute: function () {
      ...
      _export("RaceKickItem", RaceKickItem = (_dec = ccclass('RaceKickItem'), _dec2 = property(_crd && AVirtualScrollView === void 0 ? (_reportPossibleCrUseOfAVirtualScrollView({
        ...
        startTime() {
          ...
          this.timeFunc = () => {
            for (var i = 0; i < listData.length; i++) {
              var item = listData[i]; //已报名,显示开赛倒计时
              ...
              if ((item.timeState == (_crd && RaceTimeType === void 0 ? (_reportPossibleCrUseOfRaceTimeType({
                ...
                var startTime = item.start_timestamp; //new Date(item.gameStartTime).valueOf();
                ...
                var _startTime = item.end_timestamp; //new Date(item.registerEndTime).valueOf();
                ...
                var _startTime2 = item.start_timestamp; //new Date(item.gameStartTime).valueOf();

                var _nowTime2 = new Date().valueOf(); //剩余时间


                item.leftTime = Math.floor((_startTime2 - _nowTime2) / 1000);
        ...
        changeState(data, updateState, updateTime, updateTimeState) {
          ...
          for (var i = 0; i < childs.length; i++) {
            ...
            var element = (_childs$i = childs[i]) == null ? void 0 : _childs$i.getComponent(_crd && RaceKickLineItem === void 0 ? (_reportPossibleCrUseOfRaceKickLineItem({
              error: Error()
        ...
        updateTeamInfo(datas) {
          if (datas && datas.zu_id && datas.list && datas.list.length > 0) {
            ...
            for (var index = 0; index < datas.list.length; index++) {
              var element = datas.list[index];
        ...
        onKickGameInfo(data) {
          var desLun = "当前进行第" + data.lun + "轮比赛\n";
          var desZhuo = "还有" + data.game + "桌比赛中";
          (_crd && UIManager === void 0 ? (_reportPossibleCrUseOfUIManager({
            error: Error()
          }), UIManager) : UIManager).Instace.showUI({
            path: (_crd && UIConfig === void 0 ? (_reportPossibleCrUseOfUIConfig({
        ...
        onLookKickGameInfo(id) {
          var baseInfo = GameMsg.OutGame.create({
            gameId: id
          });
          var baseBuffer = GameMsg.OutGame.encode(baseInfo).finish();
...
//# sourceMappingURL=066a7a1b40cc662472874840fab3e56b8c3a3fb1.js.map