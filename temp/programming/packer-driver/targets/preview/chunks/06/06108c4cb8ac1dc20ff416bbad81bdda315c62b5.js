    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function autoSortCards(cards) {
          ...
          var remain = () => cards.filter(c => !used.has(c)); // 🥇 固定顺序牌型优先识别


          addGroup(findWangzha(remain()));
          addGroup(findZ(remain(), 6));
          addGroup(findTonghuashun(remain()));
          addGroup(findZ(remain(), 5));
          addGroup(findZ(remain(), 4)); // 🧩 随机识别复杂牌型（放到最后，组合顺序打乱）
          ...
          addRandomGroup(findFeiji(remain()));
          addRandomGroup(findLiandui(remain()));
          addRandomGroup(findStraightByCard(remain()));
          addRandomGroup(findThreeWithTwo(remain())); // 🧩 随机组合推到 result 末尾

          shuffle(randomGroups).forEach(g => result.push(g)); // 🟩 最后识别对子 + 单牌

          addGroup(findPair(remain()));
          addGroup(findAllSingles(remain()));
          return result;
        }
...
//# sourceMappingURL=06108c4cb8ac1dc20ff416bbad81bdda315c62b5.js.map