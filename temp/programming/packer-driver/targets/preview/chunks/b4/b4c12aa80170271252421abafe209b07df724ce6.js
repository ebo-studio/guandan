      ...
      _export("CardLayer", CardLayer = (_dec = ccclass('CardLayer'), _dec2 = property(Prefab), _dec3 = property(SpriteAtlas), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(_crd && GameTimer === void 0 ? (_reportPossibleCrUseOfGameTimer({
          ...
          if (viewid == (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            (function () {
              ...
              if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                ...
                var outIndexes = new Set(_this2.selectCardIndex);

                for (var i = _this2.handCards.length - 1; i >= 0; i--) {
                  var card = _this2.handCards[i];

                  if (outIndexes.has(card.getIndex())) {
                    _this2.outCards[viewid].push(card);

                    _this2.handCards.splice(i, 1);
                  }
                }
              } else {
              ...
              for (var _k2 = 0; _k2 < _this2.outCards[viewid].length; _k2++) {
                ...
                var _card3 = _this2.outCards[viewid][_k2];

                _card3.setMask(false);

                _card3.node.setSiblingIndex(_k2);

                _card3.node.setScale(_this2.outScaleSelf, _this2.outScaleSelf, _this2.outScaleSelf);

                tween(_card3.node).to(.15, {
                  position: v3(posX, 100, 0)
                }).start();
              }
              ...
              if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                ...
                if (isAuto) {
                  _this2.groupedCards = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).smartSortCards(tempCards);
                } else {
                  _this2.groupedCards = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).removeOutCardsFromGrouped(that.groupedCards, tempCards);
                  console.log("出牌后 groupedCards: ", (_crd && utils === void 0 ? (_reportPossibleCrUseOfutils({
                    error: Error()
                  }), utils) : utils).deepCopy(_this2.groupedCards)); // 保险：核对分组总牌数，若不匹配则强制重跑智能理牌

                  var groupedCnt = _this2.groupedCards.reduce((s, g) => s + g.length, 0);

                  if (groupedCnt !== _this2.handCardsValue.length) {
                    console.warn('[dealOutCards] groupedCards count mismatch, fallback smartSort');
                    _this2.groupedCards = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                      error: Error()
                    ...
                    if (card.getValue() == valueSize && useIdxList.indexOf(card.getIndex()) == -1) {
                      ...
                      var moveFunc = function moveFunc(posX, posY, idx) {
                        tween(card.node).delay(0.2).to(0.1, {
                          ...
                          if (_i == sameSizeList.length - 1 && _j == 0) {
                            that.setCanTouch(true);
                          }
                        }).start();
                      };
        ...
        onScreenTouchMove(event) {
          ...
          for (var _i2 = this.handCards.length - 1; _i2 >= 0; _i2--) {
            ...
            if (_card6.node && _card6.node.getComponent(UITransform).getBoundingBoxToWorld().contains(startPos)) {
              _card6.setMask(true);
          ...
          for (var _i3 = this.handCards.length - 1; _i3 >= 0; _i3--) {
            var _card7 = this.handCards[_i3];

            if (_card7.node && _card7.node.getComponent(UITransform).getBoundingBoxToWorld().contains(movePos)) {
              _card7.setMask(true);
        ...
        onReviceCard(data, type) {
          if (type) {
            ...
            var _card8 = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).convertCardListS2C([data.card])[0];
            (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.giveCard = _card8;
            this.setHandCards(this.handCardsValue.concat(_card8), false, true, (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.oneCard);
            this.showBtnLayer(false);
          }
...
//# sourceMappingURL=b4c12aa80170271252421abafe209b07df724ce6.js.map