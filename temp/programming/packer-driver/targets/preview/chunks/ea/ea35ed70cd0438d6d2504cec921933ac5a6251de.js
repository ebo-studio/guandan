    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findStraightWithHongtao2_Safe(cards) {
          ...
          var isHeartLevelCard = card => getRank(card) === 2 && getColor(card) === HONGTAO; // 拆分红心级牌与其他牌
          ...
          seqList.push([10, 11, 12, 13, 1]); // AKQJ10 特殊顺子
          ...
          for (var [_color, list] of colorMap.entries()) {
            ...
            for (var seq of seqList) {
              ...
              for (var _r of seq) {
                var arr = rankMap.get(_r);

                if (arr && arr.length > 0) {
                  group.push(arr.shift()); // 用原牌
                } else if (usedHeart.length < 2 && heartCards.length > usedHeart.length) {
                  group.push(heartCards[usedHeart.length]); // 用红心级牌

                  usedHeart.push(heartCards[usedHeart.length]);
                } else {
                  // 无法填补
                  break;
                }
        ...
        function isFengRenPeiSorted(cards) {
          ...
          var heartCount = heartCards.length;
          if (heartCount > 2) return false; // 最多允许2张红心级牌

          var ranks = Array.from(new Set(cards.filter(c => !isHeartCard(c)).map(c => c % 16).filter(r => r >= 3 && r <= 13))).sort((a, b) => a - b);
          if (ranks.length + heartCount < 5) return false; // AKQJ10 特殊顺子

          var akqj10 = [10, 11, 12, 13, 1];
          var akqj10Count = akqj10.filter(r => ranks.includes(r)).length;
          if (akqj10Count + heartCount >= 5) return true;
          ...
          for (var start = 0; start < ranks.length; start++) {
            ...
            for (var i = start + 1; i < ranks.length; i++) {
              ...
              if (count >= 5 && needHearts <= heartCount && needHearts <= 2) {
                return true;
              }
            }
          }
...
//# sourceMappingURL=ea35ed70cd0438d6d2504cec921933ac5a6251de.js.map