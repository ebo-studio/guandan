      ...
      (function (_GameLogic) {
        ...
        function findFeiji(cards) {
          ...
          for (var _i7 = 0; _i7 < tripletRanks.length - 1; _i7++) {
            ...
            if (next === curr + 1) {
              return [[...triplets[_i7], ...triplets[_i7 + 1]]]; // ✅ 找到飞机
            }
          }
        ...
        function getCardTypeByFeiji(cards) {
          ...
          for (var _rank6 of sortedRanks) {
            var list = countMap.get(_rank6);
            if (!list) return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              error: Error()
            }), GameDefine) : GameDefine).KIND_CARDS_ERROR;

            if (list.length === 3) {
              tripletRanks.push(_rank6);
        ...
        function findBombsByCount(cards, count) {
          ...
          for (var [_rank9, group] of map.entries()) {
        ...
        function findFlushStraight(cards) {
          ...
          for (var [_color, colorCards] of colorMap.entries()) {
            ...
            for (var _card2 of colorCards) {
              var _rank10 = _card2 % 16;

              if (!rankMap.has(_rank10)) rankMap.set(_rank10, []);
              rankMap.get(_rank10).push(_card2);
            }
          ...
          while (i <= ranks.length - 5) {
            ...
            if (length == 5) {
              ...
              for (var _rank11 of seq) {
                var cardsOfRank = map.get(_rank11);
        ...
        function findStraight_II(cards, usedIndex) {
          ...
          for (var _i13 = 0; _i13 < cards.length; _i13++) {
            ...
            if (!rankMap.has(rank)) rankMap.set(rank, []);
            rankMap.get(rank).push(_i13); // 保存的是索引，方便精确控制使用
          }
          ...
          while (i <= sortedRanks.length - 5) {
            ...
            if (isConsecutive) {
              ...
              for (var _rank12 of seq) {
                var available = rankMap.get(_rank12).filter(idx => !usedIndex.has(idx));
        ...
        function countThreeWithTwoAndStraight(unused) {
          ...
          for (var _i14 = 0; _i14 < unused.length; _i14++) {
            var _rank13 = getCardSize(unused[_i14]);

            if (_rank13 >= 3 && _rank13 <= 14 && !usedIndex.has(_i14)) {
              if (!rankToIndexes.has(_rank13)) rankToIndexes.set(_rank13, []);
              rankToIndexes.get(_rank13).push(_i14);
            }
          }
          ...
          while (i <= ranks.length - 5) {
            ...
            if (length == 5) {
              ...
              for (var _rank14 of seq) {
                var cardsOfRank = map.get(_rank14);
                straight.push(cardsOfRank.pop());
        ...
        function newfindBombs(cards, levelRank) {
          ...
          for (var _rank15 of sortedRanks) {
            var group = countMap.get(_rank15);
...
//# sourceMappingURL=02e0140da75ded7b7e4d299f864d3b17609becdf.js.map