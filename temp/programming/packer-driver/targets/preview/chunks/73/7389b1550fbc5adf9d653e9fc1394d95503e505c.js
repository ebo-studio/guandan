    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          var restoreSet = new Set(); // ✅ 找出需要“恢复”的理牌组（只要有1张被选中，就整组恢复）

          for (var group of prevGrouped) {
            var isRestored = group.some(card => selectedSet.has(card));
          ...
          } // ✅ 这次理牌选中的部分 = selected - restoreSet（去除旧组中被恢复的）


          var thisSelected = selected.filter(c => !restoreSet.has(c));
          var thisSelectedSet = new Set(thisSelected); // ✅ 三带二识别

          var triple = [];
          var pair = [];
          var map = getCardCountMap(thisSelected);
          ...
          var feijiList = findFeiji(thisSelected.filter(c => !usedCards.has(c)));

          for (var _group2 of feijiList) {
          ...
          var straights = findStraightByCard(thisSelected.filter(c => !usedCards.has(c)));
          ...
          var lianduiList = findLiandui(thisSelected.filter(c => !usedCards.has(c)));
          ...
          var remainCards = cards.filter(c => !usedCards.has(c) && !restoreSet.has(c));
          var sortValue = sortCardsBySizeDown(remainCards, remainCards.length);
          var uniqueList = getUniqueCard(sortValue);
          var idx = 0;
          ...
          for (var j = 0; j < uniqueList.length; j++) {
            ...
            for (var i = idx; i < sortValue.length; i++) {
              ...
              if (getCardSize(card) === rank) {
                ...
                if (i === sortValue.length - 1) {
                  if (list1.length > 0) result.push(list1);
                }
              } else {
                idx = i;
                if (list1.length > 0) result.push(list1);
...
//# sourceMappingURL=7389b1550fbc5adf9d653e9fc1394d95503e505c.js.map