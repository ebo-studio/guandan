        ...
        function findFlushStraight(cards) {
          ...
          for (var group of colorMap.values()) {
            ...
            for (var _card2 of group) {
              ...
              if (!rankMap.has(_rank)) rankMap.set(_rank, []);
              rankMap.get(_rank).push(_card2);
            }
            ...
            for (var i = 0; i <= ranks.length - 5; i++) {
              ...
              if (isConsecutive) {
                ...
                for (var _rank2 of seq) {
                  straight.push(rankMap.get(_rank2).pop());
                  if (rankMap.get(_rank2).length === 0) rankMap.delete(_rank2);
                }
          ...
          switch (type) {
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var i = groupedCards.length - 1; i >= 0; i--) {
                ...
                if (group.length === 1) {
                  ...
                  if (compareCardSize(card, targetCards[0]) > targetMainSize) {
                    hintList.push([card]);
                  }
                }
              } // 没找到，再从对子中拆一张
              ...
              if (hintList.length === 0) {
                for (var _i8 = groupedCards.length - 1; _i8 >= 0; _i8--) {
                  ...
                  if (_group4.length === 2) {
                    ...
                    if (compareCardSize(_card3, targetCards[0]) > targetMainSize) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i9 = groupedCards.length - 1; _i9 >= 0; _i9--) {
                ...
                if (_group5.length === 2) {
                  if (GameLogic.getCardSize(_group5[0]) > targetMainSize) {
                    hintList.push([..._group5]); // 复制一个对子
                  }
                }
              }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_3:
              {
                ...
                for (var _i10 = groupedCards.length - 1; _i10 >= 0; _i10--) {
                  ...
                  if (_group6.length === 3) {
                    var size = GameLogic.getCardSize(_group6[0]);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i12 = groupedCards.length - 1; _i12 >= 0; _i12--) {
                ...
                if (_group7.length >= 4) {
                  var groupRank = GameLogic.getCardSize(_group7[0]);
              ...
              var kingGroup = groupedCards.find(g => g.length === 2 && isKing(g[0]) && isKing(g[1]));

              if (kingGroup) {
                hintList.push([...kingGroup]);
              }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_1:
              {
                ...
                for (var _card4 of singles) {
                  var _size2 = GameLogic.getCardSize(_card4);

                  if (!rankMap.has(_size2)) rankMap.set(_size2, []);
                  rankMap.get(_size2).push(_card4);
                }

                var allRanks = Array.from(rankMap.keys()).sort((a, b) => a - b);

                var _loop2 = function _loop2(_i13) {
                  var seq = allRanks.slice(_i13, _i13 + targetLength);
                  ...
                  if (isContinuous && seq[0] > targetStart) {
                    ...
                    for (var r of seq) {
                      _group8.push(rankMap.get(r).pop());
                    }
                ...
                for (var _i13 = 0; _i13 <= allRanks.length - targetLength; _i13++) {
                  var _ret = _loop2(_i13);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_COLOR:
              {
                ...
                for (var _card5 of _singles) {
                  var _size3 = GameLogic.getCardSize(_card5);

                  if (!_rankMap.has(_size3)) _rankMap.set(_size3, []);

                  _rankMap.get(_size3).push(_card5);
                }

                var _allRanks = Array.from(_rankMap.keys()).sort((a, b) => a - b);

                var _loop3 = function _loop3(_i14) {
                  var seq = _allRanks.slice(_i14, _i14 + _targetLength);
                  ...
                  if (isContinuous && seq[0] > _targetStart) {
                    ...
                    for (var r of seq) {
                      _group9.push(_rankMap.get(r).pop());
                    }
                ...
                for (var _i14 = 0; _i14 <= _allRanks.length - _targetLength; _i14++) {
                  var _ret2 = _loop3(_i14);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_3:
              {
                ...
                for (var _i15 = 0; _i15 <= tripletGroups.length - _targetLength2 / 3; _i15++) {
                  var _group10 = [];
                  var ok = true;

                  for (var _j2 = 0; _j2 < _targetLength2 / 3; _j2++) {
                    var cur = tripletGroups[_i15 + _j2];

                    if (_j2 > 0 && cur.rank !== tripletGroups[_i15 + _j2 - 1].rank + 1) {
                      ok = false;
                      break;
                    }

                    _group10.push(...cur.cards);
                  }

                  if (ok && tripletGroups[_i15].rank > targetMin) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_2:
              {
                ...
                for (var _i16 = 0; _i16 <= pairGroups.length - _targetLength3 / 2; _i16++) {
                  var _group11 = [];
                  var _ok = true;

                  for (var _j3 = 0; _j3 < _targetLength3 / 2; _j3++) {
                    var _cur = pairGroups[_i16 + _j3];

                    if (_j3 > 0 && _cur.rank !== pairGroups[_i16 + _j3 - 1].rank + 1) {
                      _ok = false;
                      break;
                    }

                    _group11.push(..._cur.cards);
                  }

                  if (_ok && pairGroups[_i16].rank > _targetMin) {
...
//# sourceMappingURL=8ed3b76cdbcbe80071aee93c8a29cca63304cbbd.js.map