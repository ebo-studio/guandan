    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findStraightByCard(cards) {
          ...
          var HONGTAO = 1;
          var results = [];
          var usedIndexes = new Set(); // 构建 rank => [index[]]
          ...
          for (var _i6 = 0; _i6 <= legalRanks.length - 5; _i6++) {
            for (var len = 5; len <= legalRanks.length - _i6; len++) {
              seqList.push(legalRanks.slice(_i6, _i6 + len));
            }
          }
          ...
          var _loop = function _loop(seq) {
            var group = [];
            var usedThisGroup = new Set();
            var valid = true;

            for (var _r of seq) {
              if (_r === 2) {
                var idx = cards.findIndex((c, i) => getCardRank(c) === 2 && getCardColor(c) === HONGTAO && !usedIndexes.has(i) && !usedThisGroup.has(i));
                ...
                var available = idxList.find(i => !usedIndexes.has(i) && !usedThisGroup.has(i));
        ...
        function getCardType(cards) {
          ...
          if (cards.length === 5) {
            var map = getCardCountMap(cards);
            var heartCards = cards.filter(c => isHeartCard(c));
            var nonHeart = cards.filter(c => !isHeartCard(c)); // 查找三张相同点数的非红心牌

            var rankMap = new Map();

            for (var card of nonHeart) {
              var rank = card % 16;
            ...
            for (var [_rank4, list] of rankMap.entries()) {
              if (list.length === 3) {
                triple = list;
                break;
              }
            ...
            if (triple.length === 3) {
              var used = new Set(triple);
              var remain = cards.filter(c => !used.has(c));
              var pairMap = new Map();
              var redHeartList = [];

              for (var _card of remain) {
                if (isHeartCard(_card)) {
                  redHeartList.push(_card);
                  continue;
                }

                var _rank5 = _card % 16;

                if (!pairMap.has(_rank5)) pairMap.set(_rank5, []);
                pairMap.get(_rank5).push(_card);
              } // 尝试将红心级牌补到已有点数中


              for (var red of redHeartList) {
                // 优先补到数量为1的，组成对子
                var targetRank = null;

                for (var [_rank6, _list2] of pairMap.entries()) {
                  if (_list2.length === 1) {
                    targetRank = _rank6;
                    break;
                  }
              ...
              for (var _list3 of pairMap.values()) {
                if (_list3.length >= 2) {
                  return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
                    error: Error()
          ...
          if (cards.length == 5) {
            ...
            for (var _card2 of cards) {
              var _rank7 = getCardSize(_card2);

              if (_rank7 < 3 || _rank7 > 14) return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
                error: Error()
              }), GameDefine) : GameDefine).KIND_CARDS_ERROR; // 不能包含2和王
        ...
        function getTripleWithPairPreferBig(cards) {
          ...
          for (var _rank8 of ranksSorted) {
            var list = rankMap.get(_rank8);
        ...
        function findFlushStraight(cards) {
          ...
          for (var [_color, colorCards] of colorMap.entries()) {
            ...
            for (var _card4 of colorCards) {
              var _rank11 = _card4 % 16;

              if (!rankMap.has(_rank11)) rankMap.set(_rank11, []);
              rankMap.get(_rank11).push(_card4);
            }
        ...
        function getHintCards(targetCards, groupedCards) {
          ...
          switch (type) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _card9 of groupedCards.flat()) {
                var color = GameLogic.getCardColor(_card9);
                if (!colorGroups.has(color)) colorGroups.set(color, []);
                colorGroups.get(color).push(_card9);
              }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_1:
              {
                ...
                for (var _card10 of singles) {
                  var _size2 = GameLogic.getCardSize(_card10);

                  if (!rankMap.has(_size2)) rankMap.set(_size2, []);
                  rankMap.get(_size2).push(_card10);
                }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_COLOR:
              {
                ...
                for (var _card11 of _singles) {
                  var _size3 = GameLogic.getCardSize(_card11);

                  if (!_rankMap.has(_size3)) _rankMap.set(_size3, []);

                  _rankMap.get(_size3).push(_card11);
                }
            ...
            if (hintList.length === 0) {
              for (var _i39 = groupedCards.length - 1; _i39 >= 0; _i39--) {
                ...
                if (_group32.length === 3) {
                  var _card12 = _group32[0];

                  if (isSingleCardStronger(_card12, targetCards[0], (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
                  }), GlobalData) : GlobalData).cardInfo.levelCard)) {
                    hintList.push([_card12]);
                  }
                }
              }
            }
...
//# sourceMappingURL=c9cbd067bfdea6eb7531a868b833ffe87b9e4157.js.map