  ...
  return {
    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function sortByLengthAsc(groups) {
          return groups.slice().sort((a, b) => a.length - b.length);
        }

        _GameLogic.sortByLengthAsc = sortByLengthAsc;
        ...
        function findFeiji(cards) {
          ...
          for (var card of normalCards) {
            var rank = getCardSize(card);
            if (rank < 3 || rank > 14) continue; // 排除 2 和王

            if (!map.has(rank)) map.set(rank, []);
            map.get(rank).push(card);
          }

          var rankToTriplet = new Map();
          var redLeft = redCount; // 尝试每个 rank 是否能组成三张（原生或拼红心）

          for (var [_rank3, list] of map.entries()) {
            if (list.length === 3) {
              rankToTriplet.set(_rank3, [...list]);
            } else if (list.length === 2 && redLeft >= 1) {
              rankToTriplet.set(_rank3, [...list, redCards[redCount - redLeft]]);
              redLeft--;
            }
          }

          if (rankToTriplet.size < 2) return [];
          var sortedRanks = Array.from(rankToTriplet.keys()).sort((a, b) => a - b);

          for (var i = 0; i < sortedRanks.length - 1; i++) {
            var group = [rankToTriplet.get(sortedRanks[i])];
            ...
            for (var j = i + 1; j < sortedRanks.length; j++) {
              ...
              if (curr === prev + 1) {
                group.push(rankToTriplet.get(curr));

                if (group.length >= 2) {
                  return [group.flat()]; // ✅ 找到第一组飞机
                }
              } else {
                break;
              }
        ...
        function manualSortCards(selected, prevSelected, prevGrouped, allCards) {
          ...
          for (var [_rank4, list] of countMap.entries()) {
            if (list.length >= 6) {
              result.unshift(list);
          ...
          for (var [_rank5, _list] of rankMap.entries()) {
          ...
          for (var _i8 = 0; _i8 <= sortedPairs.length - 3; _i8++) {
            var _r2 = sortedPairs[_i8][0] % 16;

            var _r3 = sortedPairs[_i8 + 1][0] % 16;
        ...
        function getCardTypeByFeiji(cards) {
          ...
          for (var _rank7 of sortedRanks) {
            var list = countMap.get(_rank7);

            if (list.length === 3) {
              tripletRanks.push(_rank7);
            } else if (list.length === 2 && redLeft >= 1) {
              tripletRanks.push(_rank7);
              redLeft -= 1;
        ...
        function getCardTypeByLiandui(cards) {
          ...
          for (var [_rank8, list] of map.entries()) {
            if (list.length === 2) {
              pairRanks.push(_rank8);
        ...
        function getOneCardSizeList(value) {
          ...
          for (var _i9 = 0; _i9 < pairs.length; _i9++) {
            if (temp.length === 0) {
              ...
              var current = getCardSize(pairs[_i9][0]);
          ...
          var findStraightGroups = cards => {
            ...
            for (var _i10 = 0; _i10 < unique.length; _i10++) {
              var cur = unique[_i10];
        ...
        function getGroupPower(cards) {
          ...
          if (size >= 4 && size % 2 === 0) {
            ...
            for (var _i11 = 1; _i11 < pairs.length; _i11++) {
              if (pairs[_i11] !== pairs[_i11 - 1] + 1 || pairs[_i11] >= 15) return 0;
            }
        ...
        function getTripleWithPairPreferBig(cards) {
          ...
          var ranksSorted = [...rankMap.keys()].sort((a, b) => b - a);

          for (var _rank9 of ranksSorted) {
            var list = rankMap.get(_rank9);
            var need = 3 - list.length;

            if (list.length >= 1 && heartCards.length >= need) {
              var _ret3 = function () {
                // 成功构成三条
                var triple = [...list, ...heartCards.slice(0, need)];
                var used = new Set(triple); // 从剩下的牌中找对子（也可以拼红心）

                var remain = cards.filter(c => !used.has(c));
                var redLeft = remain.filter(isHeartCard);
                var pairMap = new Map();

                for (var c of remain) {
                  if (isHeartCard(c)) continue;
                  var r = getCardRank_I(c);
                  if (!pairMap.has(r)) pairMap.set(r, []);
                  pairMap.get(r).push(c);
                } // 红心补对子


                for (var red of redLeft) {
                  for (var [_r5, l] of pairMap.entries()) {
                    if (l.length === 1) {
                      l.push(red);
                      break;
                    }
        ...
        function getCardCountMap_I(cards) {
          ...
          if (heardCard != 0) {
            for (var [_rank10, list] of map.entries()) {
              map.get(_rank10).push(heardCard);
            }
          }
        ...
        function findBombsByCount(cards, count) {
          ...
          for (var [_rank11, group] of map.entries()) {
        ...
        function findFlushStraight(cards) {
          ...
          for (var [_color, colorCards] of colorMap.entries()) {
            ...
            for (var _card2 of colorCards) {
              var _rank12 = _card2 % 16;

              if (!rankMap.has(_rank12)) rankMap.set(_rank12, []);
              rankMap.get(_rank12).push(_card2);
            }
            ...
            for (var start = 3; start <= 10; start++) {
              ...
              if (missing === 0) {
                ...
                if (frp !== undefined) {
                  ...
                  // 恢复
                  for (var [_r6, _c4] of tempPopped) {
                    if (!rankMap.has(_r6)) rankMap.set(_r6, []);
                    rankMap.get(_r6).push(_c4);
                  }
                }
              } else {
                // 恢复
                for (var [_r7, _c5] of tempPopped) {
                  if (!rankMap.has(_r7)) rankMap.set(_r7, []);
                  rankMap.get(_r7).push(_c5);
                }
              }
            }
          }
        ...
        function findStraight_II(cards, usedIndex) {
          ...
          for (var _i12 = 0; _i12 < cards.length; _i12++) {
            if (usedIndex.has(_i12)) continue;
            var card = cards[_i12];
            var rank = card % 16;
            if (rank < 3 || rank > 13) continue; // 只允许 3~K 进入顺子

            if (!rankMap.has(rank)) rankMap.set(rank, []);
            rankMap.get(rank).push(_i12); // 保存的是索引，方便精确控制使用
          }
          ...
          while (i <= sortedRanks.length - 5) {
            ...
            if (isConsecutive) {
              ...
              for (var _rank14 of seq) {
                var available = rankMap.get(_rank14).filter(idx => !usedIndex.has(idx));
        ...
        function countThreeWithTwoAndStraight(unused) {
          ...
          for (var _i13 = 0; _i13 < unused.length; _i13++) {
            var _rank15 = getCardSize(unused[_i13]);

            if (_rank15 >= 3 && _rank15 <= 14 && !usedIndex.has(_i13)) {
              if (!rankToIndexes.has(_rank15)) rankToIndexes.set(_rank15, []);
              rankToIndexes.get(_rank15).push(_i13);
            }
          }
          ...
          for (var _i14 = 0; _i14 < sorted.length; _i14++) {
            var r = sorted[_i14];
        ...
        function findStraight(cards) {
          ...
          while (i <= ranks.length - 5) {
            ...
            if (length == 5) {
              ...
              for (var _rank16 of seq) {
                var cardsOfRank = map.get(_rank16);
                straight.push(cardsOfRank.pop());
        ...
        function getHintCards(targetCards, groupedCards) {
          ...
          switch (type) {
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i15 = groupedCards.length - 1; _i15 >= 0; _i15--) {
                var _group9 = groupedCards[_i15];
              ...
              for (var _i16 = groupedCards.length - 1; _i16 >= 0; _i16--) {
                var _group10 = groupedCards[_i16];
              ...
              for (var _i17 = groupedCards.length - 1; _i17 >= 0; _i17--) {
                var _group11 = groupedCards[_i17];
              ...
              for (var _i18 = groupedCards.length - 1; _i18 >= 0; _i18--) {
                var _group12 = groupedCards[_i18];

                if (_group12.length == 5) {
                  var _bombs = GameLogic.newfindBombs(_group12);
              ...
              for (var _i19 = groupedCards.length - 1; _i19 >= 0; _i19--) {
                var _group13 = groupedCards[_i19];

                if (_group13.length == 5) {
                  var _bombs2 = GameLogic.findFlushStraight(_group13);
              ...
              for (var _i20 = groupedCards.length - 1; _i20 >= 0; _i20--) {
                var _group14 = groupedCards[_i20];

                if (_group14.length == 6) {
                  var _bombs3 = GameLogic.newfindBombs(_group14);
              ...
              for (var _i21 = groupedCards.length - 1; _i21 >= 0; _i21--) {
                var _group15 = groupedCards[_i21];

                if (_group15.length == 4) {
                  var _bombs4 = GameLogic.findRocket(_group15);
              ...
              for (var _i22 = groupedCards.length - 1; _i22 >= 0; _i22--) {
                var _group16 = groupedCards[_i22]; // if (group.length == 4) {

                var _bombs5 = newfindBombs(_group16);
              ...
              for (var _i23 = result.length - 1; _i23 >= 0; _i23--) {
                var _group17 = result[_i23]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);

                if (_group17.length != 0) {
                  hintList.push(_group17);
                } // }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i24 = groupedCards.length - 1; _i24 >= 0; _i24--) {
                var _group18 = groupedCards[_i24];

                if (_group18.length === 2) {
                  var _card5 = _group18[0];
              ...
              for (var _i25 = groupedCards.length - 1; _i25 >= 0; _i25--) {
                var _group19 = groupedCards[_i25];

                if (_group19.length === 3) {
                  var _card6 = _group19[0];
              ...
              for (var _i26 = groupedCards.length - 1; _i26 >= 0; _i26--) {
                var _group20 = groupedCards[_i26];

                if (_group20.length == 4) {
                  var _bombs6 = GameLogic.newfindBombs(_group20);
              ...
              for (var _i27 = groupedCards.length - 1; _i27 >= 0; _i27--) {
                var _group21 = groupedCards[_i27];

                if (_group21.length == 5) {
                  var _bombs7 = GameLogic.newfindBombs(_group21);
              ...
              for (var _i28 = groupedCards.length - 1; _i28 >= 0; _i28--) {
                var _group22 = groupedCards[_i28];

                if (_group22.length == 5) {
                  var _bombs8 = GameLogic.findFlushStraight(_group22);
              ...
              for (var _i29 = groupedCards.length - 1; _i29 >= 0; _i29--) {
                var _group23 = groupedCards[_i29];

                if (_group23.length == 6) {
                  var _bombs9 = GameLogic.newfindBombs(_group23);
              ...
              for (var _i30 = groupedCards.length - 1; _i30 >= 0; _i30--) {
                var _group24 = groupedCards[_i30];

                if (_group24.length == 4) {
                  var _bombs10 = GameLogic.findRocket(_group24);

                  if (_bombs10.length != 0) {
              ...
              for (var _i31 = result1.length - 1; _i31 >= 0; _i31--) {
                var _group25 = result1[_i31]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);

                if (_group25.length != 0) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_3:
              {
                ...
                for (var _i32 = groupedCards.length - 1; _i32 >= 0; _i32--) {
                  var _group26 = groupedCards[_i32];

                  if (_group26.length === 3) {
                    var size = GameLogic.getCardSize(_group26[0]);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              {
                // 三带二：找出三张+一对，三张部分要大
                for (var _i33 = groupedCards.length - 1; _i33 >= 0; _i33--) {
                  var group3 = groupedCards[_i33];
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i34 = groupedCards.length - 1; _i34 >= 0; _i34--) {
                var _group27 = groupedCards[_i34];

                if (_group27.length >= 4) {
                  // ✅ 点数必须全部一致，才是炸弹
                  var isBomb = isSamePointWithFrp(_group27, (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
                  }), GlobalData) : GlobalData).cardInfo.levelCard);
                  if (!isBomb) continue;
                  var groupRank = GameLogic.getCardSize(_group27[0]);

                  if (_group27.length > targetCards.length || _group27.length === targetCards.length && groupRank > targetMainSize) {
              ...
              for (var cards of colorGroups.values()) {
                ...
                for (var _i35 = 0; _i35 <= sizes.length - 5; _i35++) {
                  var ok = true;

                  for (var _j2 = 1; _j2 < 5; _j2++) {
                    if (sizes[_i35 + _j2] !== sizes[_i35 + _j2 - 1] + 1) {
                      ok = false;
                      break;
                    }
                  }
                  ...
                  if (ok) {
                    ...
                    for (var _j3 = 0; _j3 < 5; _j3++) {
                      var _s = sizes[_i35 + _j3];
                      var list = sizeMap.get(_s);

                      _group28.push(list.pop()); // 拿一张即可
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i36 = groupedCards.length - 1; _i36 >= 0; _i36--) {
                var _group29 = groupedCards[_i36];

                if (_group29.length >= 6) {
                  if (!isSamePointWithFrp(_group29, (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
                  }), GlobalData) : GlobalData).cardInfo.levelCard)) continue;

                  var _groupRank = GameLogic.getCardSize(_group29[0]);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_1:
              {
                ...
                var _loop4 = function _loop4(_i37) {
                  var seq = allRanks.slice(_i37, _i37 + targetLength);
                  ...
                  if (isContinuous && seq[0] > targetStart) {
                    ...
                    for (var r of seq) {
                      _group30.push(rankMap.get(r).pop());
                    }
                ...
                for (var _i37 = 0; _i37 <= allRanks.length - targetLength; _i37++) {
                  var _ret4 = _loop4(_i37);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_COLOR:
              {
                ...
                var _loop5 = function _loop5(_i38) {
                  var seq = _allRanks.slice(_i38, _i38 + _targetLength);
                  ...
                  if (isContinuous && seq[0] > _targetStart) {
                    ...
                    for (var r of seq) {
                      _group31.push(_rankMap.get(r).pop());
                    }
                ...
                for (var _i38 = 0; _i38 <= _allRanks.length - _targetLength; _i38++) {
                  var _ret5 = _loop5(_i38);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_3:
              {
                ...
                for (var _i39 = 0; _i39 <= tripletGroups.length - _targetLength2 / 3; _i39++) {
                  var _group32 = [];
                  var _ok = true;

                  for (var _j4 = 0; _j4 < _targetLength2 / 3; _j4++) {
                    var cur = tripletGroups[_i39 + _j4];

                    if (_j4 > 0 && cur.rank !== tripletGroups[_i39 + _j4 - 1].rank + 1) {
                      _ok = false;
                      break;
                    }

                    _group32.push(...cur.cards);
                  }

                  if (_ok && tripletGroups[_i39].rank > targetMin) {
                    hintList.push(_group32);
                    break;
                  }
                }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_2:
              {
                ...
                for (var _i40 = 0; _i40 <= pairGroups.length - _targetLength3 / 2; _i40++) {
                  var _group33 = [];
                  var _ok2 = true;

                  for (var _j5 = 0; _j5 < _targetLength3 / 2; _j5++) {
                    var _cur = pairGroups[_i40 + _j5];

                    if (_j5 > 0 && _cur.rank !== pairGroups[_i40 + _j5 - 1].rank + 1) {
                      _ok2 = false;
                      break;
                    }

                    _group33.push(..._cur.cards);
                  }

                  if (_ok2 && pairGroups[_i40].rank > _targetMin) {
          ...
          if (targetType === (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            if (hintList.length === 0) {
              for (var _i41 = groupedCards.length - 1; _i41 >= 0; _i41--) {
                var _group34 = groupedCards[_i41];

                if (_group34.length === 2) {
                  var card = _group34[0];
            ...
            if (hintList.length === 0) {
              for (var _i42 = groupedCards.length - 1; _i42 >= 0; _i42--) {
                var _group35 = groupedCards[_i42];

                if (_group35.length === 3) {
                  var _card10 = _group35[0];
            ...
            if (hintList.length === 0) {
              for (var _i43 = groupedCards.length - 1; _i43 >= 0; _i43--) {
                var _group36 = groupedCards[_i43];

                if (_group36.length >= 4) {
                  var bombs = GameLogic.newfindBombs(_group36);
            ...
            for (var _group37 of groupedCards) {
              if (_group37.length === 2 && GameLogic.getCardSize(_group37[0]) > targetRank) {
                hintList.push([..._group37]);
              }
            }
          } // ------------------ 三不带 ------------------
          else if (targetType === (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            for (var _group38 of groupedCards) {
              if (_group38.length === 3 && GameLogic.getCardSize(_group38[0]) > targetRank) {
                hintList.push([..._group38]);
              }
            }
          } // ------------------ 三带二 ------------------
          else if (targetType === (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            var _loop6 = function _loop6(_group39) {
              if (_group39.length === 3 && GameLogic.getCardSize(_group39[0]) > targetRank) {
                // 找一个对子作为带牌
                var pair = groupedCards.find(g => g.length === 2 && g !== _group39);

                if (pair) {
                  hintList.push([..._group39, ...pair]);
                }
              }
            };

            for (var _group39 of groupedCards) {
              _loop6(_group39);
            }
          } // // ------------------ 顺子 ------------------
        ...
        function findBombsWithHeartSupport(groups) {
          var result = []; // 拆分红心级牌与其他组

          var redCards = groups.flat().filter(c => isHeartCard(c));
          var normalGroups = groups.filter(g => !g.some(c => isHeartCard(c)) && getCardRank_I(g[0]) !== 2);

          var _loop7 = function _loop7(group) {
          ...
          for (var group of normalGroups) {
            var _ret6 = _loop7(group);

            if (_ret6 === "continue") continue;
          } // 检查红心组是否本身构成炸弹（如红心 + 4张6）


          var heartGroup = groups.find(g => g.some(c => isHeartCard(c)));

          if (heartGroup) {
            var rankGroups = new Map();

            for (var card of heartGroup) {
              var r = getCardRank_I(card);
              if (!rankGroups.has(r)) rankGroups.set(r, []);
              rankGroups.get(r).push(card);
            }

            for (var list of rankGroups.values()) {
              if (list.length >= 4) {
                result.push([...list]);
              }
            }
        ...
        function newfindBombs(cards, levelRank) {
          ...
          for (var _rank17 of sortedRanks) {
            var group = countMap.get(_rank17);
...
//# sourceMappingURL=1a162b9e4bafe4c5e71f3d31870268c9dec2765d.js.map