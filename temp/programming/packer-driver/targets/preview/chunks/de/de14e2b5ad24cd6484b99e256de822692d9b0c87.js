    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          console.log("🔍 输入 cards:", cards); // console.log("🔍 当前 usedIndex:", Array.from(usedIndex));

          var map = new Map(); // 构建点数 -> [牌列表]
          ...
          var result = []; // 找所有 >=3 的点数，按点数升序（越小的在前）

          var tripleRanks = Array.from(map.keys()).filter(rank => map.get(rank).length >= 3).sort((a, b) => a - b);

          var _loop = function _loop(_rank2) {
            var three = map.get(_rank2).slice(0, 3); // 找其他任意一个不同点数的对子（最小的）

            var pairRank = Array.from(map.keys()).filter(r => r !== _rank2 && map.get(r).length >= 2).sort((a, b) => a - b)[0];

            if (pairRank !== undefined) {
              var pair = map.get(pairRank).slice(0, 2);
              result.push([...three, ...pair]);
              return "break"; // 只取一个最小三带二
            }
          };

          for (var _rank2 of tripleRanks) {
            var _ret = _loop(_rank2);
...
//# sourceMappingURL=de14e2b5ad24cd6484b99e256de822692d9b0c87.js.map