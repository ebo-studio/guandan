    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          var remain = []; // 把已理牌的牌从原数组中剔除

          for (var card of cards) {
            var idx = selectedCopy.indexOf(card);
          ...
          for (var _card of remain) {
            if (temp.length === 0 || GameLogic.getCardSize(temp[0]) === GameLogic.getCardSize(_card)) {
              temp.push(_card);
            } else {
              groupList.push(temp);
              temp = [_card];
...
//# sourceMappingURL=757ef28ae4a8b37fc28f10f5db9d1af4c2a0f77e.js.map