    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex, maxCount) {
          ...
          // return []; // 找不到合法三带二
          var result = [];
          var countMap = new Map(); // 分组：按点数归类（排除特殊点数）

          for (var i = 0; i < cards.length; i++) {
            var card = cards[i];
            var rank = card % 16;
            var color = Math.floor(card / 16); // ❌ 排除非红桃的级牌2、大王、小王、参谋
          ...
          }).sort((a, b) => a[0] - b[0]); // 所有对子（按点数升序）

          var pairs = Array.from(countMap.entries()).filter(_ref2 => {
            var [_, list] = _ref2;
            return list.length >= 2;
          }).sort((a, b) => a[0] - b[0]);
          var usedCardIndex = new Set(usedIndex);

          for (var [tripleRank, tripleList] of triples) {
            var triple = tripleList.filter(x => !usedCardIndex.has(x.index));
            if (triple.length < 3) continue;

            for (var [pairRank, pairList] of pairs) {
              if (pairRank === tripleRank) continue;
              var pair = pairList.filter(x => !usedCardIndex.has(x.index));
              if (pair.length < 2) continue;
              var group = [triple[0].card, triple[1].card, triple[2].card, pair[0].card, pair[1].card]; // 标记为已使用
            ...
            if (result.length >= maxCount) break; // 达到上限了
          }
        ...
        function smartSortCards(cards) {
          ...
          if (findStraight(getUnused()).length == 1) {
            ...
            if (findStraight(getUnused()).length == 0) {
              findThreeWithTwo(getUnused(), usedIndex, 2).forEach(g => pushAndMark(g, false)); // 三带二（最小三张）
            } else {
              findStraight(getUnused()).forEach(g => pushAndMark(g, false)); // 顺子
            }
...
//# sourceMappingURL=9451b4e6e9fa8c31e2412f9e0b1356d525c7e8ce.js.map