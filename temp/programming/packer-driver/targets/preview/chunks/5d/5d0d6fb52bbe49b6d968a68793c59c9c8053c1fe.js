    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function autoSortCards(cards) {
          var result = [];
          var used = new Set(); // 🔥 先处理王炸、N炸、同花顺等大牌（按固定顺序）

          addGroup(findWangzha(cards.filter(c => !used.has(c))));
          addGroup(findZ(cards.filter(c => !used.has(c)), 6));
          addGroup(findTonghuashun(cards.filter(c => !used.has(c)))); // ...
          // 🔁 剩下牌型随机（飞机、连对、顺子、三带二）

          var shuffledTypes = shuffleArray([findFeiji, findLiandui, findStraightByCard, findThreeWithTwo]);

          for (var findFn of shuffledTypes) {
            addGroup(findFn(cards.filter(c => !used.has(c))));
          } // 🔚 剩余按点数分组


          addGroup(GameLogic.getSameCardSizeList(cards.filter(c => !used.has(c)), false, [], []));
          return result;
...
//# sourceMappingURL=5d0d6fb52bbe49b6d968a68793c59c9c8053c1fe.js.map