    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function smartSortCards(cards) {
          ...
          var levelRank = (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            error: Error()
          }), GlobalData) : GlobalData).cardInfo.levelCard; // 👈 替换为你的实际“级牌”点数（如 A 为 14，3 为 3）
          ...
          findFlushStraight(getUnused()).forEach(g => pushAndMark(g));
          findBombsByCount(getUnused(), 5).forEach(g => pushAndMark(g));
          findBombsByCount(getUnused(), 4).forEach(g => pushAndMark(g)); // 单张大王（15）

          getUnused().filter(c => c % 16 === 15).forEach(c => pushAndMark([c])); // 单张小王（14）

          getUnused().filter(c => c % 16 === 14).forEach(c => pushAndMark([c])); // 级牌（例如点数 5）

          getUnused().filter(c => c % 16 === levelRank).forEach(c => pushAndMark([c])); // 杂牌型识别：飞机、连对、顺子、三带二、对子、单张

          var restGroups = [];
          restGroups.push(...findPlane(getUnused()));
          restGroups.push(...findChainPairs(getUnused()));
          restGroups.push(...findStraight(getUnused()));
          restGroups.push(...findThreeWithTwo(getUnused()));
          restGroups.push(...findPair(getUnused()));
          getUnused().forEach(c => restGroups.push([c])); // 剩余单张
          // 按点数降序排序组（用每组最大点数）

          restGroups.sort((a, b) => {
            var maxA = Math.max(...a.map(c => c % 16));
            var maxB = Math.max(...b.map(c => c % 16));
            return maxB - maxA;
          }); // 标记 restPart 并加入 randomPart
          ...
          restGroups.forEach(group => {
            ...
            if (realGroup.length > 0) {
              randomPart.push(realGroup);
            }
          });
          return mainResult.concat(randomPart);
        }
...
//# sourceMappingURL=f4bb9e90634bd901495fac49261c0067deec8990.js.map