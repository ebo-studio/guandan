    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findStraightByCard(cards) {
          ...
          var rankMap = new Map(map); // 深拷贝用于消耗

          var results = []; // 所有合法点数范围：3 ~ A(14)

          var legalRanks = Array.from({
            length: 12
          }, (_, i) => i + 3); // [3, 4, ..., 14]

          for (var start = 0; start <= legalRanks.length - 5; start++) {
            for (var len = 5; start + len <= legalRanks.length; len++) {
              var seq = legalRanks.slice(start, start + len); // 取出一段连续 rank
              // 检查每个 rank 是否存在一张牌
...
//# sourceMappingURL=78cab0af19052feb51d72dd0036f3092f6599027.js.map