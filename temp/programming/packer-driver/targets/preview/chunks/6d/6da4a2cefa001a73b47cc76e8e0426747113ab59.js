    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          var selectedSet = new Set(selectedHistory); // 2. 查找三带二的组合
          ...
          var usedRanks = new Set();

          if (triple.length === 3 && pair.length === 2) {
            var group = [...triple, ...pair];
            moveToBack.push(group); // 整体推到末尾

            group.forEach(c => usedRanks.add(getCardSize(c))); // 用点数标记
          } // 4. 分组剩余的手牌
          ...
          for (var j = 0; j < uniqueList.length; j++) {
            ...
            for (var i = idx; i < sortValue.length; i++) {
              ...
              if (usedRanks.has(getCardSize(card))) continue; // 过滤掉组合过的牌
...
//# sourceMappingURL=6da4a2cefa001a73b47cc76e8e0426747113ab59.js.map