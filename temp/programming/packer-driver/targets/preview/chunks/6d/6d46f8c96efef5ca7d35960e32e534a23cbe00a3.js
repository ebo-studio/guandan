    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function smartSortCards(cards) {
          ...
          var scatteredPart = []; // 单张/对子/三张（降序）

          var randomPart = []; // 飞机、顺子、连对、三带二（升序）
          ...
          findBombsByCount(getUnused(), 4).forEach(g => pushAndMark(g, mainResult)); // 单张识别：大王、小王、级牌
          ...
          var miscGroups = [...findPlane(getUnused()), ...findChainPairs(getUnused()), ...findStraight(getUnused()), ...findThreeWithTwo(getUnused())];
          miscGroups.sort((a, b) => minRank(a) - minRank(b)).forEach(g => pushAndMark(g, randomPart)); // 剩余牌（三张/对子/单张） → 按最大点数降序

          var countMap = getCardCountMap(getUnused());
          var remainGroups = [];

          for (var [rank, group] of countMap.entries()) {
            var copy = [...group];

            while (copy.length >= 3) {
              remainGroups.push(copy.splice(0, 3));
            }

            while (copy.length >= 2) {
              remainGroups.push(copy.splice(0, 2));
            }

            while (copy.length >= 1) {
              remainGroups.push(copy.splice(0, 1));
            }
          }

          remainGroups.sort((a, b) => maxRank(b) - maxRank(a)).forEach(g => pushAndMark(g, scatteredPart));
...
//# sourceMappingURL=6d46f8c96efef5ca7d35960e32e534a23cbe00a3.js.map