    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function smartSortCards(cards) {
          ...
          findBombsByCount(getUnused(), 4).forEach(g => pushAndMark(g, mainResult)); // 2. 单张特例：大王、小王、级牌
          ...
          miscGroups.forEach(g => pushAndMark(g, randomPart)); // 4. 散牌（单张、对子、三张）

          var countMap = getCardCountMap(getUnused());
          var remain = [];

          for (var group of countMap.values()) {
            remain.push(...group);
          }

          var scatteredGrouped = groupBySizeDown(remain);
          scatteredGrouped.forEach(g => scatteredPart.push(g)); // 5. 排序
...
//# sourceMappingURL=fdd15a5c710e6b4e6803628a47a966063a85b34e.js.map