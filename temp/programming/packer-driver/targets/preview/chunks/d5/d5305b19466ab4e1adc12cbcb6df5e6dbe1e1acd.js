  ...
  return {
    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, groupedCards) {
          // const result: number[][] = [];
          ...
          var prevSet = new Set(prevSelected); // 选中历史（按顺序追加）

          for (var c of selected) {
            if (!prevSet.has(c)) prevSelected.push(c);
          } // ✅ 找出需要恢复的牌（即选中的牌在 groupedCards 中已有）


          var restoreSet = new Set();
          var keepGrouped = [];

          for (var group of groupedCards) {
            var overlap = group.filter(c => selectedSet.has(c));

            if (overlap.length > 0) {
              // 说明当前组需要被恢复
              group.forEach(c => restoreSet.add(c));
            } else {
              // 保留未被打扰的组
              keepGrouped.push(group);
            }
          } // ✅ 收集“未理牌部分” + “被恢复的组”参与重新分组


          var regroupList = cards.filter(c => !groupedCards.flat().includes(c) || restoreSet.has(c)); // 先识别牌型组合

          var usedSet = new Set(); // ✅ 三带二识别
          ...
          var straightList = findStraightByCard(selected.filter(c => !usedSet.has(c)));

          for (var _group2 of straightList) {
          ...
          var lianduiList = findLiandui(selected.filter(c => !usedSet.has(c)));

          for (var _group3 of lianduiList) {
          ...
          var feijiList = findFeiji(selected.filter(c => !usedSet.has(c)));

          for (var _group4 of feijiList) {
          ...
          var sortValue = sortCardsBySizeDown(regroupList, regroupList.length);
          var uniqueList = getUniqueCard(sortValue);
          var idx = 0;
          ...
          for (var j = 0; j < uniqueList.length; j++) {
            ...
            for (var i = idx; i < sortValue.length; i++) {
              ...
              if (getCardSize(card) === rank) {
                list1.push(card);
                if (i === sortValue.length - 1) idx = i + 1;
              } else {
                idx = i;
                break;
              }
          ...
          } // ✅ 合并所有组（保留未被恢复的原有组 + 其他 + 新移动到后面的）


          return keepGrouped.concat(result).concat(moveToBack);
        }
        ...
        function findPlane(cards) {
          var map = GameLogic.getCardCountMap(cards);
...
//# sourceMappingURL=d5305b19466ab4e1adc12cbcb6df5e6dbe1e1acd.js.map