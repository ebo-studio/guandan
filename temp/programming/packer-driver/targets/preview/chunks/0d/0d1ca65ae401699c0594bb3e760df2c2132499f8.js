    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function getTripleWithPairPreferBig(cards) {
          ...
          for (var rank of ranksSorted) {
            var base = rankMap.get(rank); // if (!base) continue;

            var need = 3 - base.length;

            if (base.length >= 1 && heartCards.length >= need) {
              var _ret3 = function () {
                var triple = [...base, ...heartCards.slice(0, need)];
                var used = new Set(triple); // 查找对子部分
                ...
                for (var red of redLeft) {
                  for (var [_r5, list] of pairMap.entries()) {
                    if (list.length === 1) {
                      list.push(red);
                      break;
                    }
        ...
        function getCardCountMap_I(cards) {
          ...
          if (heardCard != 0) {
            for (var [_rank9, list] of map.entries()) {
              map.get(_rank9).push(heardCard);
            }
          }
        ...
        function findBombsByCount(cards, count) {
          ...
          for (var [_rank10, group] of map.entries()) {
        ...
        function findFlushStraight(cards) {
          ...
          for (var [_color, colorCards] of colorMap.entries()) {
            ...
            for (var _card2 of colorCards) {
              var _rank11 = _card2 % 16;

              if (!rankMap.has(_rank11)) rankMap.set(_rank11, []);
              rankMap.get(_rank11).push(_card2);
            }
        ...
        function findStraight_I(cards) {
          ...
          while (i <= ranks.length - 5) {
            ...
            if (length == 5) {
              ...
              for (var _rank12 of seq) {
                var cardsOfRank = map.get(_rank12);
          ...
          while (i <= sortedRanks.length - 5) {
            ...
            if (isConsecutive) {
              ...
              for (var _rank13 of seq) {
                var available = rankMap.get(_rank13).filter(idx => !usedIndex.has(idx));
        ...
        function countThreeWithTwoAndStraight(unused) {
          ...
          for (var _i13 = 0; _i13 < unused.length; _i13++) {
            var _rank14 = getCardSize(unused[_i13]);

            if (_rank14 >= 3 && _rank14 <= 14 && !usedIndex.has(_i13)) {
              if (!rankToIndexes.has(_rank14)) rankToIndexes.set(_rank14, []);
              rankToIndexes.get(_rank14).push(_i13);
            }
          }
        ...
        function findStraight(cards) {
          ...
          while (i <= ranks.length - 5) {
            ...
            if (length == 5) {
              ...
              for (var _rank15 of seq) {
                var cardsOfRank = map.get(_rank15);
                straight.push(cardsOfRank.pop());
        ...
        function newfindBombs(cards, levelRank) {
          ...
          for (var _rank16 of sortedRanks) {
            var group = countMap.get(_rank16);
...
//# sourceMappingURL=0d1ca65ae401699c0594bb3e760df2c2132499f8.js.map