    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function smartSortCards(cards) {
          ...
          // const pushAndMark = (group: number[]) => {
          //     result.push(group);
          //     group.forEach(card => used.add(card));
          // };
          // const getUnused = () => cards.filter(c => !used.has(c));
          // const rocket = this.findRocket(getUnused());
          // rocket.forEach(pushAndMark);
          // const bomb6 = findBombsByCount(getUnused(), 6);
          // bomb6.forEach(pushAndMark);
          // const flushStraights = this.findFlushStraight(getUnused());
          // flushStraights.forEach(pushAndMark);
          // const bomb5 = findBombsByCount(getUnused(), 5);
          // bomb5.forEach(pushAndMark);
          // const bomb4 = findBombsByCount(getUnused(), 4);
          // bomb4.forEach(pushAndMark);
          // const planes = findPlane(getUnused());
          // planes.forEach(pushAndMark);
          // const chainPairs = findChainPairs(getUnused());
          // chainPairs.forEach(pushAndMark);
          // const straights = findStraight(getUnused());
          // straights.forEach(pushAndMark);
          // const threes = findThreeWith(getUnused());
          // threes.forEach(pushAndMark);
          // const pairs = findPair(getUnused());
          // pairs.forEach(pushAndMark);
          ...
          getUnused().forEach(c => pushAndMark([c]));
          console.log("理牌分组：", mainResult.concat(randomPart));
        ...
        function findPlane(cards) {
          ...
          for (var card of cards) {
            var rank = card % 16;
            if (rank >= 14 || rank <= 1) continue; // 排除大小王 & 特殊点数
        ...
        function findChainPairs(cards) {
          var map = getCardCountMap(cards); // const ranks = Array.from(map.keys()).filter(r => map.get(r)!.length >= 2).sort((a, b) => a - b);
...
//# sourceMappingURL=dd9bba8f22a0811335b6646853ba57688a1946c1.js.map