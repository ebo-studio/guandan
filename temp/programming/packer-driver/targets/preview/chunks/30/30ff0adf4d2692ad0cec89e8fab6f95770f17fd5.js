    ...
    execute: function () {
      ...
      _export("CardLayer", CardLayer = (_dec = ccclass('CardLayer'), _dec2 = property(Prefab), _dec3 = property(SpriteAtlas), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(_crd && GameTimer === void 0 ? (_reportPossibleCrUseOfGameTimer({
            ...
            (function () {
              ...
              if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                ...
                _this2.groupedCards = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                  error: Error()
                }), GameLogic) : GameLogic).removeOutCardsFromGrouped(that.groupedCards, _this2.handCardsValue); // this.groupedCards = GameLogic.smartSortCards(this.handCardsValue);
        ...
        onBtnHint() {
          ...
          if (this.outCardList.length == 1 && (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            error: Error()
          }), GlobalData) : GlobalData).cardInfo.oneCard) {
            var hints = this.hintCards;
            var grouped = this.groupedCards;
            var total = hints.length;
            var checked = 0;
            var found = false;
            var hintCard = null; // 当前提示目标（用于比较）
            // ✅ 第一阶段：从 hintCards 中找不属于成型组合的单张

            while (checked < total) {
              var hint = hints[this.hintIndex];
              ...
              if (hint.length === 1) {
                var _ret2 = function () {
                  var card = hint[0];
                  hintCard = card;
                  var hintSize = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).getCardSize(card);
                  console.log('点数>>', hintSize); // ✅ 先从散牌中找一个比它大的

                  var biggerSingles = grouped.filter(g => g.length === 1 && (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).getCardSize(g[0]) >= hintSize).sort((a, b) => (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).getCardSize(a[0]) - (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).getCardSize(b[0]));

                  if (biggerSingles.length > 0) {
                    _this3.selectCardValue = [biggerSingles[0][0]]; // ✅ 选一个最小的符合条件的散牌
            ...
            if (!found && hintCard != null) {
              var hintSize = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).getCardSize(hintCard); // 拆对子

              var pairs = grouped.filter(g => g.length === 2 && (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).getCardSize(g[0]) >= hintSize).sort((a, b) => (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).getCardSize(a[0]) - (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              ...
              if (pairs.length > 0) {
                var pairGroup = pairs[this.breakPairIndex % pairs.length];
                var card = pairGroup[0];
                this.selectCardValue = [card];
                found = true;
                this.breakPairIndex++;
              }
            ...
            if (!found && hintCard != null) {
              var _hintSize = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).getCardSize(hintCard);

              var triples = grouped.filter(g => g.length === 3 && (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).getCardSize(g[0]) >= _hintSize).sort((a, b) => (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).getCardSize(a[0]) - (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).getCardSize(b[0]));
              ...
              if (triples.length > 0) {
                var tripleGroup = triples[this.breakTripleIndex % triples.length];
                var _card6 = tripleGroup[0];
                this.selectCardValue = [_card6];
                found = true;
            ...
            if (!found) {
              var bombs = grouped.filter(g => g.length >= 4);

              if (bombs.length > 0) {
                var bomb = bombs[this.breakBombIndex % bombs.length];
...
//# sourceMappingURL=30ff0adf4d2692ad0cec89e8fab6f95770f17fd5.js.map