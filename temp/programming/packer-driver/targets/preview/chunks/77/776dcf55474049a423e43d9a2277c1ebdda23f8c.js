      ...
      (function (_GameLogic) {
        ...
        function getCardCountMap_I(cards) {
          ...
          for (var _card4 of heartCards) {
            var targetKey = null;
            var minLen = Infinity; // 找一个 <4张的组，补进去（牌数越少越优先）
            ...
            if (targetKey !== null) {
              map.get(targetKey).push(_card4);
            } else {
              // 所有组都满了（极少见），补到任意组（比如最大长度组）
              var fallback = [...map.entries()].sort((a, b) => b[1].length - a[1].length)[0];
              fallback[1].push(_card4);
            }
          }
...
//# sourceMappingURL=776dcf55474049a423e43d9a2277c1ebdda23f8c.js.map