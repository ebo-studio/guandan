    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findBombsWithHeartCard(cards) {
          var countMap = getCardCountMap(cards); // 获取每个点数的牌组

          var bombs = [];
          var heartCards = cards.filter(card => isHeartCard(card)); // 识别红心级牌
          ...
          var _loop = function _loop(rank, list) {
            ...
            if (list.length === 3) {
              // 找到剩余的牌，检查是否有红心级牌
              var remainingCards = cards.filter(card => card !== list[0]);
              var sortedRemainingCards = remainingCards.sort((a, b) => a - b); // 判断是否能通过红心级牌补充

              var heartCard = heartCards.find(card => !remainingCards.includes(card));
        ...
        function getCardType(cards) {
          ...
          if (cards.length === 5) {
            ...
            for (var _list of _map.values()) {
              if (_list.length === 3) hasTriple = true;
              if (_list.length === 2) hasPair = true;
            }
          ...
          if (cards.length == 5) {
            ...
            for (var card of cards) {
              var _rank = getCardSize(card);

              if (_rank < 3 || _rank > 14) return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
                error: Error()
              }), GameDefine) : GameDefine).KIND_CARDS_ERROR; // 不能包含2和王
            ...
            for (var _i5 = 1; _i5 < ranks.length; _i5++) {
              if (ranks[_i5] !== ranks[_i5 - 1] + 1) {
                return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
                  error: Error()
                }), GameDefine) : GameDefine).KIND_CARDS_ERROR; // 非连续
              }
          ...
          for (var i = 0; i <= ranks.length - 5; i++) {
            ...
            if (isConsecutive) {
              ...
              for (var _rank4 of seq) {
                straight.push(map.get(_rank4).pop());
                if (map.get(_rank4).length === 0) map.delete(_rank4);
              }
          ...
          switch (type) {
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              if (hintList.length === 0) {
                for (var _i11 = groupedCards.length - 1; _i11 >= 0; _i11--) {
                  var _group6 = groupedCards[_i11]; // 只判断是否为炸弹（四张及以上，同点数）

                  if (_group6.length == 4) {
                    var _ret = function () {
                      var rank = _group6[0] % 16;

                      var isBomb = _group6.every(card => card % 16 === rank);

                      if (isBomb) {
                        hintList.push([..._group6]); // 不需要比较大小，炸弹就是万能压单
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var cards of colorGroups.values()) {
                ...
                for (var _i16 = 0; _i16 <= sizes.length - 5; _i16++) {
                  var ok = true;

                  for (var _j2 = 1; _j2 < 5; _j2++) {
                    if (sizes[_i16 + _j2] !== sizes[_i16 + _j2 - 1] + 1) {
                      ok = false;
                      break;
                    }
                  }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_1:
              {
                ...
                var _loop3 = function _loop3(_i18) {
                  var seq = allRanks.slice(_i18, _i18 + targetLength);
                ...
                for (var _i18 = 0; _i18 <= allRanks.length - targetLength; _i18++) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_COLOR:
              {
                ...
                var _loop4 = function _loop4(_i19) {
                  var seq = _allRanks.slice(_i19, _i19 + _targetLength);
                ...
                for (var _i19 = 0; _i19 <= _allRanks.length - _targetLength; _i19++) {
                  var _ret3 = _loop4(_i19);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_3:
              {
                ...
                for (var _i20 = 0; _i20 <= tripletGroups.length - _targetLength2 / 3; _i20++) {
                  var _group14 = [];
                  var _ok = true;

                  for (var _j4 = 0; _j4 < _targetLength2 / 3; _j4++) {
                    var cur = tripletGroups[_i20 + _j4];

                    if (_j4 > 0 && cur.rank !== tripletGroups[_i20 + _j4 - 1].rank + 1) {
                      _ok = false;
                      break;
                    }
                  ...
                  if (_ok && tripletGroups[_i20].rank > targetMin) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_2:
              {
                ...
                for (var _i21 = 0; _i21 <= pairGroups.length - _targetLength3 / 2; _i21++) {
                  var _group15 = [];
                  var _ok2 = true;

                  for (var _j5 = 0; _j5 < _targetLength3 / 2; _j5++) {
                    var _cur = pairGroups[_i21 + _j5];

                    if (_j5 > 0 && _cur.rank !== pairGroups[_i21 + _j5 - 1].rank + 1) {
                      _ok2 = false;
                      break;
                    }
                  ...
                  if (_ok2 && pairGroups[_i21].rank > _targetMin) {
...
//# sourceMappingURL=a162c2c29df40d04c11049d89a28b68468c00fdc.js.map