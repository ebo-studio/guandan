    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          var result = [];
          var moveToBack = [];
          var used = new Set(); // 判断当前选中是否为三带二

          var isThreeWithTwo = false;
          var triple = [];
          var pair = [];
          ...
          for (var j = 0; j < uniqueList.length; j++) {
            ...
            for (var i = idx; i < sortValue.length; i++) {
              if (getCardSize(d) === getCardSize(sortValue[i])) {
                ...
                if (i === sortValue.length - 1) {
                  if (list1.length) result.push(list1);
                }
              } else {
                idx = i;
                if (list1.length) result.push(list1);
...
//# sourceMappingURL=1e4b715f6d6d4011aed6a5113386d382ec58c6c3.js.map