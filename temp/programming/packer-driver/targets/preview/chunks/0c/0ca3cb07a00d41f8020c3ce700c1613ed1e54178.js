    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findFlushStraight(cards) {
          var result = [];
          var rankMap = new Map(); // 点数 → 牌组

          var redTrumpCards = []; // 红桃级牌

          var usedFrp = new Set(); // 分类
          ...
          for (var start = 3; start <= 10; start++) {
            var seq = [start, start + 1, start + 2, start + 3, start + 4];
            var missingRanks = [];
            var usedCards = [];

            for (var r of seq) {
              if (rankMap.has(r) && rankMap.get(r).length > 0) {
                usedCards.push(rankMap.get(r).pop());
                if (rankMap.get(r).length === 0) rankMap.delete(r);
              } else {
                missingRanks.push(r);
              }
            }
            ...
            if (missingRanks.length === 0) {
              result.push(usedCards);
              start += 4;
            } else if (missingRanks.length === 1 && redTrumpCards.length > 0) {
              ...
              if (frp !== undefined) {
                ...
                // 恢复用过的牌
                for (var c of usedCards) {
                  var _r2 = c % 16;

                  if (!rankMap.has(_r2)) rankMap.set(_r2, []);
                  rankMap.get(_r2).push(c);
                }
              }
            } else {
              // 恢复用过的牌
              for (var _c of usedCards) {
                var _r3 = _c % 16;

                if (!rankMap.has(_r3)) rankMap.set(_r3, []);
                rankMap.get(_r3).push(_c);
              }
            }
          }
          ...
          for (var i = 0; i <= ranks.length - 5; i++) {
            ...
            if (isConsecutive) {
              ...
              for (var _rank2 of seq) {
                straight.push(map.get(_rank2).pop());
                if (map.get(_rank2).length === 0) map.delete(_rank2);
              }
        ...
        function getHintCards(targetCards, groupedCards) {
          ...
          switch (type) {
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              if (hintList.length === 0) {
                for (var _i8 = groupedCards.length - 1; _i8 >= 0; _i8--) {
                  ...
                  if (_group4.length === 2) {
                    var _card2 = _group4[0];

                    if (compareCardSize(_card2, targetCards[0]) > 0) {
                      hintList.push([_card2]);
                    }
                  }
                }
              }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _card4 of groupedCards.flat()) {
                var color = GameLogic.getCardColor(_card4);
                if (!colorGroups.has(color)) colorGroups.set(color, []);
                colorGroups.get(color).push(_card4);
              }
...
//# sourceMappingURL=0ca3cb07a00d41f8020c3ce700c1613ed1e54178.js.map