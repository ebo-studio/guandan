    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          var remaining = selected.filter(card => !usedCards.has(card)); // ✅ 只处理选中的未用牌

          var sortValue = sortCardsBySizeDown(remaining, remaining.length); // 用 Map 按点数分组

          var rankMap = new Map();

          for (var card of sortValue) {
            var _rank5 = getCardSize(card); // 点数


            if (!rankMap.has(_rank5)) rankMap.set(_rank5, []);
            rankMap.get(_rank5).push(card);
          } // 再按是否选中来归入 moveToBack 或 result


          for (var _list2 of rankMap.values()) {
            if (_list2.some(c => selectedSet.has(c))) {
...
//# sourceMappingURL=47552652903fbb3173abf933c8d91970aaf592ec.js.map