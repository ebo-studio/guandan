    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function smartSortCards(cards) {
          ...
          var getUnused = () => cards.filter(c => !used.has(c));

          var rocket = this.findRocket(getUnused());
          rocket.forEach(pushAndMark);
          var bomb6 = findBombsByCount(getUnused(), 6);
          bomb6.forEach(pushAndMark);
          var flushStraights = this.findFlushStraight(getUnused());
          flushStraights.forEach(pushAndMark);
          var bomb5 = findBombsByCount(getUnused(), 5);
          bomb5.forEach(pushAndMark);
          var bomb4 = findBombsByCount(getUnused(), 4);
          ...
          var threes = findThreeWithTwo(getUnused());
          threes.forEach(pushAndMark);
          var pairs = findPair(getUnused());
          pairs.forEach(pushAndMark);
          getUnused().forEach(c => pushAndMark([c]));
          return result; // const mainResult: number[][] = [];     // 主牌型区（固定优先级）
...
//# sourceMappingURL=0f9f253a81ab729d1c049afc9ac058ccd7034d65.js.map