        ...
        onHandleBtn(data) {
          ...
          if (data.hitCards && data.hitCards.length > 0) {
            ...
            this.hintCards = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).getOnlyValueList(this.hintCards);
            console.log('服务器发过来的>>>', this.hintCards);
            this.hintCards.sort((a, b) => {
              // 单张优先
              if (a.length === 1 && b.length > 1) return -1;
              if (a.length > 1 && b.length === 1) return 1; // 点数小的优先

              var aSize = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).getCardSize(a[0]);
              var bSize = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).getCardSize(b[0]);
              return aSize - bSize;
            });
            console.log('自己排的>>>', this.hintCards); // console.log("this.hintCards---> ", this.hintCards);
            // //测试
            // let testList = [];
...
//# sourceMappingURL=03962babb18d672ea83f59ba215ba251bcbea0b7.js.map