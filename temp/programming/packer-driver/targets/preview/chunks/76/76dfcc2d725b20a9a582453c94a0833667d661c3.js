      ...
      (function (_GameLogic) {
        ...
        function findStraightByCard(cards) {
          ...
          for (var i = 0; i < cards.length; i++) {
            ...
            if (r < 3 || r > 13) continue; // 排除点数2、大小王

            if (!rankToIndexes.has(r)) rankToIndexes.set(r, []);
            rankToIndexes.get(r).push(i);
          }

          var uniqueRanks = [...rankToIndexes.keys()].sort((a, b) => a - b);

          var _loop = function _loop(start) {
            var group = [];
            var usedThisGroup = new Set();
            var heartUsed = 0;
            var expected = uniqueRanks[start];
            var current = expected;
            var index = start;

            while (true) {
              var hasRank = rankToIndexes.has(current);

              if (hasRank) {
                var _i6 = rankToIndexes.get(current).find(i => !usedIndexes.has(i) && !usedThisGroup.has(i));

                if (_i6 !== undefined) {
                  group.push(cards[_i6]);
                  usedThisGroup.add(_i6);
                } else {
                  // 无可用牌，尝试补红心
                  var red = redIndexes.find(i => !usedIndexes.has(i) && !usedThisGroup.has(i));

                  if (red !== undefined) {
                    group.push(cards[red]);
                    usedThisGroup.add(red);
                    heartUsed++;
                  } else {
                    break;
                  }
                ...
                // 完全没有该点数，必须用红心
                var _red = redIndexes.find(i => !usedIndexes.has(i) && !usedThisGroup.has(i));

                if (_red !== undefined) {
                  group.push(cards[_red]);
                  usedThisGroup.add(_red);
                  heartUsed++;
                } else {
                  break;
                }
          ...
          for (var start = 0; start < uniqueRanks.length; start++) {
            _loop(start);
          }
        ...
        function manualSortCards(selected, prevSelected, prevGrouped, allCards) {
          ...
          for (var _i8 = 0; _i8 <= sortedPairs.length - 3; _i8++) {
            var _r = sortedPairs[_i8][0] % 16;

            var _r2 = sortedPairs[_i8 + 1][0] % 16;

            var r3 = sortedPairs[_i8 + 2][0] % 16;

            if (_r2 === _r + 1 && r3 === _r2 + 1) {
          ...
          for (var _c3 of rest) {
            ...
            if (!restMap.has(_r3)) restMap.set(_r3, []);
            restMap.get(_r3).push(_c3);
          }
        ...
        function getTripleWithPairPreferBig(cards) {
          ...
          for (var _rank4 of ranksSorted) {
            var list = rankMap.get(_rank4);
            ...
            if (list.length >= 1 && heartCards.length >= need) {
              var _ret2 = function () {
                ...
                for (var red of redLeft) {
                  for (var [_r4, l] of pairMap.entries()) {
                    if (l.length === 1) {
                      l.push(red);
                      break;
                    }
        ...
        function getCardCountMap_I(cards) {
          ...
          for (var [_rank5, list] of map.entries()) {
            map.get(_rank5).push(heardCard);
          }
        ...
        function findBombsByCount(cards, count) {
          ...
          for (var [_rank6, group] of map.entries()) {
        ...
        function findFlushStraight(cards) {
          ...
          for (var [_color, colorCards] of colorMap.entries()) {
            ...
            for (var _card2 of colorCards) {
              var _rank7 = _card2 % 16;

              if (!rankMap.has(_rank7)) rankMap.set(_rank7, []);
              rankMap.get(_rank7).push(_card2);
            }
        ...
        function findStraight_I(cards) {
          ...
          while (i <= ranks.length - 5) {
            ...
            if (length == 5) {
              ...
              for (var _rank8 of seq) {
                var cardsOfRank = map.get(_rank8);
          ...
          while (i <= sortedRanks.length - 5) {
            ...
            if (isConsecutive) {
              ...
              for (var _rank9 of seq) {
                var available = rankMap.get(_rank9).filter(idx => !usedIndex.has(idx));
        ...
        function countThreeWithTwoAndStraight(unused) {
          ...
          for (var _i13 = 0; _i13 < unused.length; _i13++) {
            var _rank10 = getCardSize(unused[_i13]);

            if (_rank10 >= 3 && _rank10 <= 14 && !usedIndex.has(_i13)) {
              if (!rankToIndexes.has(_rank10)) rankToIndexes.set(_rank10, []);
              rankToIndexes.get(_rank10).push(_i13);
            }
          }
        ...
        function newfindBombs(cards, levelRank) {
          ...
          for (var _rank12 of sortedRanks) {
            var group = countMap.get(_rank12);
...
//# sourceMappingURL=76dfcc2d725b20a9a582453c94a0833667d661c3.js.map