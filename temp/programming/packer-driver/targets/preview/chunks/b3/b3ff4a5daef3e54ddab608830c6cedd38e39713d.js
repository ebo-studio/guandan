    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          var remaining = cards.filter((_, idx) => !usedIndex.has(idx)); // const remaining = cards.filter(card => !usedCards.has(card));
          ...
          for (var j = 0; j < uniqueList.length; j++) {
            ...
            for (var _i15 = idx; _i15 < sortValue.length; _i15++) {
              var card = sortValue[_i15];
              if (usedCards.has(card)) continue;

              if (getCardSize(card) === _rank5) {
                list1.push(card);

                if (_i15 === sortValue.length - 1) {
                  if (list1.length > 0) {
...
//# sourceMappingURL=b3ff4a5daef3e54ddab608830c6cedd38e39713d.js.map