    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          var map = new Map(); // 构建 rank -> cards 映射
          ...
          var tripleRanks = Array.from(map.keys()).filter(rank => map.get(rank).length >= 3).sort((a, b) => a - b); // 从小到大，优先出小的

          var _loop = function _loop(_rank2) {
            var three = map.get(_rank2).slice(0, 3); // 找不同点数的最小对子
...
//# sourceMappingURL=84b9b8d233145486e5d7357ec9419c3484f7eace.js.map