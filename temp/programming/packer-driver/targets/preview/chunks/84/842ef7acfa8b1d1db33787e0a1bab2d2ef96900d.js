    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findFeiji(cards) {
          var map = getCardCountMap(cards);
          var rankToCards = new Map(); // 收集所有三张牌的 rank 和对应的牌值

          for (var [rank, list] of map.entries()) {
            if (rank < 3 || rank > 14) continue; // 排除非法牌点

            if (list.length >= 3) {
              rankToCards.set(rank, list.slice(0, 3)); // 只取3张
            }
          }

          if (rankToCards.size < 2) return [];
          var sortedRanks = Array.from(rankToCards.keys()).sort((a, b) => a - b); // 寻找连续的牌点 rank 组成的飞机

          var feiji = [];

          for (var i = 0; i < sortedRanks.length - 1; i++) {
            var start = sortedRanks[i];
            var group = [rankToCards.get(start)];
            ...
            for (var j = i + 1; j < sortedRanks.length; j++) {
              ...
              if (curr === prev + 1) {
                group.push(rankToCards.get(curr));

                if (group.length >= 2) {
                  // 找到连续的两组三条就组成飞机（可以扩展成更长）
                  var flat = group.flat();
                  return [flat];
                }
              } else {
...
//# sourceMappingURL=842ef7acfa8b1d1db33787e0a1bab2d2ef96900d.js.map