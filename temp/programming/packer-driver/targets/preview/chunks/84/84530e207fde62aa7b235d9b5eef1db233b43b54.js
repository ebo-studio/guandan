      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          var triples = Array.from(map.entries()).filter(_ref => {
            ...
            var [rank, group] = _ref2;
            return {
              rank,
              group: group.slice(0, 3)
            };
          }).sort((a, b) => a.rank - b.rank); // 小牌在前
          // 找出所有可用对子（排除三张的点数）

          var pairs = Array.from(map.entries()).filter(_ref3 => {
            var [rank, group] = _ref3;
            return group.length >= 2;
          }).map(_ref4 => {
            var [rank, group] = _ref4;
            return {
              rank,
          ...
          var _loop = function _loop(triple) {
            var pair = pairs.find(p => p.rank !== triple.rank);

            if (pair) {
              return {
                v: [[...triple.group, ...pair.group]]
              };
            }
          };
...
//# sourceMappingURL=84530e207fde62aa7b235d9b5eef1db233b43b54.js.map