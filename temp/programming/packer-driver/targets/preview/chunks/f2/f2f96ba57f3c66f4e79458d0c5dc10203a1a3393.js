    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          var selectedSet = new Set(selectedHistory); // 2. 查找三带二组合（并确保三带二不拆分）

          var triple = [];
          var pair = [];
          var map = getCardCountMap(selectedHistory); // 获取点数的分组

          for (var list of map.values()) {
            if (list.length === 3) triple = list;else if (list.length === 2) pair = list;
          }

          var usedRanks = new Set(); // 过滤用的点数集

          if (triple.length === 3 && pair.length === 2) {
            var group = [...triple, ...pair];
            moveToBack.push(group); // 组合好的三带二推到最后

            group.forEach(c => usedRanks.add(c)); // 标记已使用的牌（而不仅仅是点数）
          ...
          for (var j = 0; j < uniqueList.length; j++) {
            ...
            for (var i = idx; i < sortValue.length; i++) {
              ...
              if (usedRanks.has(card)) continue; // 跳过已组合的牌
...
//# sourceMappingURL=f2f96ba57f3c66f4e79458d0c5dc10203a1a3393.js.map