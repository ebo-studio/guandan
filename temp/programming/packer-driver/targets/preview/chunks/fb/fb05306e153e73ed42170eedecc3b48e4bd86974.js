    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          } // 2. 遍历 tmpList，将 selected 中的牌分组移到后面


          var selectedSet = [...selected];
          var prevSet = [...prevSelected];
          var remain = [];
          var prevGroup = [];
          var newGroup = [];

          for (var group of tmpList) {
            // 判断这一组是否属于 prevSelected
            if (group.every(card => prevSet.includes(card))) {
              prevGroup.push(group);
              continue;
            } // 判断这一组是否属于本次新选中


            if (group.every(card => selectedSet.includes(card))) {
              newGroup.push(group);
              continue;
            } // 否则归为未选中
          ...
          return [...remain, ...prevGroup, ...newGroup];
        }
...
//# sourceMappingURL=fb05306e153e73ed42170eedecc3b48e4bd86974.js.map