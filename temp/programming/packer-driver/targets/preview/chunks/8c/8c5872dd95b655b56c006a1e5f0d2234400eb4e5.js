    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          var pairList = []; // for (const [rank, list] of countMap.entries()) {
          //     if (list.length === 2) {
          //         pairList.push(list);
          //     }
          // }

          if (countMap[0].length == 2) {
            pairList.push(countMap[0]);
          } // ✅ 步骤8：将剩余未处理的牌按点数分组
        ...
        function getCardType(cards) {
          ...
          if (cards.length === 5) {
            ...
            for (var [_rank3, list] of rankMap.entries()) {
              if (list.length === 3) {
                triple = list;
                break;
              }
            ...
            if (triple.length === 3) {
              ...
              for (var _card of remain) {
                ...
                if (!pairMap.has(_rank4)) pairMap.set(_rank4, []);
                pairMap.get(_rank4).push(_card);
              } // 判断是否存在一对（包括红心级牌拼凑）


              for (var _list2 of pairMap.values()) {
                if (_list2.length >= 2) {
                  return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
                    error: Error()
          ...
          if (cards.length == 5) {
            ...
            for (var _card2 of cards) {
              var _rank5 = getCardSize(_card2);

              if (_rank5 < 3 || _rank5 > 14) return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
                error: Error()
              }), GameDefine) : GameDefine).KIND_CARDS_ERROR; // 不能包含2和王
        ...
        function getCardCountMap_I(cards) {
          ...
          for (var card of cards) {
            if (isHeartCard(card)) {
              heartCards.push(card); // 暂时不加入 map
            ...
            map.get(rank).push(card);
          } // 第二步：将红心级牌补充到已有点数组中（按需加入）


          for (var _card4 of heartCards) {
            // ✅ 选择一个数量最少的组补充
            var targetRank = null;
            var minLen = Infinity;

            var _loop3 = function _loop3(list, _rank6) {
              if (list.length < 4 && list.every(c => c % 16 === _rank6)) {
                // 仅补到点数一致的组
            ...
            if (targetRank !== null) {
              map.get(targetRank).push(_card4);
            } else {
              // ❗️如果没有合适的目标，就保留为单独 key（防止遗漏）
              map.set(_card4 % 16, [_card4]);
            }
          }
        ...
        function findFlushStraight(cards) {
          ...
          for (var [_color, colorCards] of colorMap.entries()) {
            ...
            for (var _card5 of colorCards) {
              var _rank8 = _card5 % 16;

              if (!rankMap.has(_rank8)) rankMap.set(_rank8, []);
              rankMap.get(_rank8).push(_card5);
            }
        ...
        function getHintCards(targetCards, groupedCards) {
          ...
          switch (type) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i23 = groupedCards.length - 1; _i23 >= 0; _i23--) {
                ...
                if (_group18.length === 3) {
                  var _card9 = _group18[0];

                  if (isSingleCardStronger(_card9, targetCards[0], (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
                  }), GlobalData) : GlobalData).cardInfo.levelCard)) {
                    hintList.push([_card9]);
                  }
                }
              } //找四炸
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _card10 of groupedCards.flat()) {
                var color = GameLogic.getCardColor(_card10);
                if (!colorGroups.has(color)) colorGroups.set(color, []);
                colorGroups.get(color).push(_card10);
              }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_1:
              {
                ...
                for (var _card11 of singles) {
                  var _size2 = GameLogic.getCardSize(_card11);

                  if (!rankMap.has(_size2)) rankMap.set(_size2, []);
                  rankMap.get(_size2).push(_card11);
                }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_COLOR:
              {
                ...
                for (var _card12 of _singles) {
                  var _size3 = GameLogic.getCardSize(_card12);

                  if (!_rankMap.has(_size3)) _rankMap.set(_size3, []);

                  _rankMap.get(_size3).push(_card12);
                }

                var _allRanks = Array.from(_rankMap.keys()).sort((a, b) => a - b);

                var _loop6 = function _loop6(_i35) {
        ...
        function getHintList(handCards, targetCards) {
          ...
          if (targetType === (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            if (hintList.length === 0) {
              for (var _i39 = groupedCards.length - 1; _i39 >= 0; _i39--) {
                ...
                if (_group33.length === 3) {
                  var _card13 = _group33[0];

                  if (isSingleCardStronger(_card13, targetCards[0], (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
                  }), GlobalData) : GlobalData).cardInfo.levelCard)) {
                    hintList.push([_card13]);
                  }
                }
              }
            }
            ...
            var _loop7 = function _loop7(_group37) {
...
//# sourceMappingURL=8c5872dd95b655b56c006a1e5f0d2234400eb4e5.js.map