    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          // 基础分组
          var result = [];
          var moveToBack = []; // 1. 更新历史记录

          var prevSet = new Set(prevSelected);
          var newSelected = selected.filter(card => !prevSet.has(card));

          for (var card of newSelected) {
            if (!selectedHistory.includes(card)) {
              selectedHistory.push(card);
            }
          } // 2. 查找三带二组合（主动构建，不依赖 group 拆分）


          var map = getCardCountMap(selectedHistory);
          var triple = [];
          var pair = [];
          var threeWithTwoGroup = [];

          for (var list of map.values()) {
            if (list.length === 3) triple = list;
            if (list.length === 2) pair = list;
          }

          var usedSet = new Set();

          if (triple.length === 3 && pair.length === 2) {
            threeWithTwoGroup = [...triple, ...pair];
            moveToBack.push(threeWithTwoGroup);
            threeWithTwoGroup.forEach(c => usedSet.add(c));
          } // 3. 将剩下的牌按点数分组（排除掉组合牌）
          ...
          for (var i = 0; i < sortValue.length;) {
            var _card = sortValue[i];

            if (usedSet.has(_card)) {
              i++;
              continue;
            }

            var size = getCardSize(_card);
            var group = [];

            while (i < sortValue.length && getCardSize(sortValue[i]) === size && !usedSet.has(sortValue[i])) {
            ...
            if (group.length > 0) {
              // 按是否是历史选中过的决定推后或保留
              if (group.some(c => selectedHistory.includes(c))) {
                moveToBack.push(group);
              } else {
                result.push(group);
              }
...
//# sourceMappingURL=7a254f2a4c5bd27ac113a3ed1b7d97abe9899940.js.map