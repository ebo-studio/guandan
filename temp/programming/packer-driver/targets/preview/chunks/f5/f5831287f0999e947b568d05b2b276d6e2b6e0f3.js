    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function smartSortCards(cards) {
          ...
          if (result.straightCount === 1) {
            ...
            if (result.threeWithTwoCount >= 2) {
              findThreeWithTwoBy(getUnused()).forEach(g => pushAndMark(g, false));
            } else {
              if (result.threeWithTwoCount != 0) {
                findThreeWithTwo(getUnused(), usedIndex).forEach(g => pushAndMark(g, false)); // 三带二
              } else {
        ...
        function countThreeWithTwoAndStraight(unused) {
          var usedIndex = new Set(); // 标记用过的牌索引

          var rankMap = new Map(); // 点数 -> 索引列表
          // 分组：点数 -> 所有索引
          ...
          var sortedRanks = Array.from(rankMap.keys()).sort((a, b) => b - a); // 大点优先
          // ✅ Step 1: 找三带二

          for (var mainRank of sortedRanks) {
            var mainIdxList = rankMap.get(mainRank).filter(idx => !usedIndex.has(idx));

            if (mainIdxList.length >= 3) {
              var three = mainIdxList.slice(0, 3); // 找对子（不能是同点数）
          ...
          } // ✅ Step 2: 找顺子（最少5张，不含2和王）


          var rankToIndexes = new Map();

          for (var _i9 = 0; _i9 < unused.length; _i9++) {
            var _card3 = unused[_i9];

            var _rank5 = _card3 % 16;
...
//# sourceMappingURL=f5831287f0999e947b568d05b2b276d6e2b6e0f3.js.map