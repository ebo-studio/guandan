        ...
        function findThreeWithTwo(cards) {
          ...
          for (var [rank, list] of map.entries()) {
            if (rank === (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.levelCard) continue; // ❗ 过滤掉级牌 2

            if (list.length >= 3) {
              threes.push(list.slice(0, 3));

              if (list.length >= 5) {
                // 多于5张也可能做对子（如55555）
                pairs.push(list.slice(3, 5));
              }
          ...
          threes.sort((a, b) => getCardSize(a[0]) - getCardSize(b[0]));

          var _loop = function _loop(three) {
            var threeRank = getCardSize(three[0]);
            var pair = pairs.find(p => getCardSize(p[0]) !== threeRank);
          ...
          for (var three of threes) {
            var _ret = _loop(three);
...
//# sourceMappingURL=97716604b7788ba7102772f3a8b5c3c736cd2ad1.js.map