    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards) {
          ...
          var getSize = card => card % 16; // 获取张数映射
          ...
          for (var card of cards) {
            ...
            map.get(size).push(card);
          }

          var usedSize = new Set(); // 获取所有 >=3 的点数（按牌点从小到大排序）

          var threeRanks = Array.from(map.entries()).filter(_ref => {
            var [_, list] = _ref;
            return list.length >= 3;
          }).sort((a, b) => getSize(a[0]) - getSize(b[0]));

          var _loop = function _loop(rank, list3) {
            var three = list3.slice(0, 3);
            usedSize.add(rank); // 找带的对子（不能是同一 rank，不能是王或级牌）

            var pairRank = Array.from(map.entries()).filter(_ref2 => {
              var [r, list] = _ref2;
              return list.length >= 2 && r !== rank && !usedSize.has(r) && getSize(r) < 14 && // 排除大王(14)、小王(15)
              getSize(r) !== (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                error: Error()
              }), GlobalData) : GlobalData).cardInfo.levelCard;
            } // 排除级牌 2
            ).sort((a, b) => getSize(a[0]) - getSize(b[0])) // 小点优先
            .map(_ref3 => {
              var [r, list] = _ref3;
            ...
            if (pairRank) {
              result.push([...three, ...pairRank]); // 标记这组牌用过

              usedSize.add(getSize(pairRank[0]));
            }
          };

          for (var [rank, list3] of threeRanks) {
            _loop(rank, list3);
          }
        ...
        function smartSortCards(cards) {
          ...
          findThreeWithTwo(getUnused()).forEach(g => pushAndMark(g, false)); // 三带二（最小三张）
          // ===== 剩余散牌，按点数从大到小排列 =====
...
//# sourceMappingURL=58217edc4779210ed55a844ff2d63db7154df801.js.map