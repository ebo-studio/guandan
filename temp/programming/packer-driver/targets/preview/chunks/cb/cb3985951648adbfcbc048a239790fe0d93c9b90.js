    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function smartSortCards(cards) {
          ...
          var randomPart = []; // 杂牌型：飞机、连对、顺子、三带二

          var scatteredPart = []; // 未组成牌型的单张、对子、三张

          var usedIndex = new Set();

          var getUnused = () => cards.filter((_, i) => !usedIndex.has(i));

          var pushAndMark = (group, target) => {
            var realGroup = [];
          ...
          findBombsByCount(getUnused(), 4).forEach(g => pushAndMark(g, mainResult)); // 单张大王/小王/级牌识别

          var remain = getUnused();

          for (var i = 0; i < remain.length; i++) {
            var point = remain[i] % 16;

            if (point === 15 || point === 14 || point === (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.levelCard) {
              pushAndMark([remain[i]], mainResult);
            }
          } // 杂牌型识别（飞机、连对、顺子、三带二）


          var plane = findPlane(getUnused());
          var chainPairs = findChainPairs(getUnused());
          var straights = findStraight(getUnused());
          var threeWithTwo = findThreeWithTwo(getUnused());
          [...plane, ...chainPairs, ...straights, ...threeWithTwo].sort((a, b) => minRank(b) - minRank(a)).forEach(g => pushAndMark(g, randomPart)); // 剩余未组牌的 => 散牌（单张、对子、三张）

          var countMap = getCardCountMap(getUnused());
          var remainGroups = [];

          for (var [rank, list] of countMap.entries()) {
            var group = list.slice(); // 所有未用的相同点数牌

            while (group.length >= 3) {
              remainGroups.push(group.splice(0, 3));
            }

            while (group.length >= 2) {
              remainGroups.push(group.splice(0, 2));
            }

            while (group.length >= 1) {
              remainGroups.push(group.splice(0, 1));
            }
          }

          remainGroups.sort((a, b) => getCardSize(b[0]) - getCardSize(a[0])).forEach(g => pushAndMark(g, scatteredPart));
...
//# sourceMappingURL=cb3985951648adbfcbc048a239790fe0d93c9b90.js.map