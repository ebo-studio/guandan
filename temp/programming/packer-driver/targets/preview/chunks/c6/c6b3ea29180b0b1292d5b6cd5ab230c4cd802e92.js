      ...
      (function (_GameLogic) {
          ...
          for (var seq of seqList) {
            ...
            for (var r of seq) {
              ...
              for (var _i5 = 0; _i5 < cards.length; _i5++) {
                if (usedIndex.has(_i5)) continue;
                var card = cards[_i5];

                var _rank = getCardRank(card);

                var color = getCardColor(card);

                if (r === 2) {
                  if (_rank === 2 && color === HONGTAO) {
                    group.push(card);
                    usedIndex.add(_i5);
                    found = true;
                    break;
        ...
        function findStraightByCard(cards) {
          ...
          for (var i = 0; i < cards.length; i++) {
            ...
            if (r < 3 || r > 13) continue; // 排除2、小王、大王
          ...
          for (var _i6 = 0; _i6 <= legalRanks.length - 5; _i6++) {
            for (var len = legalRanks.length - _i6; len >= 5; len--) {
              allSeqs.push(legalRanks.slice(_i6, _i6 + len));
            }
          }
          ...
          var _loop = function _loop(seq) {
            ...
            for (var _r of seq) {
              var idxList = rankToIndexes.get(_r) || [];
        ...
        function findLiandui(cards) {
          ...
          for (var card of normalCards) {
            var _rank2 = getCardSize(card);

            if (_rank2 < 3 || _rank2 > 14) continue; // 排除2和王

            if (!map.has(_rank2)) map.set(_rank2, []);
            map.get(_rank2).push(card);
          }

          var rankToPair = new Map(); // 尝试识别每个点数是否能组成对子（原生或补红心）

          for (var [_rank3, list] of map.entries()) {
            if (list.length === 2) {
              rankToPair.set(_rank3, [...list]);
            } else if (list.length === 1 && redLeft.length >= 1) {
              // 补一张红心成对子
              var red = redLeft.shift();
              rankToPair.set(_rank3, [list[0], red]);
              redUsed.add(red);
            }
          }
          ...
          for (var i = 0; i < sortedRanks.length; i++) {
            var _rank4 = sortedRanks[i];

            if (temp.length === 0 || _rank4 === sortedRanks[i - 1] + 1) {
              temp.push(rankToPair.get(_rank4));
            } else {
              if (temp.length >= 3) {
                results.push(temp.flat());
              }

              temp = [rankToPair.get(_rank4)];
            }
          } // 最后一次缓存
        ...
        function findFeiji(cards) {
          ...
          for (var card of normalCards) {
            var _rank5 = getCardRank(card);

            if (_rank5 < 3 || _rank5 > 14) continue; // 排除 2 和大小王

            if (!rankMap[_rank5]) rankMap[_rank5] = [];

            rankMap[_rank5].push(card);
          }

          var rankToTriplet = {};
          var redLeft = [...redCards];
          var sortedRanks = Object.keys(rankMap).map(Number).sort((a, b) => a - b);

          for (var i = 0; i < sortedRanks.length; i++) {
            var _rank6 = sortedRanks[i];
            var list = rankMap[_rank6];

            if (list.length >= 3) {
              rankToTriplet[_rank6] = list.slice(0, 3); // 原生三张
            } else if (list.length === 2 && redLeft.length >= 1) {
              rankToTriplet[_rank6] = [...list, redLeft.shift()];
            } else if (list.length === 1 && redLeft.length >= 2) {
              rankToTriplet[_rank6] = [...list, redLeft.shift(), redLeft.shift()];
            }
          }

          var tripletRanks = Object.keys(rankToTriplet).map(Number).sort((a, b) => a - b);

          for (var _i7 = 0; _i7 < tripletRanks.length - 1; _i7++) {
            var a = tripletRanks[_i7];
            var b = tripletRanks[_i7 + 1];

            if (b === a + 1) {
              var triplet1 = rankToTriplet[a];
              var triplet2 = rankToTriplet[b];

              if (triplet1 && triplet2 && triplet1.length === 3 && triplet2.length === 3) {
        ...
        function isFengRenPeiSorted(cards) {
          // 判断是否是连续的顺子
          var heartCount = cards.filter(isHeartCard).length;
          var ranks = Array.from(new Set(cards.filter(c => !isHeartCard(c)).map(c => c % 16).filter(r => r >= 3 && r <= 13) // 排除 2、大小王
          )).sort((a, b) => a - b);
          if (ranks.length + heartCount < 5) return false; // 总长度不足
          // 尝试从所有起点出发，连续补红心

          for (var start = 0; start <= ranks.length - 1; start++) {
            var needHearts = 0;
            var last = ranks[start];
            var count = 1;

            for (var i = start + 1; i < ranks.length; i++) {
              var gap = ranks[i] - last;
              if (gap === 0) continue; // 重复，跳过
              ...
              if (count >= 5 && needHearts <= heartCount) {
                return true;
              }
            } // 边界情况：所有牌已经连着，但还差红心补尾部


            if (ranks.length >= 1 && ranks.length + heartCount >= 5) {
              return true;
            }
          }
        ...
        function manualSortCards(selected, prevSelected, prevGrouped, allCards) {
          ...
          for (var c of remaining) {
            ...
            if (!countMap.has(_rank7)) countMap.set(_rank7, []);
            countMap.get(_rank7).push(c);
          }

          for (var [_rank8, list] of countMap.entries()) {
            if (list.length >= 6) {
              result.unshift(list);
          ...
          for (var [_rank9, _list] of rankMap.entries()) {
          ...
          for (var _i8 = 0; _i8 < sorted.length; _i8++) {
            var curr = sorted[_i8];
          ...
          for (var _i9 = 0; _i9 <= sortedPairs.length - 3; _i9++) {
            var _r2 = sortedPairs[_i9][0] % 16;

            var _r3 = sortedPairs[_i9 + 1][0] % 16;

            var r3 = sortedPairs[_i9 + 2][0] % 16;

            if (_r3 === _r2 + 1 && r3 === _r3 + 1) {
              var _group5 = [...sortedPairs[_i9].slice(0, 2), ...sortedPairs[_i9 + 1].slice(0, 2), ...sortedPairs[_i9 + 2].slice(0, 2)];
          ...
          for (var _c3 of rest) {
            ...
            if (!restMap.has(_r4)) restMap.set(_r4, []);
            restMap.get(_r4).push(_c3);
          }
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          for (var _group7 of triplets) {
            ...
            _group7.forEach(c => usedCards.add(c));
          ...
          for (var s of straights) {
            ...
            if (isSameColorStraight) {
              (function () {
                // 检查是否为同花顺
                var firstCardColor = s[0] % 4;
                var isSameColor = s.every(card => card % 4 === firstCardColor);
          ...
          for (var _group8 of lianduiGroups) {
            ...
            _group8.forEach(c => usedCards.add(c));
          } // ✅ 步骤7：识别两对
          ...
          for (var [_rank10, list] of countMap.entries()) {
            if (list.length === 2) {
        ...
        function findBombsWithHeartCard(cards) {
          ...
          var _loop2 = function _loop2(list, _rank12) {
          ...
          for (var [_rank12, list] of countMap) {
            var _ret2 = _loop2(list, _rank12);
        ...
        function getCardTypeByFeiji(cards) {
          ...
          for (var card of normalCards) {
            var _rank13 = getCardSize(card);

            if (_rank13 < 3 || _rank13 > 14) return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              error: Error()
            }), GameDefine) : GameDefine).KIND_CARDS_ERROR; // 不允许 2 和王

            if (!countMap.has(_rank13)) countMap.set(_rank13, []);
            countMap.get(_rank13).push(card);
          }

          var tripletRanks = [];
          var redLeft = redCount; // 遍历点数，尝试补出三张

          var sortedRanks = [...countMap.keys()].sort((a, b) => a - b);

          for (var _rank14 of sortedRanks) {
            var list = countMap.get(_rank14);

            if (list.length === 3) {
              tripletRanks.push(_rank14);
            } else if (list.length === 2 && redLeft >= 1) {
              tripletRanks.push(_rank14);
              redLeft -= 1;
            } else if (list.length === 1 && redLeft >= 2) {
              tripletRanks.push(_rank14);
              redLeft -= 2;
            }
          }
          ...
          if (tripletRanks.length < 2) return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            error: Error()
          }), GameDefine) : GameDefine).KIND_CARDS_ERROR; // 按顺序检查是否连续
          ...
          return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            error: Error()
          }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_3; // ✅ 是合法飞机
        }
        ...
        function getCardTypeByLiandui(cards) {
          ...
          for (var card of normalCards) {
            var _rank15 = getCardSize(card);

            if (_rank15 < 3 || _rank15 > 14) return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              error: Error()
            }), GameDefine) : GameDefine).KIND_CARDS_ERROR; // 排除2和王

            if (!map.has(_rank15)) map.set(_rank15, []);
            map.get(_rank15).push(card);
          }

          var pairRanks = [];
          var redLeft = [...redCards];

          for (var [_rank16, list] of map.entries()) {
            if (list.length === 2) {
              pairRanks.push(_rank16);
        ...
        function getCardType(cards) {
          ...
          if (cards.length == 3) {
            ...
            for (var [_rank17, list] of map.entries()) {
              if (list.length === 3) {
                return (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
                  error: Error()
          ...
          var straights = findStraightWithHongtao2_Safe(cards);
        ...
        function findWangzha(cards) {
          ...
          for (var [_rank18, list] of map.entries()) {
            if ((_rank18 === 16 || _rank18 === 17) && list.length >= 2) {
              // 16: 小王，17: 大王
        ...
        function findTonghuashun(cards) {
          ...
          for (var colorCards of mapByColor.values()) {
            ...
            for (var _card of colorCards) {
              var _rank19 = getCardSize(_card);

              if (_rank19 < 3 || _rank19 > 14) continue; // 不能包含2或王

              if (!ranksMap.has(_rank19)) ranksMap.set(_rank19, []);
              ranksMap.get(_rank19).push(_card);
            }
        ...
        function findThreeWithTwoBy(cards, usedIndex, maxCount) {
          ...
          for (var card of cards) {
            var _rank20 = getCardSize(card);

            ;
            var color = Math.floor(card / 16); // 排除大小王、参谋、非红桃级牌2

            if (_rank20 === (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.levelCard && color !== 0 || _rank20 <= 1 || _rank20 > 14) continue;
            if (!countMap.has(_rank20)) countMap.set(_rank20, []);
            countMap.get(_rank20).push(card);
          }
        ...
        function findThreeWithTwo(cards, usedIndex, maxCount) {
          ...
          for (var card of cards) {
            ...
            if (_rank21 <= (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.levelCard || _rank21 > 14) continue; // 排除大小王/参谋/级牌

            if (!countMap.has(_rank21)) countMap.set(_rank21, []);
            countMap.get(_rank21).push(card);
          } // 找所有三张
        ...
        function getOneCardSizeList(value) {
          ...
          for (var _i10 = 0; _i10 < pairs.length; _i10++) {
            if (temp.length === 0) {
              temp.push(pairs[_i10]);
            } else {
              var last = getCardSize(temp[temp.length - 1][0]);
              var current = getCardSize(pairs[_i10][0]);
          ...
          var findStraightGroups = cards => {
            ...
            for (var _i11 = 0; _i11 < unique.length; _i11++) {
              var cur = unique[_i11];
        ...
        function getGroupPower(cards) {
          ...
          if (size >= 4 && size % 2 === 0) {
            ...
            for (var _i12 = 1; _i12 < pairs.length; _i12++) {
              if (pairs[_i12] !== pairs[_i12 - 1] + 1 || pairs[_i12] >= 15) return 0;
            }
        ...
        function isStraight(cards) {
          ...
          for (var card of cards) {
            var _rank22 = getCardSize(card);

            if (_rank22 >= 3 && _rank22 <= 14) {
              // 只考虑3到A的牌
              rankSet.add(_rank22);
            }
          } // 顺子必须没有重复的牌
        ...
        function isLiandui(cards) {
          ...
          for (var [_rank23, list] of map.entries()) {
            if (_rank23 < 3 || _rank23 > 14) continue; // 排除 2 和大小王

            if (list.length === 2) {
              pairRanks.push(_rank23); // 记录有效的对子
            }
          } // 必须有足够的对子
        ...
        function isThreeWithTwo(cards) {
          ...
          for (var [_rank24, list] of map.entries()) {
            if (_rank24 < 3 || _rank24 > 14) continue; // 排除 2 和 大小王
        ...
        function getTripleWithPairPreferBig(cards) {
          ...
          for (var i = 0; i < ranksSorted.length; i++) {
            var _rank25 = ranksSorted[i];
            var base = rankMap.get(_rank25);
            console.log('尝试点数：', _rank25, ' 找到牌组：', base);
            if (!base) continue;
        ...
        function getCardCountMap_I(cards) {
          ...
          for (var card of cards) {
            ...
            map.get(_rank27).push(card);
          }

          if (heardCard != 0) {
            for (var [_rank28, list] of map.entries()) {
              map.get(_rank28).push(heardCard);
            }
          }
        ...
        function findBombs(cards) {
          ...
          for (var [_rank29, group] of map.entries()) {
            if (group.length >= 4) {
        ...
        function findBombsByCount(cards, count) {
          ...
          for (var card of cards) {
            ...
            if (_rank30 === (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.levelCard && color === 2) {
              frpCards.push(card);
            } else {
              if (!map.has(_rank30)) map.set(_rank30, []);
              map.get(_rank30).push(card);
            }
          }

          var usedFrp = new Set(); // 红桃级牌使用记录

          for (var [_rank31, group] of map.entries()) {
        ...
        function findFlushStraight(cards) {
          ...
          for (var card of cards) {
            ...
            var color = Math.floor(card / 16);
            if (_rank32 < 3 || _rank32 > 14) continue;

            if (color === 2 && _rank32 === (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.levelCard) {
          ...
          for (var [_color, colorCards] of colorMap.entries()) {
            ...
            for (var _card2 of colorCards) {
              var _rank33 = _card2 % 16;

              if (!rankMap.has(_rank33)) rankMap.set(_rank33, []);
              rankMap.get(_rank33).push(_card2);
            }
            ...
            for (var start = 3; start <= 10; start++) {
              ...
              if (missing === 0) {
                ...
                if (frp !== undefined) {
                  ...
                  // 恢复
                  for (var [_r5, _c4] of tempPopped) {
                    if (!rankMap.has(_r5)) rankMap.set(_r5, []);
                    rankMap.get(_r5).push(_c4);
                  }
                }
              } else {
                // 恢复
        ...
        function findPlane(cards) {
          ...
          for (var card of cards) {
            ...
            if (_rank34 >= 14 || _rank34 <= 1) continue; // 排除非红桃的级牌（2）

            if (_rank34 === (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
        ...
        function findStraight_I(cards) {
          ...
          for (var card of cards) {
            var _rank35 = getCardSize(card);

            if (_rank35 < 3 || _rank35 > 14) continue; // 3~A

            if (!map.has(_rank35)) map.set(_rank35, []);
            map.get(_rank35).push(card);
          }
          ...
          while (i <= ranks.length - 5) {
            ...
            if (length == 5) {
              ...
              for (var _rank36 of seq) {
                var cardsOfRank = map.get(_rank36);
        ...
        function findStraight_II(cards, usedIndex) {
          ...
          for (var _i13 = 0; _i13 < cards.length; _i13++) {
            if (usedIndex.has(_i13)) continue;
            var card = cards[_i13];

            var _rank37 = card % 16;

            if (_rank37 < 3 || _rank37 > 13) continue; // 只允许 3~K 进入顺子

            if (!rankMap.has(_rank37)) rankMap.set(_rank37, []);
            rankMap.get(_rank37).push(_i13); // 保存的是索引，方便精确控制使用
          }
          ...
          while (i <= sortedRanks.length - 5) {
            ...
            if (isConsecutive) {
              ...
              for (var _rank38 of seq) {
                var available = rankMap.get(_rank38).filter(idx => !usedIndex.has(idx));
        ...
        function countThreeWithTwoAndStraight(unused) {
          ...
          for (var i = 0; i < unused.length; i++) {
            ...
            if (!rankMap.has(_rank39)) rankMap.set(_rank39, []);
            rankMap.get(_rank39).push(i);
          }
          ...
          for (var mainRank of sortedRanks) {
            ...
            for (var subRank of sortedRanks) {
              ...
              var subIdxList = rankMap.get(subRank).filter(idx => !usedIndex.has(idx));
          ...
          for (var _i14 = 0; _i14 < unused.length; _i14++) {
            var _rank40 = getCardSize(unused[_i14]);

            if (_rank40 >= 3 && _rank40 <= 14 && !usedIndex.has(_i14)) {
              if (!rankToIndexes.has(_rank40)) rankToIndexes.set(_rank40, []);
              rankToIndexes.get(_rank40).push(_i14);
            }
          }
          ...
          for (var _i15 = 0; _i15 < sorted.length; _i15++) {
            var r = sorted[_i15];
        ...
        function findStraight(cards) {
          ...
          for (var card of cards) {
            var _rank41 = getCardSize(card);

            if (_rank41 < 3 || _rank41 > 14) continue; // 3~A

            if (!map.has(_rank41)) map.set(_rank41, []);
            map.get(_rank41).push(card);
          }
          ...
          while (i <= ranks.length - 5) {
            ...
            if (length == 5) {
              ...
              for (var _rank42 of seq) {
                var cardsOfRank = map.get(_rank42);
                straight.push(cardsOfRank.pop());
        ...
        function isSamePointWithFrp(group, levelRank) {
          ...
          for (var card of group) {
            ...
            var color = Math.floor(card / 16);
            var isFrp = _rank44 === levelRank && color === 2;
          ...
          switch (type) {
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i16 = groupedCards.length - 1; _i16 >= 0; _i16--) {
                var _group9 = groupedCards[_i16];

                if (_group9.length === 2) {
                  var _card3 = _group9[0];
              ...
              for (var _i17 = groupedCards.length - 1; _i17 >= 0; _i17--) {
                var _group10 = groupedCards[_i17];

                if (_group10.length === 3) {
              ...
              for (var _i18 = groupedCards.length - 1; _i18 >= 0; _i18--) {
                var _group11 = groupedCards[_i18];

                if (_group11.length == 4) {
                  var bombs = GameLogic.newfindBombs(_group11);
              ...
              for (var _i19 = result_three.length - 1; _i19 >= 0; _i19--) {
                var _group12 = result_three[_i19]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);

                if (_group12.length != 0) {
              ...
              for (var _i20 = result.length - 1; _i20 >= 0; _i20--) {
                var _group13 = result[_i20]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);
              ...
              for (var _i21 = sameShunzi.length - 1; _i21 >= 0; _i21--) {
                var _group14 = sameShunzi[_i21];
              ...
              for (var _i22 = groupedCards.length - 1; _i22 >= 0; _i22--) {
                var _group15 = groupedCards[_i22];

                if (_group15.length == 4) {
                  var _bombs = GameLogic.findRocket(_group15);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i23 = groupedCards.length - 1; _i23 >= 0; _i23--) {
                var _group16 = groupedCards[_i23];

                if (_group16.length === 2) {
                  var _card5 = _group16[0];
              ...
              for (var _i24 = groupedCards.length - 1; _i24 >= 0; _i24--) {
                var _group17 = groupedCards[_i24];
              ...
              for (var _i25 = groupedCards.length - 1; _i25 >= 0; _i25--) {
                var _group18 = groupedCards[_i25];

                if (_group18.length == 4) {
                  var _bombs2 = GameLogic.newfindBombs(_group18);
              ...
              for (var _i26 = result_three1.length - 1; _i26 >= 0; _i26--) {
                var _group19 = result_three1[_i26]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);

                if (_group19.length != 0) {
              ...
              for (var _i27 = result1.length - 1; _i27 >= 0; _i27--) {
                var _group20 = result1[_i27]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);
              ...
              for (var _i28 = sameShunzi1.length - 1; _i28 >= 0; _i28--) {
                var _group21 = sameShunzi1[_i28];
              ...
              for (var _i29 = groupedCards.length - 1; _i29 >= 0; _i29--) {
                var _group22 = groupedCards[_i29];

                if (_group22.length == 4) {
                  var _bombs3 = GameLogic.findRocket(_group22);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_3:
              {
                ...
                for (var _i30 = groupedCards.length - 1; _i30 >= 0; _i30--) {
                  var _group23 = groupedCards[_i30];

                  if (_group23.length === 3) {
                    var size = GameLogic.getCardSize(_group23[0]);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              {
                // 三带二：找出三张+一对，三张部分要大
                for (var _i31 = groupedCards.length - 1; _i31 >= 0; _i31--) {
                  var group3 = groupedCards[_i31];
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              // ✅ 找更大的炸弹
              for (var _i32 = groupedCards.length - 1; _i32 >= 0; _i32--) {
                var _group24 = groupedCards[_i32];

                if (_group24.length >= 4) {
                  // ✅ 点数必须全部一致，才是炸弹
                  var isBomb = isSamePointWithFrp(_group24, (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
                  }), GlobalData) : GlobalData).cardInfo.levelCard);
                  if (!isBomb) continue;
                  var groupRank = GameLogic.getCardSize(_group24[0]);

                  if (_group24.length > targetCards.length || _group24.length === targetCards.length && groupRank > targetMainSize) {
              ...
              for (var _card7 of groupedCards.flat()) {
                var color = GameLogic.getCardColor(_card7);
                if (!colorGroups.has(color)) colorGroups.set(color, []);
                colorGroups.get(color).push(_card7);
              }
              ...
              for (var cards of colorGroups.values()) {
                ...
                for (var _i33 = 0; _i33 <= sizes.length - 5; _i33++) {
                  var ok = true;

                  for (var _j2 = 1; _j2 < 5; _j2++) {
                    if (sizes[_i33 + _j2] !== sizes[_i33 + _j2 - 1] + 1) {
                      ok = false;
                      break;
                    }
                  }
                  ...
                  if (ok) {
                    ...
                    for (var _j3 = 0; _j3 < 5; _j3++) {
                      var _s = sizes[_i33 + _j3];
                      var list = sizeMap.get(_s);

                      _group25.push(list.pop()); // 拿一张即可
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i34 = groupedCards.length - 1; _i34 >= 0; _i34--) {
                var _group26 = groupedCards[_i34];

                if (_group26.length >= 6) {
                  if (!isSamePointWithFrp(_group26, (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
                  }), GlobalData) : GlobalData).cardInfo.levelCard)) continue;

                  var _groupRank = GameLogic.getCardSize(_group26[0]);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_1:
              {
                ...
                for (var _card8 of singles) {
                  var _size2 = GameLogic.getCardSize(_card8);

                  if (!rankMap.has(_size2)) rankMap.set(_size2, []);
                  rankMap.get(_size2).push(_card8);
                }

                var allRanks = Array.from(rankMap.keys()).sort((a, b) => a - b);

                var _loop4 = function _loop4(_i35) {
                  var seq = allRanks.slice(_i35, _i35 + targetLength);
                  ...
                  if (isContinuous && seq[0] > targetStart) {
                    ...
                    for (var r of seq) {
                      _group27.push(rankMap.get(r).pop());
                    }
                ...
                for (var _i35 = 0; _i35 <= allRanks.length - targetLength; _i35++) {
                  var _ret4 = _loop4(_i35);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_COLOR:
              {
                ...
                for (var _card9 of _singles) {
                  var _size3 = GameLogic.getCardSize(_card9);

                  if (!_rankMap.has(_size3)) _rankMap.set(_size3, []);

                  _rankMap.get(_size3).push(_card9);
                }

                var _allRanks = Array.from(_rankMap.keys()).sort((a, b) => a - b);

                var _loop5 = function _loop5(_i36) {
                  var seq = _allRanks.slice(_i36, _i36 + _targetLength);
                  ...
                  if (isContinuous && seq[0] > _targetStart) {
                    ...
                    for (var r of seq) {
                      _group28.push(_rankMap.get(r).pop());
                    }
                ...
                for (var _i36 = 0; _i36 <= _allRanks.length - _targetLength; _i36++) {
                  var _ret5 = _loop5(_i36);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_3:
              {
                ...
                for (var _i37 = 0; _i37 <= tripletGroups.length - _targetLength2 / 3; _i37++) {
                  var _group29 = [];
                  var _ok = true;

                  for (var _j4 = 0; _j4 < _targetLength2 / 3; _j4++) {
                    var cur = tripletGroups[_i37 + _j4];

                    if (_j4 > 0 && cur.rank !== tripletGroups[_i37 + _j4 - 1].rank + 1) {
                      _ok = false;
                      break;
                    }

                    _group29.push(...cur.cards);
                  }

                  if (_ok && tripletGroups[_i37].rank > targetMin) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_2:
              {
                ...
                for (var _i38 = 0; _i38 <= pairGroups.length - _targetLength3 / 2; _i38++) {
                  var _group30 = [];
                  var _ok2 = true;

                  for (var _j5 = 0; _j5 < _targetLength3 / 2; _j5++) {
                    var _cur = pairGroups[_i38 + _j5];

                    if (_j5 > 0 && _cur.rank !== pairGroups[_i38 + _j5 - 1].rank + 1) {
                      _ok2 = false;
                      break;
                    }

                    _group30.push(..._cur.cards);
                  }

                  if (_ok2 && pairGroups[_i38].rank > _targetMin) {
          ...
          if (targetType === (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            if (hintList.length === 0) {
              for (var _i39 = groupedCards.length - 1; _i39 >= 0; _i39--) {
                var _group31 = groupedCards[_i39];

                if (_group31.length === 2) {
                  var card = _group31[0];
            ...
            if (hintList.length === 0) {
              for (var _i40 = groupedCards.length - 1; _i40 >= 0; _i40--) {
                var _group32 = groupedCards[_i40];

                if (_group32.length === 3) {
                  var _card10 = _group32[0];

                  if (isSingleCardStronger(_card10, targetCards[0], (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
                  }), GlobalData) : GlobalData).cardInfo.levelCard)) {
                    hintList.push([_card10]);
                  }
                }
              }
            }
            ...
            if (hintList.length === 0) {
              for (var _i41 = groupedCards.length - 1; _i41 >= 0; _i41--) {
                var _group33 = groupedCards[_i41];

                if (_group33.length >= 4) {
                  var bombs = GameLogic.newfindBombs(_group33);

                  if (bombs.length != 0) {
                    hintList.push(_group33);
                  }
                }
              }
            }
            ...
            for (var _group34 of groupedCards) {
              if (_group34.length === 2 && GameLogic.getCardSize(_group34[0]) > targetRank) {
            ...
            for (var _group35 of groupedCards) {
              if (_group35.length === 3 && GameLogic.getCardSize(_group35[0]) > targetRank) {
            ...
            var _loop6 = function _loop6(_group36) {
              if (_group36.length === 3 && GameLogic.getCardSize(_group36[0]) > targetRank) {
                // 找一个对子作为带牌
                var pair = groupedCards.find(g => g.length === 2 && g !== _group36);

                if (pair) {
                  hintList.push([..._group36, ...pair]);
                }
              }
            };
        ...
        function findFlushStraightWithHeart(groups) {
          ...
          for (var [_color2, cards] of colorMap.entries()) {
            var rankToCards = new Map();

            for (var _card11 of cards) {
              var _rank45 = _card11 % 16;

              if (!rankToCards.has(_rank45)) rankToCards.set(_rank45, []);
              rankToCards.get(_rank45).push(_card11);
            }

            var _loop7 = function _loop7(seq) {
              var group = [];
              var usedRed = new Set();
              ...
              for (var r of seq) {
                ...
                var _card12 = list.find(c => !usedCard.has(c));

                if (_card12 !== undefined) {
                  group.push(_card12);
                  usedCard.add(_card12);
                } else {
                  // 多红心级牌补牌逻辑
        ...
        function findBombsWithHeartSupportI(groups) {
          ...
          var _loop8 = function _loop8(group) {
        ...
        function findBombsWithHeartSupport(groups) {
          ...
          var _loop9 = function _loop9(group) {
            var rank = getCardRank_I(group[0]);
            var sameRank = group.every(c => getCardRank_I(c) === rank);
            if (!sameRank) return "continue";
            var groupLen = group.length; // 已经是炸弹
        ...
        function newfindBombs(cards, levelRank) {
          ...
          for (var card of cards) {
            ...
            if (!countMap.has(_rank46)) countMap.set(_rank46, []);
            countMap.get(_rank46).push(card);
          } // 红心级牌（逢人配）
          ...
          for (var _rank47 of sortedRanks) {
            var group = countMap.get(_rank47);
...
//# sourceMappingURL=c6b3ea29180b0b1292d5b6cd5ab230c4cd802e92.js.map