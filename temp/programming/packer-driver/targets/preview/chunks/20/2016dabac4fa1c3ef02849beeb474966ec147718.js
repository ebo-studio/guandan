      ...
      (function (_GameLogic) {
        ...
        function autoSortCards(cards) {
          ...
          var getRemain = function getRemain(isRandom) {
            ...
            return cards.filter(c => isRandom ? !usedForRandom.has(c) : !used.has(c)); // 判断是否已经用过
          }; // ✅ 固定牌型优先处理
          ...
          var randomOptions = ['feiji', // 飞机
          'liandui', // 连对
          'shunzi', // 顺子
          'sanzhang' // 三带二
          ]; // 随机打乱随机牌型顺序

          shuffleArray(randomOptions); // ✅ 随机牌型放最后：顺子、连对、飞机、三带二（每次点击时随机选一个）

          for (var option of randomOptions) {
            if (option === 'feiji') {
              var feiji = shuffleArray(findFeiji(getRemain(true))); // 飞机

              if (feiji.length > 0) {
                addGroup(feiji, false, true);
                break; // 每次只选一组
              }
            } else if (option === 'liandui') {
              var liandui = shuffleArray(findLiandui(getRemain(true))); // 连对

              if (liandui.length > 0) {
                addGroup(liandui, false, true);
                break; // 每次只选一组
              }
            } else if (option === 'shunzi') {
              var shunzi = shuffleArray(findStraightByCard(getRemain(true))); // 顺子

              if (shunzi.length > 0) {
                addGroup(shunzi, false, true);
                break; // 每次只选一组
              }
            } else if (option === 'sanzhang') {
              var sanzhang = shuffleArray(findThreeWithTwo(getRemain(true))); // 三带二
          ...
          var specials = cards.filter(c => !used.has(c) && (getCardSize(c) === 16 || getCardSize(c) === 17));

          for (var c of specials) {
            result.push([c]);
            used.add(c);
          } // ✅ 剩下的才是对子 + 单张
...
//# sourceMappingURL=2016dabac4fa1c3ef02849beeb474966ec147718.js.map