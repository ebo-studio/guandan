    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function getHintCards(targetCards, groupedCards) {
          ...
          var type = GameLogic.getCardType(targetCards);
          var targetMainSize = getMainCardSize(targetCards);
          var levelRank = (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            error: Error()
          }), GlobalData) : GlobalData).cardInfo.levelCard; // 当前级牌，比如 2

          function isCardBiggerThanTarget(card, targetSize) {
            var rank = GameLogic.getCardSize(card); // 对方是级牌：只能用王（或炸弹，这里不考虑炸弹）

            if (targetSize === levelRank) {
              return isKing(card); // 大王/小王
            } // 我方是级牌，且对方不是


            if (rank === levelRank && targetSize !== levelRank) return true; // 普通大小比较
          ...
          switch (type) {
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var i = groupedCards.length - 1; i >= 0; i--) {
                ...
                if (group.length === 1) {
                  ...
                  if (isCardBiggerThanTarget(card, targetMainSize)) {
                    hintList.push([card]);
                  }
                }
              } // 没找到，再从对子中拆一张
              ...
              if (hintList.length === 0) {
                for (var _i8 = groupedCards.length - 1; _i8 >= 0; _i8--) {
                  ...
                  if (_group4.length === 2) {
                    ...
                    if (isCardBiggerThanTarget(_card3, targetMainSize)) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i9 = this.groupedCards.length - 1; _i9 >= 0; _i9--) {
                var _group5 = this.groupedCards[_i9];

                if (_group5.length === 2) {
                  var rank = GameLogic.getCardSize(_group5[0]);

                  if (isCardBiggerThanTarget(_group5[0], targetMainSize)) {
...
//# sourceMappingURL=aff5cb3e05b070fca185fa5b68825d5152c3398a.js.map