    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          var regroupSet = new Set(); // ✅ 步骤0：撤回处理（如果选中的牌在某组中，整个组被移出理牌）

          var preservedGroups = [];

          for (var group of prevGrouped) {
            var overlap = group.some(c => selectedSet.has(c));
          ...
          var regroupCards = cards.filter(c => regroupSet.has(c) || !prevGrouped.flat().includes(c)); // ✅ 步骤1：识别三带二
          ...
          var rest = selected.filter(c => !usedCards.has(c));
          var feiji = findFeiji(rest);
          var shunzi = findStraightByCard(rest.filter(c => !feiji.flat().includes(c)));
          var liandui = findLiandui(rest.filter(c => !feiji.flat().includes(c) && !shunzi.flat().includes(c)));
          ...
          for (var _g of shunzi) {
            moveToBack.push(_g);

            _g.forEach(c => usedCards.add(c));
          }

          for (var _g2 of liandui) {
            moveToBack.push(_g2);

            _g2.forEach(c => usedCards.add(c));
          } // ✅ 步骤3：剩余牌重新分组


          var remaining = regroupCards.filter(c => !usedCards.has(c));
...
//# sourceMappingURL=afb3ba04f01b631f6817549674697dfc07d1e7bd.js.map