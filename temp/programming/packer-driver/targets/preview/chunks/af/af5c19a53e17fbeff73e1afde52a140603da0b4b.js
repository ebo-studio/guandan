    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          var selectedSet = new Set(selectedHistory); // 2. 从历史中判断是否有三带二

          var map = getCardCountMap(selectedHistory); // Map<点数, number[]>
          ...
          var usedRanks = new Set(); // 用点数标记组合过的

          if (triple.length === 3 && pair.length === 2) {
            var group = [...triple, ...pair];
            moveToBack.push(group);
            group.forEach(c => usedRanks.add(getCardSize(c)));
          } // 3. 遍历 cards 进行重新分组（跳过组合的点数）
          ...
          for (var j = 0; j < uniqueList.length; j++) {
            ...
            for (var i = idx; i < sortValue.length; i++) {
              ...
              if (usedRanks.has(getCardSize(card))) continue; // ✅ 跳过组合过的点数
...
//# sourceMappingURL=af5c19a53e17fbeff73e1afde52a140603da0b4b.js.map