    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          var prevSet = new Set(prevSelected); // 找出这次新选中的牌（不在上一次中的）

          var newSelected = selected.filter(card => !prevSet.has(card)); // 剩余未选中的牌

          var remain = cards.filter(card => !selectedSet.has(card)); // 保持每次理牌顺序追加：上次的 + 新的
...
//# sourceMappingURL=81c6884990cbbc16bcde49c0556e27ebff2774d2.js.map