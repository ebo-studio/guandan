    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function checkIfSameColorOrFengRenPei(straight) {
          // 检查是否为同花顺
          function isSameColorStraight(straight) {
          ...
          var isSameColor = isSameColorStraight(straight); // const firstCardColor = straight[0] % 4;
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          for (var i = 0; i < cards.length; i++) {
            var _c4 = cards[i];
            var count = selectedCardCount.get(_c4) || 0;

            if (count > 0) {
              selectedIndexSet.add(i);
              selectedCardCount.set(_c4, count - 1);
            }
          ...
          for (var group of prevGrouped) {
            ...
            for (var _c5 of group) {
              groupCount.set(_c5, (groupCount.get(_c5) || 0) + 1);
            } // 统计 selected 中包含的数量


            var isGroupSelected = false;

            var _loop3 = function _loop3(card, _count) {
              var selectedCount = selected.filter(c => c === card).length;
            ...
            for (var [card, _count] of groupCount.entries()) {
              var _ret3 = _loop3(card, _count);

              if (_ret3 === "continue") continue;
              if (_ret3 === "break") break;
            } // 若该 group 没被完整重新选中，保留到 result 中


            if (!group.some(c => selected.includes(c))) {
              result.push(group);
              group.forEach(c => usedCards.add(c));
            }
          }
          ...
          if (bomsKing.length != 0) {
            for (var rocket of bomsKing) {
              ...
              for (var _i9 = 0; _i9 < cards.length; _i9++) {
                if (rocket.includes(cards[_i9])) usedIndex.add(_i9);
              }
            }
          }
          ...
          for (var _group7 of triplets) {
            moveToBack.push(_group7); // for (let i = 0; i < cards.length; i++) {
            //     if (group.includes(cards[i])) usedIndex.add(i);
            // }

            markUsedByCardValues(cards, _group7, usedIndex); // group.forEach(c => usedCards.add(c));
          ...
          if (selected.length == 5) {
            ...
            if (map && map.length == 5) {
              moveToBack.push(map); // for (let i = 0; i < cards.length; i++) {
          ...
          for (var s of straights) {
            ...
            markUsedByCardValues(cards, s, usedIndex); // for (let i = 0; i < cards.length; i++) {
          ...
          for (var _group8 of lianduiGroups) {
            moveToBack.push(_group8); // for (let i = 0; i < cards.length; i++) {
            //     if (group.includes(cards[i])) usedIndex.add(i);
            // }
            // group.forEach(c => usedCards.add(c));

            markUsedByCardValues(cards, _group8, usedIndex);
          } // ✅ 步骤7：识别两对
          ...
          // const selectedCardCount = new Map<number, number>(); // 支持多张相同牌
          // ✅ 获取未使用的牌并按牌力降序排序
          ...
          }).sort((a, b) => getCardSize(b.c) - getCardSize(a.c)); // ✅ 对未用牌按点数分组（保留 index）
          ...
          for (var {
            ...
            var _rank5 = getCardSize(_c6);

            if (!rankMap.has(_rank5)) rankMap.set(_rank5, []);
            rankMap.get(_rank5).push(idx);
          } // ✅ 分别将选中的分组放末尾，未选中的放前面
          // for (const indexes of rankMap.values()) {
          //     const group = indexes.map(i => cards[i]);
          //     const hasSelected = indexes.some(i => selectedIndexSet.has(i));
          //     if (hasSelected) {
          //         moveToBack.push(group);
          //     } else {
          //         result.push(group);
          ...
          for (var indexes of rankMap.values()) {
            var selectedGroup = [];
            var unselectedGroup = [];

            for (var _i10 of indexes) {
              if (selectedIndexSet.has(_i10)) {
                selectedGroup.push(cards[_i10]);
              } else {
                unselectedGroup.push(cards[_i10]);
              }

              usedIndex.add(_i10); // 不管是否选中，都标记已使用
            }
        ...
        function findBombsWithHeartCard(cards) {
          ...
          var _loop4 = function _loop4(rank, list) {
        ...
        function getCardTypeByFeiji(cards) {
          ...
          var _loop5 = function _loop5(card) {
            var rank = getCardSize(card);
        ...
        function getOneCardSizeList(value) {
          ...
          for (var _i11 = 0; _i11 < pairs.length; _i11++) {
            if (temp.length === 0) {
              temp.push(pairs[_i11]);
            } else {
              var last = getCardSize(temp[temp.length - 1][0]);
              var current = getCardSize(pairs[_i11][0]);
          ...
          var findStraightGroups = cards => {
            ...
            for (var _i12 = 0; _i12 < unique.length; _i12++) {
              var cur = unique[_i12];
        ...
        function getGroupPower(cards) {
          ...
          if (size >= 4 && size % 2 === 0) {
            ...
            for (var _i13 = 1; _i13 < pairs.length; _i13++) {
              if (pairs[_i13] !== pairs[_i13 - 1] + 1 || pairs[_i13] >= 15) return 0;
            }
            ...
            for (var start = 3; start <= 10; start++) {
              ...
              if (missing === 0) {
                ...
                if (frp !== undefined) {
                  ...
                  // 恢复
                  for (var [_r6, _c7] of tempPopped) {
                    if (!rankMap.has(_r6)) rankMap.set(_r6, []);
                    rankMap.get(_r6).push(_c7);
                  }
                }
              } else {
                // 恢复
                for (var [_r7, _c8] of tempPopped) {
                  if (!rankMap.has(_r7)) rankMap.set(_r7, []);
                  rankMap.get(_r7).push(_c8);
                }
              }
            }
          }
        ...
        function findStraight_II(cards, usedIndex) {
          ...
          for (var _i14 = 0; _i14 < cards.length; _i14++) {
            if (usedIndex.has(_i14)) continue;
            var card = cards[_i14];
            var rank = card % 16;
            if (rank < 3 || rank > 13) continue; // 只允许 3~K 进入顺子

            if (!rankMap.has(rank)) rankMap.set(rank, []);
            rankMap.get(rank).push(_i14); // 保存的是索引，方便精确控制使用
          }
        ...
        function countThreeWithTwoAndStraight(unused) {
          ...
          for (var _i15 = 0; _i15 < unused.length; _i15++) {
            var _rank12 = getCardSize(unused[_i15]);

            if (_rank12 >= 3 && _rank12 <= 14 && !usedIndex.has(_i15)) {
              if (!rankToIndexes.has(_rank12)) rankToIndexes.set(_rank12, []);
              rankToIndexes.get(_rank12).push(_i15);
            }
          }
          ...
          for (var _i16 = 0; _i16 < sorted.length; _i16++) {
            var r = sorted[_i16];
          ...
          switch (type) {
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i18 = groupedCards.length - 1; _i18 >= 0; _i18--) {
                var _group10 = groupedCards[_i18];
              ...
              for (var _i19 = groupedCards.length - 1; _i19 >= 0; _i19--) {
                var _group11 = groupedCards[_i19];
              ...
              for (var _i20 = result_three.length - 1; _i20 >= 0; _i20--) {
                var _group12 = result_three[_i20]; // if (group.length == 4) {
              ...
              for (var _i21 = result.length - 1; _i21 >= 0; _i21--) {
                var _group13 = result[_i21]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);
              ...
              for (var _i22 = sameShunzi.length - 1; _i22 >= 0; _i22--) {
                var _group14 = sameShunzi[_i22];
              ...
              for (var _i23 = groupedCards.length - 1; _i23 >= 0; _i23--) {
                var _group15 = groupedCards[_i23];
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i24 = groupedCards.length - 1; _i24 >= 0; _i24--) {
                var _group16 = groupedCards[_i24];
              ...
              for (var _i25 = groupedCards.length - 1; _i25 >= 0; _i25--) {
                var _group17 = groupedCards[_i25];
              ...
              for (var _i26 = groupedCards.length - 1; _i26 >= 0; _i26--) {
                var _group18 = groupedCards[_i26];
              ...
              for (var _i27 = result_three1.length - 1; _i27 >= 0; _i27--) {
                var _group19 = result_three1[_i27]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);
              ...
              for (var _i28 = result1.length - 1; _i28 >= 0; _i28--) {
                var _group20 = result1[_i28]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);
              ...
              for (var _i29 = sameShunzi1.length - 1; _i29 >= 0; _i29--) {
                var _group21 = sameShunzi1[_i29];
              ...
              for (var _i30 = groupedCards.length - 1; _i30 >= 0; _i30--) {
                var _group22 = groupedCards[_i30];
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i33 = groupedCards.length - 1; _i33 >= 0; _i33--) {
                var _group24 = groupedCards[_i33];
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i35 = groupedCards.length - 1; _i35 >= 0; _i35--) {
                var _group26 = groupedCards[_i35];
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_1:
              {
                ...
                var _loop7 = function _loop7(_i36) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_COLOR:
              {
                ...
                var _loop8 = function _loop8(_i37) {
          ...
          if (targetType === (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            if (hintList.length === 0) {
              for (var _i40 = groupedCards.length - 1; _i40 >= 0; _i40--) {
                var _group31 = groupedCards[_i40];
            ...
            if (hintList.length === 0) {
              for (var _i41 = groupedCards.length - 1; _i41 >= 0; _i41--) {
                var _group32 = groupedCards[_i41];

                if (_group32.length === 3) {
                  var _card11 = _group32[0];
            ...
            if (hintList.length === 0) {
              for (var _i42 = groupedCards.length - 1; _i42 >= 0; _i42--) {
                var _group33 = groupedCards[_i42];
            ...
            var _loop9 = function _loop9(_group36) {
        ...
        function findFlushStraightWithHeart(groups) {
          ...
          for (var [_color3, cards] of colorMap.entries()) {
            ...
            var _loop10 = function _loop10(seq) {
              var group = [];
              var usedRed = new Set();
        ...
        function findBombsWithHeartSupport(groups) {
          ...
          var _loop12 = function _loop12(group) {
            var rank = getCardRank_I(group[0]);
            var sameRank = group.every(c => getCardRank_I(c) === rank);
            if (!sameRank) return "continue";
            var groupLen = group.length; // 已经是炸弹
...
//# sourceMappingURL=2f656fcd81a078840bd7d6b2315face8fbd13e3c.js.map