    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected) {
          ...
          for (var i = 0; i < cards.length; i++) {
            var v = cards[i];
            var index = selectedCopy.indexOf(v);

            if (index !== -1) {
              selectedCopy.splice(index, 1); // 已匹配的选中牌删掉一张
            } else {
              remainCards.push(v); // 其余留下
            }
          }

          return remainCards.concat(selected); // 拼到后面
        }
...
//# sourceMappingURL=2f3f688d302db647535f4eac24339c9d7e2c16ae.js.map