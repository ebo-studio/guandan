    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          var getCardSize = card => card % 16; // 先从 cards 中过滤出未被使用的牌


          var unusedCards = cards.map((c, i) => ({
            card: c,
            idx: i
          })).filter(_ref => {
            var {
          ...
          for (var item of unusedCards) {
            var size = getCardSize(item.card);
          ...
          for (var [_size, group] of sizeMap.entries()) {
            if (group.length >= 3) {
              // 按 idx 排序保证稳定性
              var sorted = group.slice().sort((a, b) => a.idx - b.idx);
              threes.push({
                main: sorted.slice(0, 3),
                size: _size
              });
            }
          } // 将三张按 size 从小到大排序（点数小的优先）


          threes.sort((a, b) => a.size - b.size); // 再找对子（排除已被选中用于三张的牌）

          var usedTemp = new Set();

          for (var {
            main
          } of threes) {
            main.forEach(x => usedTemp.add(x.idx));
          }

          for (var [_size2, _group4] of sizeMap.entries()) {
            var candidates = _group4.filter(x => !usedTemp.has(x.idx));

            if (candidates.length >= 2) {
              var _sorted = candidates.slice().sort((a, b) => a.idx - b.idx);

              pairs.push(_sorted.slice(0, 2));
            }
          } // 按照点数从小到大排列对子


          pairs.sort((a, b) => getCardSize(a[0].card) - getCardSize(b[0].card));
          var result = [];
          var usedFinal = new Set();

          for (var {
            main: _main
          } of threes) {
            var pair = pairs.find(p => !usedFinal.has(p[0].idx) && !usedFinal.has(p[1].idx));

            if (pair) {
              var fullGroup = [..._main, ...pair];
              fullGroup.forEach(x => usedIndex.add(x.idx));
              fullGroup.forEach(x => usedFinal.add(x.idx));
              result.push(fullGroup.map(x => x.card));
            }
          }
...
//# sourceMappingURL=52434046ebc6cdf7c6910e870490fb19326372d0.js.map