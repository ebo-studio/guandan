    ...
    execute: function () {
      ...
      _export("CardLayer", CardLayer = (_dec = ccclass('CardLayer'), _dec2 = property(Prefab), _dec3 = property(SpriteAtlas), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(_crd && GameTimer === void 0 ? (_reportPossibleCrUseOfGameTimer({
        ...
        onScreenTouchStart(event) {
          ...
          this.dragProcessedSet.clear();
          this.isToggling = false; // this.startPos = this.getLocalPos(event);
          // ✅ 判断当前点击区域是否是“旧区域”（选中过的）

          var pos = this.getLocalPos(event);
          ...
          for (var _i = this.handCards.length - 1; _i >= 0; _i--) {
            ...
            if (_card5.node && _card5.node.getComponent(UITransform).getBoundingBoxToWorld().contains(touchPos)) {
              // if (GlobalData.cardInfo.cardDir) {
              //     card.setMask(true);
              // } else {
              if (_card5.isMask()) {
                _card5.setMask(false);
              } else {
                _card5.setMask(true);
              } // }
          ...
          for (var i = this.handCards.length - 1; i >= 0; i--) {
            ...
            if (item != null) {
              ...
              if (leftUp || leftDown || rightUp || rightDown) {
                this.dragProcessedSet.add(i);

                if (this.isToggling) {
                  // ✅ 切换遮罩
                  var currentlySelected = this.selectedIndexSet.has(i);
                  this.handCards[i].setMask(!currentlySelected);
              ...
              } else {// if(!this.handCards[i].isSelect) {
                // this.handCards[i].setMask(false);
                // }
                // else {
                // }
...
//# sourceMappingURL=33ff511637dc8e14ad039da107eac91724f902a2.js.map