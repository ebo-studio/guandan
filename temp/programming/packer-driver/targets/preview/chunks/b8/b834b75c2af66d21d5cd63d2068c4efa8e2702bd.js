        ...
        setHandCards(value, ani, isUp, isOneCard) {
          ...
          if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              ...
              var _posY = this.getHandCardPosY();

              this.initHandStartPosX(); // 计算起始位置

              for (var _i = 0; _i < value.length; _i++) {
                var value1 = value[_i];
                var card = this.getOneCard();
                card.node.setSiblingIndex(_i);
                var posX = this.getHandCardPosX(_i);
                card.node.setPosition(posX, _posY);
                card.node.scale = v3(this.handScale, this.handScale, this.handScale);
                var cardSps = this.getCardsSprite(value1);

                if (cardSps) {
                  card.setValue(value1, cardSps);
                }
          ...
          if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            for (var _i5 = this.handCards.length - 1; _i5 >= 0; _i5--) {
              ...
              if (_item != null) {
                ...
                var _posY2 = _item.position.y + _item.getComponent(UITransform).contentSize.height / 2 * this.handScale; //上边界点
                ...
                var tmpPosY = this.handCards[_i5].getBottom() ? _posyYB : _posY2 - this.cardDistance_V; //下边界点
                //向左 向上  当前点纵坐标>下边界点  当前点横坐标>右边界点   牌的上边界点>起始点纵坐标    牌的左边界点<起始点横坐标  

                var leftUp = localStartPos.x >= localMovePos.x && localMovePos.y >= localStartPos.y && localMovePos.y >= tmpPosY && localMovePos.x <= tmpPosXR && _posY2 >= localStartPos.y && posXL <= localStartPos.x; //向左向下

                var leftDown = localStartPos.x >= localMovePos.x && localMovePos.y <= localStartPos.y && localMovePos.y <= _posY2 && localMovePos.x <= tmpPosXR && tmpPosY <= localStartPos.y && posXL <= localStartPos.x; //向右向上

                var rightUp = localStartPos.x <= localMovePos.x && localMovePos.y >= localStartPos.y && localMovePos.y >= tmpPosY && localMovePos.x >= posXL && _posY2 >= localStartPos.y && tmpPosXR >= localStartPos.x; //向右向下

                var rightDown = localStartPos.x <= localMovePos.x && localMovePos.y <= localStartPos.y && localMovePos.y <= _posY2 && localMovePos.x >= posXL && tmpPosY <= localStartPos.y && tmpPosXR >= localStartPos.x;
...
//# sourceMappingURL=b834b75c2af66d21d5cd63d2068c4efa8e2702bd.js.map