    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          var prevSet = new Set(prevSelected); // 获取本次新增选中的牌（diff）

          var newSelected = selected.filter(card => !prevSet.has(card));
          var newSelectedCopy = (_crd && utils === void 0 ? (_reportPossibleCrUseOfutils({
            error: Error()
          }), utils) : utils).deepCopy(newSelected);
          var remain = []; // 从原始手牌中剔除这次新增的牌

          for (var i = 0; i < cards.length; i++) {
            var v = cards[i];
            var idx = newSelectedCopy.indexOf(v);

            if (idx !== -1) {
              newSelectedCopy.splice(idx, 1);
            } else {
              remain.push(v);
            }
          } // 按照 remain 原顺序分组（点数相同连续）
          ...
          if (temp.length > 0) groupList.push(temp); // 将新选中的牌按 cards 原顺序提取出来作为一组

          var orderedNewGroup = [];
          var tempNew = [...newSelected];

          for (var card of cards) {
            var _idx = tempNew.indexOf(card);

            if (_idx !== -1) {
              orderedNewGroup.push(card);
              tempNew.splice(_idx, 1);
            }
          } // 加到最后


          if (orderedNewGroup.length > 0) {
            groupList.push(orderedNewGroup);
          }
...
//# sourceMappingURL=d38e26824afc46c209a8a38c388229b722545fbb.js.map