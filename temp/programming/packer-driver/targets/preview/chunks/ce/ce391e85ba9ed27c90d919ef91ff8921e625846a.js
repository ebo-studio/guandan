    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          var cardsWithIndex = cards.map((card, index) => ({
            card,
            index
          ...
          for (var group of prevGrouped) {
            if (!group.some(c => selectedSet.has(c))) {
              result.push(group);
              group.forEach(c => {
                var idx = cards.findIndex((x, i) => x === c && !usedIndex.has(i));
                if (idx !== -1) usedIndex.add(idx);
              });
            }
          }

          var getUnusedCards = () => cardsWithIndex.filter(_ref => {
            var {
              index
            } = _ref;
            return !usedIndex.has(index);
          }).map(_ref2 => {
            var {
              card
            } = _ref2;
          ...
          var markUsed = group => {
            var _loop2 = function _loop2(card) {
              var idx = cards.findIndex((c, i) => c === card && !usedIndex.has(i));
              if (idx !== -1) usedIndex.add(idx);
            };
          ...
          var bomsKing = findRocket(getUnusedCards());
          ...
          var bombs = findBombsWithHeartCard(getUnusedCards());
          ...
          var triplets = findFeiji(getUnusedCards());
          ...
          if (selected.length === 5) {
            var map = getTripleWithPairPreferBig(getUnusedCards());
          ...
          var straights = findStraightByCard(getUnusedCards());

          var _loop3 = function _loop3(s) {
            var isSameColor = (() => {
              var first = s.find(c => !isHeartCard(c));
          ...
          var lianduiGroups = findLiandui(getUnusedCards());
          ...
          var countMap = getCardCountMap_I(getUnusedCards());

          for (var list of countMap.values()) {
          ...
          var finalRemain = cardsWithIndex.filter(_ref3 => {
            var {
              index
            } = _ref3;
            return !usedIndex.has(index);
          });
          var sortValue = sortCardsBySizeDown(finalRemain.map(obj => obj.card), finalRemain.length);
          var grouped = [];

          var _loop4 = function _loop4() {
            var first = sortValue[0];
            var rank = getCardSize(first);
            var group = sortValue.filter(c => getCardSize(c) === rank);
            grouped.push(group);
          ...
          for (var _group9 of grouped) {
            if (_group9.some(c => selectedSet.has(c))) {
        ...
        function findBombsWithHeartCard(cards) {
          ...
          var _loop5 = function _loop5(rank, list) {
        ...
        function getCardTypeByFeiji(cards) {
          ...
          var _loop6 = function _loop6(card) {
            var rank = getCardSize(card);
        ...
        function findThreeWithTwo(cards, usedIndex, maxCount) {
          ...
          var triples = Array.from(countMap.entries()).filter(_ref6 => {
            var [_, list] = _ref6;
            return list.length >= 3;
          }).sort((a, b) => a[0] - b[0]); // 最小的三张优先
          // 找所有对子（⚠️ 只找那些 list.length === 2，不能从三张拆）

          var pairs = Array.from(countMap.entries()).filter(_ref7 => {
            var [_, list] = _ref7;
            return list.length === 2;
          ...
          switch (type) {
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i18 = result_three.length - 1; _i18 >= 0; _i18--) {
                var _group13 = result_three[_i18]; // if (group.length == 4) {
              ...
              for (var _i19 = result.length - 1; _i19 >= 0; _i19--) {
                var _group14 = result[_i19]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i25 = result_three1.length - 1; _i25 >= 0; _i25--) {
                var _group20 = result_three1[_i25]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);
              ...
              for (var _i26 = result1.length - 1; _i26 >= 0; _i26--) {
                var _group21 = result1[_i26]; // if (group.length == 4) {
                // const bombs = newfindBombs(group);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_1:
              {
                ...
                var _loop8 = function _loop8(_i34) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_COLOR:
              {
                ...
                var _loop9 = function _loop9(_i35) {
        ...
        function getMainCardSize(cards) {
          ...
          var map = getCardCountMap(cards);
          var countArr = Array.from(map.entries()).map(_ref8 => {
            var [rank, list] = _ref8;
            return {
              rank,
              count: list.length
        ...
        function getHintList(handCards, targetCards) {
          ...
          if (targetType === (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            var _loop10 = function _loop10(_group37) {
        ...
        function findFlushStraightWithHeart(groups) {
          ...
          for (var [_color3, cards] of colorMap.entries()) {
            ...
            var _loop11 = function _loop11(seq) {
              var group = [];
              var usedRed = new Set();
        ...
        function findBombsWithHeartSupport(groups) {
          ...
          var _loop13 = function _loop13(group) {
            var rank = getCardRank_I(group[0]);
            var sameRank = group.every(c => getCardRank_I(c) === rank);
            if (!sameRank) return "continue";
            var groupLen = group.length; // 已经是炸弹
...
//# sourceMappingURL=ce391e85ba9ed27c90d919ef91ff8921e625846a.js.map