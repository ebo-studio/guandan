    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findBombsWithHeartCard(cards) {
          ...
          var bombs = [];
          var heartCards = cards.filter(card => isHeartCard(card)); // 查找三张及以上相同点数的牌，尝试组合炸弹（含逢人配）
          ...
          for (var [rank, list] of countMap) {
            ...
            if (list.length >= 3 && list.length < 6) {
              (function () {
                ...
                var usedSet = new Set(list);
                var availableHeartCards = heartCards.filter(card => !usedSet.has(card)); // 可组合成炸弹（最多补成6炸）

                for (var i = 0; i < Math.min(6 - list.length, availableHeartCards.length); i++) {
                  var bomb = [...list, ...availableHeartCards.slice(0, i + 1)];
                  bombs.push(bomb);
                }
              })();
            }
...
//# sourceMappingURL=1c88599ff9a5b78cd9d9f1e067e79afa3bc4b09d.js.map