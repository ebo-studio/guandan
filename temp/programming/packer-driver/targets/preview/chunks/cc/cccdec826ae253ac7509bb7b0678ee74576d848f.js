      ...
      (function (_GameLogic) {
        ...
        function findStraightByCard(cards) {
          ...
          for (var i = 0; i <= sortedRanks.length; i++) {
            var cur = sortedRanks[i];
            var last = tempRanks[tempRanks.length - 1];
          ...
          if (tempRanks.length >= 5) {
            ...
            for (var _r of tempRanks) {
              var _list = rankMap.get(_r);

              if (_list && _list.length > 0) {
                _straight.push(_list.shift());
              }
            }

            results.push(_straight);
          }
        ...
        function getCardType(cards) {
          ...
          if (cards.length >= 6 && cards.length % 2 === 0) {
            ...
            for (var [rank, _list2] of _map.entries()) {
          ...
          if (cards.length >= 6 && cards.length % 3 === 0) {
            ...
            for (var [_rank2, _list3] of _map2.entries()) {
        ...
        function autoSortCards(cards) {
          var fixedGroups = [];
          var randomGroups = [];
          var used = new Set(); // 1. 王炸

          var jokers = cards.filter(c => getCardSize(c) >= 16);

          if (jokers.length === 2) {
            fixedGroups.push(jokers);
            jokers.forEach(c => used.add(c));
          } // 2. 炸弹类（六炸到四炸）
          ...
          for (var list of map.values()) {
            if (list.length >= 4) {
              if (list.length === 6) bombGroups.push(list);else if (list.length === 5) bombGroups.push(list);else if (list.length === 4) bombGroups.push(list);
            }
          }

          bombGroups.sort((a, b) => b.length - a.length); // 六炸 > 五炸 > 四炸

          bombGroups.forEach(group => {
            fixedGroups.push(group);
            group.forEach(c => used.add(c));
          }); // 3. 对子

          for (var _list4 of map.values()) {
            if (_list4.length === 2 && !used.has(_list4[0])) {
              fixedGroups.push(_list4);

              _list4.forEach(c => used.add(c));
            }
          } // 4. 单牌


          for (var _list5 of map.values()) {
            var remain = _list5.filter(c => !used.has(c));

            if (remain.length === 1) {
              fixedGroups.push(remain);
              used.add(remain[0]);
            }
          } // 🔀 随机牌型部分：


          var remainCards = cards.filter(c => !used.has(c));
          var planes = findFeiji(remainCards);
          var lianduis = findLiandui(remainCards);
          var shunzis = findStraightByCard(remainCards);
          var threeWithTwo = findThreeWithTwo(remainCards);
          var randomPool = [...planes, ...lianduis, ...shunzis, ...threeWithTwo]; // 标记这些牌不再参与后续处理

          randomPool.forEach(group => group.forEach(c => used.add(c))); // 随机打乱随机牌型顺序

          var shuffled = shuffle(randomPool);
          return [...fixedGroups, ...shuffled];
        }
        ...
        function findThreeWithTwo(cards) {
          ...
          for (var triple of triples) {
            for (var pair of pairs) {
              var all = [...triple, ...pair];
              var allIds = all.map(c => c);

              if (allIds.every(c => !used.has(c))) {
                result.push(all);
                allIds.forEach(c => used.add(c));
                break; // 每组三带二只取一个组合，避免重复用对子
              }
            }
          }
...
//# sourceMappingURL=cccdec826ae253ac7509bb7b0678ee74576d848f.js.map