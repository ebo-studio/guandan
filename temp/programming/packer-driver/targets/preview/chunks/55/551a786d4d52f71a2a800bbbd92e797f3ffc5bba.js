    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards) {
          var map = getCardCountMap(cards);
          var threeList = [];
          var pairList = []; // 收集所有三张与对子

          for (var [rank, list] of map.entries()) {
            if (list.length >= 3) {
              threeList.push(list.slice(0, 3));
            } else if (list.length >= 2) {
              pairList.push(list.slice(0, 2));
            }
          }

          if (threeList.length === 0 || pairList.length === 0) return []; // 优先出三张点数最小的那一组

          threeList.sort((a, b) => getCardSize(a[0]) - getCardSize(b[0]));
          var selectedThree = threeList[0];
          var threeRank = getCardSize(selectedThree[0]); // 找一个不与三张重复的对子

          var selectedPair = pairList.find(p => getCardSize(p[0]) !== threeRank);
          if (!selectedPair) return [];
          return [[...selectedThree, ...selectedPair]];
        }
        ...
        function smartSortCards(cards) {
          ...
          var mainResult = []; // 主牌型：王炸、炸弹、同花顺、大王、小王、级牌

          var scatteredPart = []; // 散牌（单张、对子、三张）

          var randomPart = []; // 杂牌型（顺子、连对、飞机、三带二）
          ...
          findBombsByCount(getUnused(), 4).forEach(g => pushAndMark(g, mainResult)); // 2. 单张特例：大王、小王、级牌（从剩余中选）

          getUnused().forEach(c => {
            var rank = c % 16;

            if (rank === 15 || rank === 14 || rank === (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.levelCard) {
              pushAndMark([c], mainResult);
            }
          }); // 3. 杂牌型：顺子、飞机、连对、三带二（基于当前剩余牌识别）

          findPlane(getUnused()).forEach(g => pushAndMark(g, randomPart));
          findChainPairs(getUnused()).forEach(g => pushAndMark(g, randomPart));
          findStraight(getUnused()).forEach(g => pushAndMark(g, randomPart));
          findThreeWithTwo(getUnused()).forEach(g => pushAndMark(g, randomPart)); // 4. 散牌（单张、对子、三张）：剩余未用牌统一分组 + 排序

          var remain = getUnused();
          var scatteredGrouped = groupBySizeDown(remain);
          scatteredGrouped.forEach(g => scatteredPart.push(g)); // 5. 排序处理

          scatteredPart.sort((a, b) => maxRank(b) - maxRank(a)); // 散牌从大到小

          randomPart.sort((a, b) => minRank(a) - minRank(b)); // 杂牌从小到大
            ...
            for (var i = 0; i <= ranks.length - 5; i++) {
              ...
              if (isConsecutive) {
                ...
                for (var _rank3 of seq) {
                  straight.push(rankMap.get(_rank3).pop());
                  if (rankMap.get(_rank3).length === 0) rankMap.delete(_rank3);
                }
...
//# sourceMappingURL=551a786d4d52f71a2a800bbbd92e797f3ffc5bba.js.map