      ...
      (function (_GameLogic) {
        ...
        function findFeiji(cards) {
          ...
          for (var [_rank, list] of map.entries()) {
            if (list.length === 3) {
              rankToTriplet.set(_rank, [...list]);
            } else if (list.length === 2 && redLeft >= 1) {
              rankToTriplet.set(_rank, [...list, redCards[redCount - redLeft]]);
              redLeft--;
            }
          }
        ...
        function manualSortCards(selected, prevSelected, prevGrouped, allCards) {
          ...
          for (var [_rank2, list] of countMap.entries()) {
            if (list.length >= 6) {
              result.unshift(list);
          ...
          for (var [_rank3, _list] of rankMap.entries()) {
        ...
        function getCardTypeByFeiji(cards) {
          ...
          for (var _rank5 of sortedRanks) {
            var list = countMap.get(_rank5);

            if (list.length === 3) {
              tripletRanks.push(_rank5);
            } else if (list.length === 2 && redLeft >= 1) {
              tripletRanks.push(_rank5);
              redLeft -= 1;
        ...
        function getTripleWithPairPreferBig(cards) {
          ...
          for (var _rank6 of ranksSorted) {
            var list = rankMap.get(_rank6);
        ...
        function findFlushStraight(cards) {
          ...
          for (var [_color, colorCards] of colorMap.entries()) {
            ...
            for (var _card2 of colorCards) {
              var _rank9 = _card2 % 16;

              if (!rankMap.has(_rank9)) rankMap.set(_rank9, []);
              rankMap.get(_rank9).push(_card2);
            }
        ...
        function countThreeWithTwoAndStraight(unused) {
          ...
          for (var _i13 = 0; _i13 < unused.length; _i13++) {
            var _rank12 = getCardSize(unused[_i13]);

            if (_rank12 >= 3 && _rank12 <= 14 && !usedIndex.has(_i13)) {
              if (!rankToIndexes.has(_rank12)) rankToIndexes.set(_rank12, []);
              rankToIndexes.get(_rank12).push(_i13);
            }
          }
...
//# sourceMappingURL=0bdb31b996925869c55a3f9db223945cbf46895b.js.map