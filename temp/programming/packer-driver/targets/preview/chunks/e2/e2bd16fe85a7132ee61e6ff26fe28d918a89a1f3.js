      ...
      _export("CardLayer", CardLayer = (_dec = ccclass('CardLayer'), _dec2 = property(Prefab), _dec3 = property(SpriteAtlas), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(_crd && GameTimer === void 0 ? (_reportPossibleCrUseOfGameTimer({
        ...
        setHandCards(value, ani, isUp, isOneCard) {
          ...
          if (isOneCard) {
            ...
            if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              ...
              this.handCardsValue = sameSizeList.flat();
              console.log('理牌后的', this.handCardsValue);
              this.sortHandCardsValue = this.handCardsValue;
            } else {
              this.handCardsValue = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).sortCardsBySizeDown(this.handCardsValue, this.handCardsValue.length);
          ...
          if (viewid == (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            (function () {
              ...
              var tmpInfo = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).getRemainCardsByDelete(_this3.sortHandCardsValue, tempCards); // console.log('出牌后的', tmpInfo);

              _this3.handCardsValue = tmpInfo.cards; // console.log("+++> ", this.handCardsValue.length);
              ...
              if (isAuto) {
                ...
                // } else {
                _this3.doHandCardPopDown_V(); // }
                //找到下标


                _this3.selectCardIndex = tmpInfo.idxs;

                if (_this3.selectCardIndex.length <= 1) {
                  console.log("贡牌下标---> ", _this3.selectCardIndex[0]);
                }
              ...
              for (var j = 0; j < count; j++) {
                for (var k in _this3.handCards) {
                  if (_this3.handCards[k] && tempCards[j] == _this3.handCards[k].getValue() && _this3.selectCardIndex.indexOf(_this3.handCards[k].getIndex()) != -1) {
                    _this3.outCards[viewid].push(_this3.handCards[k]);

                    _this3.handCards.splice(Number(k), 1);
              ...
              console.log("出牌 cnt---> ", (_crd && utils === void 0 ? (_reportPossibleCrUseOfutils({
                error: Error()
              }), utils) : utils).deepCopy(_this3.outCards[viewid].length)); //出牌动作

              _this3.initOutStartPosX(_this3.outCards[viewid].length);

              for (var _k2 = 0; _k2 < _this3.outCards[viewid].length; _k2++) {
                var posX = _this3.getOutPosX(_k2);

                var card = _this3.outCards[viewid][_k2];
                card.setMask(false);
                card.node.setSiblingIndex(_k2);
                card.node.setScale(_this3.outScaleSelf, _this3.outScaleSelf, _this3.outScaleSelf);
              ...
              if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                ...
                if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                  ...
                  sameSizeList = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).moveSelectedCardsToBack(_this3.sortHandCardsValue, []);
                } else {
                  sameSizeList = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                ...
                var _loop6 = function _loop6(_j) {
                  ...
                  var _loop7 = function _loop7(n) {
                    ...
                    if (card.getValue() == valueSize && useIdxList.indexOf(card.getIndex()) == -1) {
                      ...
                      var posY = _this3.getHandCardPosY_V(_j);
        ...
        onScreenTouchMove(event) {
          ...
          for (var _i2 = this.handCards.length - 1; _i2 >= 0; _i2--) {
            var _card5 = this.handCards[_i2];

            if (_card5.node && _card5.node.getComponent(UITransform).getBoundingBoxToWorld().contains(movePos)) {
              _card5.setMask(true);
        ...
        onBtnCardDir() {
          ...
          // GlobalData.cardInfo.sortCard = !GlobalData.cardInfo.sortCard;
          this.showCardDir(true);
          ...
          var newCardList = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
            error: Error()
          }), GameLogic) : GameLogic).sortSelectedCardsToBack(this.handCardsValue, this.selectCardValue);
          this.setHandCards(newCardList); // 清除选择状态
          // if (GlobalData.cardInfo.cardDir) {
          //     this.doHandCardPopDown();
          // } else {
        ...
        onBtnOneCardDir() {
          ...
          (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            error: Error()
          }), GlobalData) : GlobalData).cardInfo.oneCard = true; // GlobalData.cardInfo.cardDir = false;
        ...
        onReviceCard(data, type) {
          if (type) {
            ...
            var _card6 = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).convertCardListS2C([data.card])[0];
            (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.giveCard = _card6;
            this.setHandCards(this.handCardsValue.concat(_card6), false, true, (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.oneCard);
            this.showBtnLayer(false);
          }
...
//# sourceMappingURL=e2bd16fe85a7132ee61e6ff26fe28d918a89a1f3.js.map