    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function getOneCardSizeList(value) {
          ...
          var others = tmpList.filter(g => g.length !== 2); // 连对识别
          ...
          for (var _i6 = 0; _i6 < pairs.length; _i6++) {
            if (temp.length === 0) {
              ...
              var curr = getCardSize(pairs[_i6][0]);
          ...
          if (temp.length >= 2) lianDuiList.push(temp.flat()); // 去掉已合并成连对的对子

          var usedPairCards = new Set(lianDuiList.flat());
          var remainingPairs = pairs.filter(pair => !usedPairCards.has(pair[0])); // 顺子 & 同花顺识别

          var remainCards = [...remainingPairs.flat(), ...others.flat()];
          var sortedRemain = sortCardsBySizeUp(remainCards, remainCards.length);
          ...
          for (var group of straightGroups) {
            if (isSameColor(group)) {
              tongHuaShunList.push(group);
            } else {
              shunziList.push(group);
            }
          }

          var usedStraight = new Set([...tongHuaShunList.flat(), ...shunziList.flat()]);
          var remainAfterStraight = sortedRemain.filter(c => !usedStraight.has(c)); // 单张转二维

          var remainSingleList = remainAfterStraight.map(c => [c]);
          var tmpAll = [...tongHuaShunList, ...lianDuiList, ...shunziList, ...remainingPairs, ...others, ...remainSingleList];
...
//# sourceMappingURL=3f054bd591f325a4675d7a6c4a1acebe6fa42076.js.map