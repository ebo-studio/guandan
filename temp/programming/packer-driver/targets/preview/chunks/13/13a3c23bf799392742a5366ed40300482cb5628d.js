      ...
      _export("CardLayer", CardLayer = (_dec = ccclass('CardLayer'), _dec2 = property(Prefab), _dec3 = property(SpriteAtlas), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(_crd && GameTimer === void 0 ? (_reportPossibleCrUseOfGameTimer({
        ...
        onBtnHint() {
          ...
          while (checked < total) {
            ...
            this.hintIndex = (this.hintIndex + 1) % total; // 下次点击从下一组开始

            var isPartOfGrouped = false;

            for (var group of grouped) {
              if (group.length <= 1) continue; // 只跳过成型牌（长度 > 1）

              for (var card of hint) {
              ...
              if (isPartOfGrouped) break;
            }

            if (!isPartOfGrouped) {
              this.selectCardValue = hint; // 找到一组不在成型牌中的提示
            }

            checked++;
          } // 如果不是成型牌的一部分，说明是散牌，可以使用
          // if (!isPartOfGrouped) {
...
//# sourceMappingURL=13a3c23bf799392742a5366ed40300482cb5628d.js.map