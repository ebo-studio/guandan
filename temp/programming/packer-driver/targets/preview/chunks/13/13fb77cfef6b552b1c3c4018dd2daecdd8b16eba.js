      ...
      _export("CardLayer", CardLayer = (_dec = ccclass('CardLayer'), _dec2 = property(Prefab), _dec3 = property(SpriteAtlas), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(_crd && GameTimer === void 0 ? (_reportPossibleCrUseOfGameTimer({
        ...
        dealReconectOutCards(viewid, cardList, count) {
          ...
          if (viewid == (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            for (var k = 0; k < tempCards.length; k++) {
              ...
              _card.node.setSiblingIndex(k);

              _card.node.setScale(this.outScaleSelf, this.outScaleSelf, this.outScaleSelf);

              _card.node.setPosition(posX, -45);

              var cardSps = this.getCardsSprite(tempCards[k]);

              if (cardSps) {
                _card.setValue(tempCards[k], cardSps);
            ...
            for (var i = 0; i < count; i++) {
              ...
              _card3.node.setScale(this.outScaleOther, this.outScaleOther, this.outScaleOther);

              startX += this.outDistanceOther;

              _card3.node.setPosition(startX, _pos_Y, 0);

              var _cardSps2 = this.getCardsSprite(tempCards[i]);

              if (_cardSps2) {
                _card3.setValue(tempCards[i], _cardSps2);
          ...
          if (viewid == (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            (function () {
              ...
              for (var _k2 = 0; _k2 < _this2.outCards[viewid].length; _k2++) {
                ...
                var _card4 = _this2.outCards[viewid][_k2];

                _card4.setMask(false);

                _card4.node.setSiblingIndex(_k2);

                _card4.node.setScale(_this2.outScaleSelf, _this2.outScaleSelf, _this2.outScaleSelf);

                tween(_card4.node).to(.15, {
            ...
            for (var k = 0; k < tempCards.length; k++) {
              ...
              _card5.node.setSiblingIndex(k);

              _card5.node.setScale(0, 0, 0);

              _card5.node.setPosition(pos_X, pos_Y, 0);

              var cardSps = this.getCardsSprite(tempCards[k]);

              if (cardSps) {
                _card5.setValue(tempCards[k], cardSps);

                this.outCards[viewid].push(_card5);
              }

              var moveTime = 0.15;
              tween(_card5.node).parallel(tween().to(moveTime, {
            ...
            for (var i = 0; i < count; i++) {
              var _card6 = this.getOneCard();

              _card6.node.setSiblingIndex(i);

              _card6.node.setScale(0, 0, 0);

              _card6.node.setPosition(_pos_X, _pos_Y2, 0);
              ...
              if (_cardSps3) {
                _card6.setValue(tempCards[i], _cardSps3);

                this.outCards[viewid].push(_card6);
                var _moveTime = 0.15;
                tween(_card6.node).parallel(tween().to(_moveTime, {
        ...
        showOtherHandCards(viewid, cardList, count) {
          ...
          if (viewid == (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            for (var k = 0; k < tempCards.length; k++) {
              var _card7 = this.getOneCard();

              var posX = this.getOutPosX(k, true);

              _card7.node.setSiblingIndex(k);

              _card7.node.setScale(this.outScaleOther, this.outScaleOther, this.outScaleOther);

              _card7.node.setPosition(posX, pos_Y - 120, 0);

              var cardSps = this.getCardsSprite(tempCards[k]);

              if (cardSps) {
                _card7.setValue(tempCards[k], cardSps);

                this.outCards[viewid].push(_card7);
              }
            }
          } else {
            ...
            for (var i = 0; i < count; i++) {
              var _card8 = this.getOneCard();

              _card8.node.setSiblingIndex(i);

              _card8.node.setScale(this.outScaleOther, this.outScaleOther, this.outScaleOther);

              startX += this.outDistanceOther;

              _card8.node.setPosition(startX, _pos_Y3, 0);
              ...
              if (_cardSps4) {
                _card8.setValue(tempCards[i], _cardSps4);

                this.outCards[viewid].push(_card8);
              }
            }
          }
        } //排序
        ...
        clearOutCards(viewid) {
          ...
          for (var i in this.outCards[viewid]) {
            var _card9 = this.outCards[viewid][i];

            if (_card9) {
              this.recycleOneCard(_card9);
            }
          }
        ...
        clearHandCards() {
          for (var i in this.handCards) {
            var _card10 = this.handCards[i];

            if (_card10) {
              this.recycleOneCard(_card10);
            }
          }
        ...
        onScreenTouchStart(event) {
          ...
          for (var i = this.handCards.length - 1; i >= 0; i--) {
            var _card11 = this.handCards[i];

            if (_card11.node && _card11.node.getComponent(UITransform).getBoundingBoxToWorld().contains(touchPos)) {
              // if (GlobalData.cardInfo.cardDir) {
              //     card.setMask(true);
              // } else {
              if (_card11.isMask()) {
                _card11.setMask(false);
              } else {
                _card11.setMask(true);
              } // }
        ...
        onScreenTouchMove(event) {
          ...
          for (var _i = this.handCards.length - 1; _i >= 0; _i--) {
            var _card12 = this.handCards[_i];

            if (_card12.node && _card12.node.getComponent(UITransform).getBoundingBoxToWorld().contains(startPos)) {
              _card12.setMask(true);
          ...
          for (var _i2 = this.handCards.length - 1; _i2 >= 0; _i2--) {
            var _card13 = this.handCards[_i2];

            if (_card13.node && _card13.node.getComponent(UITransform).getBoundingBoxToWorld().contains(movePos)) {
              _card13.setMask(true);
        ...
        updateSelectedCards_V() {
          ...
          for (var _card14 of selectedSet) {
            if (this.sorthandCardsValue.includes(_card14)) {
              isRecover = true;
              break;
            }
          }
        ...
        onBtnHint() {
          ...
          var hint = this.hintCards[this.hintIndex];

          if (hint.length !== 1) {
            this.hintIndex++;
          } else {
            // 判断该牌是否属于 groupedCards 的成型组合（大于1张的牌组）
            for (var group of this.groupedCards) {
              if (group.length <= 1) continue; // 单牌组不算成型牌

              if (group.includes(card)) {
                isPartOfGrouped = true;
                break;
              }
            }
          }

          for (var _hint of this.hintCards) {
            if (_hint.length !== 1) continue; // 不是单牌，跳过

            var _card15 = _hint[0]; // 如果不是成型牌的一部分，说明是散牌，可以使用
        ...
        onBtnCardDir() {
          ...
          for (var _card16 of selected) {
            if (!this.sorthandCardsValue.includes(_card16)) {
              this.sorthandCardsValue.push(_card16);
            }
          } // ✅ 理牌处理
        ...
        smartSetHandCards(sortedCards) {
          ...
          for (var _i3 = 0; _i3 < sameSizeList.length; _i3++) {
            ...
            for (var _j4 = col.length - 1; _j4 >= 0; _j4--) {
              ...
              var _card17 = this.getOneCard();

              _card17.node.setSiblingIndex(idx);

              var posY = this.getHandCardPosY_V(_j4);
              _card17.node.scale = new Vec3(this.handScale, this.handScale, this.handScale);
              var cardSps = this.getCardsSprite(value);

              if (cardSps) {
                _card17.setValue(value, cardSps);
              }

              _card17.node.setPosition(v3(posX, posY));

              _card17.setIndex(idx);

              _card17.setBottom(_j4 === 0);

              _card17.setLastLine(_i3 === sameSizeList.length - 1);

              this.handCards.push(_card17);
              idx++;
            }
          }
        } ////////////////头像////////////////
        ...
        onOutCards(data) {
          ...
          if (Boolean(data.isSend)) {
            var cards = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).convertCardListS2C(data.cards); // if (viewId == GlobalData.viewId.self && this.outCardList.length != 0) {
            //     if (GlobalData.cardInfo.oneCard || GlobalData.cardInfo.sortCard) {
        ...
        onReviceCard(data, type) {
          if (type) {
            ...
            var _card18 = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).convertCardListS2C([data.card])[0];
            (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.giveCard = _card18;
            this.setHandCards(this.handCardsValue.concat(_card18), false, true, (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.oneCard);
            ...
            var _card19 = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).convertCardListS2C([data.card])[0];
            (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.giveCard = _card19;
            this.setHandCards(this.handCardsValue.concat(_card19), false, true, (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.oneCard);
            this.showBtnLayer(false);
          }
...
//# sourceMappingURL=13fb77cfef6b552b1c3c4018dd2daecdd8b16eba.js.map