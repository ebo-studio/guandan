    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function getSameCardSizeList(value) {
          ...
          } // 排序优先级：四王 > 6张炸 > 同花顺 > 5炸 > 4炸 > 其它


          result.sort((a, b) => {
            var rankA = getGroupRank(a);
            var rankB = getGroupRank(b);

            if (rankA !== rankB) {
              return rankB - rankA; // 大牌型排前面
            } // 相同类型比大小


            return getCardSize(b[0]) - getCardSize(a[0]);
          });
          return result; // let tmpList: number[][] = [];
...
//# sourceMappingURL=132176e6f4dbac5ede30d917bb7ba9e6536136db.js.map