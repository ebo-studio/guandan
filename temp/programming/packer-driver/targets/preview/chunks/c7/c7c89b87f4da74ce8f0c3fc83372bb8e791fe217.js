    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          for (var [rank, same] of map.entries()) {
            if (same.length >= 3) {
              triples.push({
                rank,
                cards: same.slice(0, 3)
              });
            }
          } // 按 rank 从小到大排序，优先出小牌的三带二


          triples.sort((a, b) => getCardSize(a.cards[0]) - getCardSize(b.cards[0]));

          var _loop = function _loop(triple) {
            // 标记 triple 用过的索引
            var tripleIdx = [];

            var _loop2 = function _loop2(tCard) {
              var idx = cards.findIndex((c, i) => c === tCard && !usedIndex.has(i));
            ...
            for (var tCard of triple.cards) {
              _loop2(tCard);
            }

            if (tripleIdx.length !== 3) return "continue"; // 查找对子

            var remains = cards.map((c, i) => ({
              c,
              i
            })).filter(_ref => {
              var {
              ...
              return !usedIndex.has(i) && !triple.cards.includes(c);
            });
            var pairMap = new Map();
            ...
            for (var {
              ...
              var size = getCardSize(c);
              if (!pairMap.has(size)) pairMap.set(size, []);
              pairMap.get(size).push(i);
            }

            for (var [_, idxList] of pairMap) {
              if (idxList.length >= 2) {
                var _ret2 = function () {
                  var group = [];
                  tripleIdx.forEach(i => {
                  ...
                  var pairIdx = idxList.slice(0, 2);
                  pairIdx.forEach(i => {
                    usedIndex.add(i);
                    group.push(cards[i]);
                  });
                  result.push(group);
                  return "break"; // 找到一个三带二就够了
                }();
...
//# sourceMappingURL=c7c89b87f4da74ce8f0c3fc83372bb8e791fe217.js.map