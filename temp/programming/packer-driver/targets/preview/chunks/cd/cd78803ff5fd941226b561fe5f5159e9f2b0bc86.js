    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function autoSortCards(cards) {
          ...
          var randomGroups = [shuffleArray(findThreeWithTwo(getRemain(true))), // 三带二
        ...
        function findThreeWithTwo(cards) {
          var result = [];
          var rankMap = new Map();
          var pairMap = new Map(); // 拆分点数映射表（排除大小王）
          ...
          for (var [_rank2, group] of rankMap.entries()) {
            if (group.length >= 3) {
          ...
          var _loop = function _loop(triple) {
            var tripleCardSet = new Set(triple); // 找出一个未冲突的对子

            var pair = pairs.find(p => !p.some(c => tripleCardSet.has(c)) && !p.some(c => used.has(c)));

            if (pair) {
              result.push([...triple, ...pair]);
        ...
        function findRocket(cards) {
          var smallKings = cards.filter(c => c % 16 === 14);
          var bigKings = cards.filter(c => c % 16 === 15);
        ...
        function findBombsByCount(cards, count) {
          ...
          for (var group of map.values()) {
            if (group.length >= count) {
              ...
              for (var i = 0; i + count <= group.length; i += count) {
        ...
        function smartSortCards(cards) {
          var mainResult = []; // 主牌型区（固定优先级）

          var randomPart = []; // 飞机、连对、顺子、三带二等

          var used = new Set(); // const pushAndMark = (group: number[], toMain: boolean = true) => {
          ...
          findRocket(getUnused()).forEach(g => pushAndMark(g)); // 1. 六炸

          findBombsByCount(getUnused(), 6).forEach(g => pushAndMark(g)); // 2. 同花顺

          findFlushStraight(getUnused()).forEach(g => pushAndMark(g)); // 3. 五炸

          findBombsByCount(getUnused(), 5).forEach(g => pushAndMark(g)); // 4. 四炸

          findBombsByCount(getUnused(), 4).forEach(g => pushAndMark(g)); // ------- 以下为杂牌型，最后追加 -------
          // 飞机（不带）

          findPlane(getUnused()).forEach(g => pushAndMark(g, false)); // 连对

          findChainPairs(getUnused()).forEach(g => pushAndMark(g, false)); // 顺子

          findStraight(getUnused()).forEach(g => pushAndMark(g, false)); // 三带二

          findThreeWithTwo(getUnused()).forEach(g => pushAndMark(g, false)); // 5. 对子

          findPair(getUnused()).forEach(g => pushAndMark(g)); // 6. 单张

          getUnused().forEach(c => pushAndMark([c]));
          var mainResult1 = getSameCardSizeList(mainResult.flat());
          console.log("理牌分组：", mainResult1.concat(randomPart)); // return mainResult.concat(randomPart); // 返回最终理牌结果
          // const mainResult: number[][] = [];      // 主牌型：王炸、炸弹、同花顺、大王、小王、级牌
          // const scatteredPart: number[][] = [];   // 单张/对子/三张（降序）
          // const randomPart: number[][] = [];      // 飞机、顺子、连对、三带二（升序）
          // const usedIndex = new Set<number>();
          // const getUnused = () => cards.filter((_, i) => !usedIndex.has(i));
          // const pushAndMark = (group: number[], target: number[][]) => {
          ...
          //         }
          //     }
          //     if (realGroup.length > 0) target.push(realGroup);
          // };
          // // 固定优先级识别
          // findRocket(getUnused()).forEach(g => pushAndMark(g, mainResult));
          // findBombsByCount(getUnused(), 6).forEach(g => pushAndMark(g, mainResult));
          // findFlushStraight(getUnused()).forEach(g => pushAndMark(g, mainResult));
          // findBombsByCount(getUnused(), 5).forEach(g => pushAndMark(g, mainResult));
          // findBombsByCount(getUnused(), 4).forEach(g => pushAndMark(g, mainResult));
          // // 单张识别：大王、小王、级牌
          // getUnused().forEach(c => {
          //     const rank = c % 16;
          //     if (rank === 15 || rank === 14 || rank === GlobalData.cardInfo.levelCard) {
          //         pushAndMark([c], mainResult);
          //     }
          // });
          // // 杂牌型识别（按组内最小牌点升序）
          // const miscGroups = [
          //     ...findPlane(getUnused()),
          //     ...findChainPairs(getUnused()),
          //     ...findStraight(getUnused()),
          //     ...findThreeWithTwo(getUnused())
          // ];
          // miscGroups
          //     .sort((a, b) => minRank(a) - minRank(b))
          //     .forEach(g => pushAndMark(g, randomPart));
          // // 剩余牌（三张/对子/单张） → 按最大点数降序
          // const countMap = getCardCountMap(getUnused());
          // const remainGroups: number[][] = [];
          // for (const [rank, group] of countMap.entries()) {
          //     const copy = [...group];
          //     while (copy.length >= 3) remainGroups.push(copy.splice(0, 3));
          //     while (copy.length >= 2) remainGroups.push(copy.splice(0, 2));
          //     while (copy.length >= 1) remainGroups.push(copy.splice(0, 1));
          // }
          // remainGroups
          //     .sort((a, b) => maxRank(b) - maxRank(a))
          //     .forEach(g => pushAndMark(g, scatteredPart));
          // return [...mainResult, ...scatteredPart, ...randomPart];
        }
        ...
        function findFlushStraight(cards) {
          ...
          for (var group of colorMap.values()) {
            ...
            var ranks = Array.from(rankMap.keys()).sort((a, b) => a - b);
            ...
            for (var i = 0; i <= ranks.length - 5; i++) {
              ...
              if (isConsecutive) {
                ...
                for (var _rank4 of seq) {
                  straight.push(rankMap.get(_rank4).pop());
                  if (rankMap.get(_rank4).length === 0) rankMap.delete(_rank4);
                }
        ...
        function findPlane(cards) {
          ...
          while (i < ranks.length) {
            ...
            while (j < ranks.length && ranks[j] === ranks[j - 1] + 1) {
              j++;
            }
            ...
            if (length >= 2) {
              ...
              for (var k = i; k < j; k++) {
                plane.push(...map.get(ranks[k]).slice(0, 3));
              }

              result.push(plane);
            } else {
              // 单组三张也加进去（不是飞机）
              result.push(map.get(ranks[i]).slice(0, 3));
            }
        ...
        function findChainPairs(cards) {
          ...
          for (var i = 0; i < filtered.length; i++) {
            ...
            if (temp.length === 0 || filtered[i - 1] === curr - 1) {
              temp.push(map.get(curr).slice(0, 2)); // ✅ 超过3对，就截断，不要后面的
              ...
              if (temp.length >= 3) {
                results.push(temp.slice(0, 3).flat());
                break; // 只取最前面3对
              }
          ...
          if (temp.length >= 3 && results.length === 0) {
            results.push(temp.slice(0, 3).flat());
          }
          ...
          for (var i = 0; i <= ranks.length - 5; i++) {
            ...
            if (isConsecutive) {
              ...
              for (var _rank5 of seq) {
                straight.push(map.get(_rank5).pop());
                if (map.get(_rank5).length === 0) map.delete(_rank5);
              }
...
//# sourceMappingURL=cd78803ff5fd941226b561fe5f5159e9f2b0bc86.js.map