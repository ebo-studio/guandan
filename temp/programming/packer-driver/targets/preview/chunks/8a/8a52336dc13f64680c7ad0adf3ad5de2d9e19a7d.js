    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          var map = getCardCountMap(cards);
          var results = []; // 所有三张牌（按牌点从小到大）

          var triples = Array.from(map.entries()).filter(_ref => {
            var [rank, group] = _ref;
            return group.length >= 3;
          }).sort((a, b) => getCardSize(a[0]) - getCardSize(b[0]));

          for (var [rank, group] of triples) {
            // 找3张还没用过的
            var threeCards = [];

            for (var i = 0; i < cards.length; i++) {
              if (!usedIndex.has(i) && getCardSize(cards[i]) === rank) {
                threeCards.push(cards[i]);
                if (threeCards.length === 3) break;
              }
            }

            if (threeCards.length < 3) continue; // 找两张不同点数的对子作为带牌，不能是已有三张或炸弹牌

            var pair = null;

            for (var [r, g] of map.entries()) {
              if (r === rank || g.length < 2) continue;
              var tempPair = [];

              for (var _i5 = 0; _i5 < cards.length; _i5++) {
                if (!usedIndex.has(_i5) && getCardSize(cards[_i5]) === r) {
                  tempPair.push(cards[_i5]);
                  if (tempPair.length === 2) break;
                }
              }
            ...
            if (!pair) continue; // 添加结果并标记 usedIndex

            var full = [...threeCards, ...pair];

            for (var _i6 = 0; _i6 < cards.length; _i6++) {
              if (full.includes(cards[_i6]) && !usedIndex.has(_i6)) {
                usedIndex.add(_i6);
                full[full.indexOf(cards[_i6])] = NaN; // 避免重复标记
              }
            }

            results.push(threeCards.concat(pair));
          }
...
//# sourceMappingURL=8a52336dc13f64680c7ad0adf3ad5de2d9e19a7d.js.map