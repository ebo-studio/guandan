    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          // 基础分组
          var result = [];
          var moveToBack = []; // 找出新选中的牌（这一次比上一次多出来的）

          var prevSet = new Set(prevSelected);
          var newlySelected = selected.filter(c => !prevSet.has(c)); // 更新历史选中牌

          for (var card of newlySelected) {
            if (!selectedHistory.includes(card)) {
              selectedHistory.push(card);
            }
          }

          var selectedSet = new Set(selectedHistory); // 分组并处理
...
//# sourceMappingURL=b17140ece1952e80fd9cdd5fe8a77b84f47492de.js.map