      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          for (var bomb of bombs) {
            ...
            bomb.forEach(c => usedCards.add(c)); // 记得将炸弹的牌标记为已使用
          } // ✅ 步骤4：识别飞机
        ...
        function findBombsWithHeartCard(cards) {
          ...
          // 查找六炸、五炸、四炸（即三张、四张、五张相同点数的牌 + 红心级牌）

          var _loop = function _loop(rank, list) {
            if (list.length >= 3) {
              // 对于每组三张、四张、五张相同点数的牌，检查是否能组成炸弹
              var remainingCards = cards.filter(card => card !== list[0]);
              var sortedRemainingCards = remainingCards.sort((a, b) => a - b); // 判断是否能通过红心级牌组成炸弹
...
//# sourceMappingURL=242907e7e781f13f1d5ceb39f9254feb9b3aecda.js.map