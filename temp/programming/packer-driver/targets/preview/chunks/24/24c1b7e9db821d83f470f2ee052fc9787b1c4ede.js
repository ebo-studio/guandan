      ...
      (function (_GameLogic) {
        ...
        function getRemainCardsByDelete(value, deleteCards) {
          ...
          if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            sameSizeList = GameLogic.moveSelectedCardsToBack(value, []);
            console.log('出牌后>>', sameSizeList);
          } else {
        ...
        function sortSelectedCardsToBack(cardList, selected) {
          // 复制原始数组以防修改原数组
          var remainCards = [];
          var selectedCopy = (_crd && utils === void 0 ? (_reportPossibleCrUseOfutils({
            error: Error()
          }), utils) : utils).deepCopy(selected); // 用于最终排序后的牌组

          var finalList = [];
          var pickedSet = new Set(); // 选中部分按理牌优先级识别（例如炸弹 > 顺子 > 对子）

          var sortedSelected = GameLogic.getSortCard(selectedCopy); // 将识别出的组合提取

          var matchedGroups = []; // 依次识别复杂牌型（优先炸弹）

          var bombList = GameLogic.getSameCardSizeList(sortedSelected).filter(g => g.length >= 4);

          for (var group of bombList) {
            matchedGroups.push(group);
            group.forEach(c => pickedSet.add(c));
          } // 继续识别顺子（需要过滤掉已用的）


          var leftForStraight = sortedSelected.filter(c => !pickedSet.has(c));
          var straights = (GameLogic.findStraight == null ? void 0 : GameLogic.findStraight(leftForStraight)) || [];

          if (straights.length > 0) {
            var straight = straights[0];
            matchedGroups.push(straight);
            straight.forEach(c => pickedSet.add(c));
          } // 更多牌型可继续加（如连对、飞机、三带等）
          // === 整理最终顺序 ===
          // 1. 遍历原手牌，保留未选或未组成牌型的
          ...
          for (var i = 0; i < cardList.length; i++) {
            ...
            if (!pickedSet.has(card)) {
              finalList.push(card);
            }
          } // 2. 把组合好的牌型依次放在后面


          for (var _group of matchedGroups) {
            finalList.push(..._group);
          }

          return finalList;
        }

        _GameLogic.sortSelectedCardsToBack = sortSelectedCardsToBack;

        function moveSelectedCardsToBack(cards, selected) {
          var selectedCopy = (_crd && utils === void 0 ? (_reportPossibleCrUseOfutils({
            error: Error()
          }), utils) : utils).deepCopy(selected);
          var remain = []; // 从原牌组中剔除选中牌（按值逐个移除）

          for (var i = 0; i < cards.length; i++) {
            var v = cards[i];
            var idx = selectedCopy.indexOf(v);
          ...
          } // 按照 remain 原顺序分组（相同点数连续）


          var groupList = [];
          var temp = [];

          for (var _i5 = 0; _i5 < remain.length; _i5++) {
            var cur = remain[_i5];
          ...
          if (temp.length > 0) groupList.push(temp); // 最后一组是选中牌（不分组，直接作为一组）
...
//# sourceMappingURL=24c1b7e9db821d83f470f2ee052fc9787b1c4ede.js.map