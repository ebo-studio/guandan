    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          ...
          } // 按 rank 从小到大排序（三带二优先出小的）


          rankGroups.sort((a, b) => getCardSize(a.rank) - getCardSize(b.rank));
          var result = [];

          for (var triple of rankGroups) {
            // 查找未被使用的对子（不能用其他三张或炸弹里的）
            var usedRanks = new Set(rankGroups.map(g => g.rank));
            var pairs = [];

            for (var [_rank2, _group4] of map.entries()) {
              if (_rank2 === triple.rank || usedRanks.has(_rank2)) continue;

              if (_group4.length >= 2) {
                // 确保这两张牌还未被 usedIndex 使用
                var pair = [];

                for (var i = 0; i < cards.length; i++) {
                  var card = cards[i];

                  if (!usedIndex.has(i) && getCardSize(card) === _rank2) {
                    pair.push(card);
            ...
            if (pairs.length > 0) {
              ...
              for (var _i5 = 0; _i5 < cards.length; _i5++) {
                if (!usedIndex.has(_i5) && getCardSize(cards[_i5]) === triple.rank) {
                  threeCards.push(cards[_i5]);
                  if (threeCards.length === 3) break;
                }
              }

              if (threeCards.length === 3) {
                var combo = [...threeCards, ...pairs[0]]; // 标记使用索引

                for (var _i6 = 0; _i6 < cards.length; _i6++) {
                  if (combo.includes(cards[_i6])) {
                    usedIndex.add(_i6); // 避免重复添加同一张

                    combo[combo.indexOf(cards[_i6])] = NaN;
                  }
                }

                result.push(threeCards.concat(pairs[0]));
              }
            }
          }
...
//# sourceMappingURL=4a6256ab0f617d5c3a590ba845af49aa65f2e6a6.js.map