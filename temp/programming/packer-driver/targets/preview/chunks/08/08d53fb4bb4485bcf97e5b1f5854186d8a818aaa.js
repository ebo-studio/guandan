    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function getOneCardSizeList(value) {
          ...
          var triples = tmpList.filter(g => g.length === 3);
          var pairs = tmpList.filter(g => g.length === 2);
          var singles = tmpList.filter(g => g.length === 1);
          var others = tmpList.filter(g => g.length !== 1 && g.length !== 2 && g.length !== 3);
          var used = new Set(); // 三带二

          var sanDaiErList = [];

          for (var _i6 = 0; _i6 < triples.length; _i6++) {
            for (var _j3 = 0; _j3 < pairs.length; _j3++) {
              if (getCardSize(triples[_i6][0]) !== getCardSize(pairs[_j3][0]) && !used.has(_i6) && !used.has(1000 + _j3)) {
                sanDaiErList.push([...triples[_i6], ...pairs[_j3]]);
                used.add(_i6);
                used.add(1000 + _j3);
                break;
              }
          ...
          for (var _i7 = 0; _i7 < triples.length; _i7++) {
            if (used.has(_i7)) continue;

            for (var _j4 = 0; _j4 < singles.length; _j4++) {
              if (getCardSize(triples[_i7][0]) !== getCardSize(singles[_j4][0]) && !used.has(2000 + _j4)) {
                sanDaiYiList.push([...triples[_i7], ...singles[_j4]]);
                used.add(_i7);
                used.add(2000 + _j4);
                break;
              }
          ...
          var remainingPairs = pairs.filter((_, i) => !used.has(1000 + i));
          var lianDuiList = [];
          var temp = [];

          for (var _i8 = 0; _i8 < remainingPairs.length; _i8++) {
            if (temp.length === 0) {
              temp.push(remainingPairs[_i8]);
            } else {
              var last = getCardSize(temp[temp.length - 1][0]);
              var curr = getCardSize(remainingPairs[_i8][0]);

              if (curr === last + 1 && curr < 15) {
                temp.push(remainingPairs[_i8]);
              } else {
                if (temp.length >= 2) lianDuiList.push(temp.flat());
                temp = [remainingPairs[_i8]];
              }
            }
          }

          if (temp.length >= 2) lianDuiList.push(temp.flat());
          var usedPairCards = new Set(lianDuiList.flat());
          var finalPairs = remainingPairs.filter(pair => !usedPairCards.has(pair[0]));
          var finalTriples = triples.filter((_, i) => !used.has(i));
          var finalSingles = singles.filter((_, i) => !used.has(2000 + i)); // 同花顺/顺子识别

          var remainingForStraight = [...finalSingles.flat(), ...finalTriples.flat(), ...finalPairs.flat(), ...others.flat()];
          var sortedRemain = sortCardsBySizeUp(remainingForStraight, remainingForStraight.length);
          ...
          var findStraightGroups = cards => {
            ...
            for (var _i9 = 0; _i9 < unique.length; _i9++) {
              ...
              if (temp.length === 0) {
                ...
                var prev = getCardSize(temp[temp.length - 1]);

                var _curr = getCardSize(cur);
          ...
          var isSameColor = cards => {
            var color = getCardColor(cards[0]);
            return cards.every(c => getCardColor(c) === color);
          };

          var straightGroups = findStraightGroups(sortedRemain);
          var tongHuaShunList = [];
          var shunziList = [];

          for (var group of straightGroups) {
            if (isSameColor(group)) tongHuaShunList.push(group);else shunziList.push(group);
          }

          var usedStraight = new Set([...tongHuaShunList.flat(), ...shunziList.flat()]);
          var remainAfterStraight = sortedRemain.filter(c => !usedStraight.has(c)); // 最终组合并排序

          var tmpAll = [...tongHuaShunList, ...sanDaiErList, ...sanDaiYiList, ...lianDuiList, ...finalPairs, ...finalTriples, ...shunziList, ...remainAfterStraight.map(c => [c])];
          tmpAll.sort((a, b) => getGroupPower(b) - getGroupPower(a));
          return tmpAll;
        }
        ...
        function getGroupPower(cards) {
          ...
          var allSame = isAllSameSize(cards);
          var kingCount = sizes.filter(s => s === 14 || s === 15).length;
          if (size === 4 && kingCount === 4) return 100; // 四王

          if (size >= 6 && allSame) return 90;
          if (size === 5 && allSame) return 80;
          if (size === 4 && allSame) return 60;

          if (size >= 5 && isStraight(sizes)) {
            return isSameColor(cards) ? 70 : 40;
          }

          if (size === 5 && isThreeTwo(cards)) return 35;
          if (size === 4 && isThreeOne(cards)) return 30;
          if (size >= 4 && size % 2 === 0 && isLianDui(cards)) return 45;
          if (size === 2 && getCardSize(cards[0]) === 15) return 25;
          if (size === 3 && allSame) return 30;
          if (size === 2 && allSame) return 20;
          if (size === 1) return 10;
          return 0;
        }
        ...
        function isThreeTwo(cards) {
          ...
          return map.size === 2 && [...map.values()].includes(3);
        }
        ...
        function isStraight(sizes) {
          for (var i = 1; i < sizes.length; i++) {
            if (sizes[i] !== sizes[i - 1] + 1 || sizes[i] >= 15) return false;
          }
        ...
        function isLianDui(cards) {
          ...
          for (var i = 0; i < cards.length; i += 2) {
            if (getCardSize(cards[i]) !== getCardSize(cards[i + 1])) return false;

            if (i >= 2) {
              var prev = getCardSize(cards[i - 2]);
              var curr = getCardSize(cards[i]);
              if (curr !== prev + 1 || curr >= 15) return false;
            }
          }
...
//# sourceMappingURL=08d53fb4bb4485bcf97e5b1f5854186d8a818aaa.js.map