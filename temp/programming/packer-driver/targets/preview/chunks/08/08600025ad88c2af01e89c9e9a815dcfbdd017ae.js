  ...
  return {
    ...
    execute: function () {
      ...
      _export("CardLayer", CardLayer = (_dec = ccclass('CardLayer'), _dec2 = property(Prefab), _dec3 = property(SpriteAtlas), _dec4 = property(Node), _dec5 = property(Button), _dec6 = property(Button), _dec7 = property(Button), _dec8 = property(Button), _dec9 = property(_crd && GameTimer === void 0 ? (_reportPossibleCrUseOfGameTimer({
        ...
        error: Error()
      }), CardAction) : CardAction), _dec11 = property(Node), _dec12 = property(Node), _dec13 = property(SpriteFrame), _dec14 = property(Sprite), _dec15 = property(SpriteFrame), _dec16 = property(Sprite), _dec(_class = (_class2 = class CardLayer extends Component {
        constructor() {
          super(...arguments);
          ...
          _initializerDefineProperty(this, "spCardDirs", _descriptor14, this);
        ...
        testSelfHandCard() {
          ...
          console.log("GlobalData.cardInfo.cardDir---> ", (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            error: Error()
          }), GlobalData) : GlobalData).cardInfo.cardDir);
        ...
        setAndSortHandCards(value) {
          ...
          var sameSizeList = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
            error: Error()
          }), GameLogic) : GameLogic).moveSelectedCardsToBack(value, this.selectCardValue);
          this.initHandStartPosX_V(sameSizeList.length);
          var idx = 0;
          var tmpIdx = 0;
          var that = this; // console.log("---> ",sameSizeList[0][0]);
        ...
        setHandCards(value, ani, isUp, isOneCard) {
          ...
          if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            this.cardPosY = posY; // console.log("this.handCardsValue ", this.handCardsValue);

            var _loop3 = function _loop3(i) {
              // console.log("i-------> ", i);
              var card = _this2.getOneCard();

              card.node.setSiblingIndex(i);

              var posX = _this2.getHandCardPosX(i);

              card.node.scale = new Vec3(_this2.handScale, _this2.handScale, _this2.handScale); //手牌位置

              var cardSps = _this2.getCardsSprite(_this2.handCardsValue[i]);

              if (cardSps) {
                card.setValue(_this2.handCardsValue[i], cardSps);
              } //有发牌动画,最后一张显示背面


              if (ani) {
                if (i == _this2.handCardsValue.length - 1) {
                  card.setBack(true);
                }
              } //动画


              var moveFunc = function moveFunc(posX, posY) {
                if (ani) {
                  card.node.setPosition(v3(0, 0, 0));
                  tween(card.node).delay(i * delayTime).call(() => {
                    ...
                    if (i == that.handCardsValue.length - 1) {
                      that.sortAllCards();
                      console.log("牌发完了---> ", (_crd && utils === void 0 ? (_reportPossibleCrUseOfutils({
                        error: Error()
                      }), utils) : utils).deepCopy(that.handCardsValue));
                    }
                  }).start();
                } else {
                  if (isUp && that.handCardsValue[i] == (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
                  }), GlobalData) : GlobalData).cardInfo.giveCard) {
                    isUp = false; //牌值可能有两张一样的,提示一张就行

                    card.node.setPosition(v3(posX, posY + that.cardPopUpHight));
                    tween(card.node).sequence(tween().delay(0.2), tween().to(0.15, {
                      position: v3(posX, posY, 0)
                    })).start();
                  } else {
                    card.node.setPosition(v3(posX, posY));
            ...
            (function () {
              ...
              if (isOneCard) {
                ...
                sameSizeList = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                  error: Error()
                }), GameLogic) : GameLogic).getOneCardSizeList(value);
              } else {
                if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                  error: Error()
                  ...
                  sameSizeList = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).moveSelectedCardsToBack(value, _this2.selectCardValue);
                  _this2.handCardsValue = sameSizeList.flat();
                } else {
                  _this2.handCardsValue = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).sortCardsBySizeDown(_this2.handCardsValue, _this2.handCardsValue.length);
                  sameSizeList = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).getSameCardSizeList(value);
                }
              ...
              var _loop4 = function _loop4(_i) {
                var list = sameSizeList[_i];

                var posX = _this2.getHandCardPosX_V(_i);
                ...
                var _loop5 = function _loop5(j) {
                  ...
                  var posY = _this2.getHandCardPosY_V(j);

                  card.node.scale = new Vec3(_this2.handScale, _this2.handScale, _this2.handScale); //手牌位置

                  var cardSps = _this2.getCardsSprite(valueSize);
                  ...
                  if (ani) {
                    if (_i == sameSizeList.length - 1 && j == 0) {
                      card.setBack(true);
                      tmpIdx = idx;
                    }
                  }
                  ...
                  var moveFunc = function moveFunc(posX, posY, tmpIdx) {
                    if (ani) {
                      card.node.setPosition(v3(0, 0, 0));
                      tween(card.node).delay(_i * delayTime).call(() => {
                        if (tmpIdx == idx - 1) {
                          card.setBack(false);
                          that.setCanTouch(true);
                        }
                  ...
                  card.setIndex(idx);
                  card.setBottom(j == 0);
                  card.setLastLine(_i == sameSizeList.length - 1);
                  idx++;
              ...
              for (var _i = 0; _i < sameSizeList.length; _i++) {
                _loop4(_i);
              }
            })();
          }
        ...
        dealOutCards(viewid, cardList, count, isAuto) {
          ...
          if (viewid == (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            var tmpInfo = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).getRemainCardsByDelete(this.handCardsValue, tempCards);
            ...
            for (var _k2 = 0; _k2 < this.outCards[viewid].length; _k2++) {
              var posX = this.getOutPosX(_k2);
              var card = this.outCards[viewid][_k2];
              card.setMask(false);
            ...
            if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              ...
              (function () {
                //整理手牌
                var posY = _this3.getHandCardPosY(); //实际起始位置


                _this3.initHandStartPosX();

                var _loop6 = function _loop6(_j) {
                  var card = _this3.handCards[_j];
                  card.node.scale = v3(_this3.handScale, _this3.handScale, _this3.handScale);
                  card.setIndex(_j); //手牌位置

                  var posX = _this3.getHandCardPosX(_j);

                  var moveFunc = function moveFunc(posX, tmpIdx) {
                ...
                for (var _j = 0; _j < _this3.handCards.length; _j++) {
                  _loop6(_j);
                }
              })();
            } else {
              (function () {
                _this3.handCardsValue = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                  error: Error()
                }), GameLogic) : GameLogic).sortCardsBySizeDown(_this3.handCardsValue, _this3.handCardsValue.length);
                var sameSizeList;
                ...
                if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                  ...
                  sameSizeList = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).getOneCardSizeList(_this3.handCardsValue);
                } else {
                  sameSizeList = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                    error: Error()
                  }), GameLogic) : GameLogic).getSameCardSizeList(_this3.handCardsValue);
                }

                _this3.initHandStartPosX_V(sameSizeList.length);

                var idx = 0;
                var useIdxList = [];

                _this3.setCanTouch(false);

                var _loop7 = function _loop7(i) {
                  var list = sameSizeList[i];

                  var posX = _this3.getHandCardPosX_V(i);

                  var _loop8 = function _loop8(_j2) {
                    var valueSize = list[_j2];

                    var _loop9 = function _loop9(n) {
                      var card = _this3.handCards[n];

                      if (card.getValue() == valueSize && useIdxList.indexOf(card.getIndex()) == -1) {
                        useIdxList.push(card.getIndex());
                        card.node.setSiblingIndex(idx);

                        var posY = _this3.getHandCardPosY_V(_j2);

                        card.node.scale = new Vec3(_this3.handScale, _this3.handScale, _this3.handScale);
                        card.setBottom(_j2 == 0);
                        card.setLastLine(i == sameSizeList.length - 1);
                        ...
                        var moveFunc = function moveFunc(posX, posY, idx) {
                          tween(card.node).delay(0.2).to(0.1, {
                            ...
                            if (i == sameSizeList.length - 1 && _j2 == 0) {
                              that.setCanTouch(true);
                            }
                          }).start();
                        };
                    ...
                    for (var n = 0; n < _this3.handCards.length; n++) {
                      var _ret = _loop9(n);
                  ...
                  for (var _j2 = list.length - 1; _j2 >= 0; _j2--) {
                    _loop8(_j2);
                  }
                };
            ...
            for (var _k3 = 0; _k3 < tempCards.length; _k3++) {
              var _card3 = this.getOneCard();

              var _posX2 = this.getOutPosX(_k3, true);

              _card3.node.setSiblingIndex(_k3);

              _card3.node.setScale(0, 0, 0);

              _card3.node.setPosition(pos_X, pos_Y, 0);

              var cardSps = this.getCardsSprite(tempCards[_k3]);

              if (cardSps) {
                _card3.setValue(tempCards[_k3], cardSps);

                this.outCards[viewid].push(_card3);
              }

              var moveTime = 0.15;
              tween(_card3.node).parallel(tween().to(moveTime, {
                position: v3(_posX2, pos_Y - 120, 0)
              }), tween().to(moveTime, {
            ...
            for (var i = 0; i < count; i++) {
              ...
              _card4.node.setScale(0, 0, 0);

              _card4.node.setPosition(_pos_X, _pos_Y2, 0);
              ...
              if (_cardSps3) {
                _card4.setValue(tempCards[i], _cardSps3);

                this.outCards[viewid].push(_card4);
                var _moveTime = 0.15;
                tween(_card4.node).parallel(tween().to(_moveTime, {
        ...
        showOtherHandCards(viewid, cardList, count) {
          ...
          if (viewid == (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            for (var i = 0; i < count; i++) {
              var _card5 = this.getOneCard();

              _card5.node.setSiblingIndex(i);

              _card5.node.setScale(this.outScaleOther, this.outScaleOther, this.outScaleOther);

              startX += this.outDistanceOther;

              _card5.node.setPosition(startX, _pos_Y3, 0);
              ...
              if (_cardSps4) {
                _card5.setValue(tempCards[i], _cardSps4);

                this.outCards[viewid].push(_card5);
              }
            }
          }
        } //排序
        ...
        onScreenTouchMove(event) {
          ...
          if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            for (var i = this.handCards.length - 1; i >= 0; i--) {
              ...
              if (item != null) {
                var posX = item.position.x - item.getComponent(UITransform).contentSize.width / 2 * this.handScale; //左边界点
            ...
            for (var _i3 = this.handCards.length - 1; _i3 >= 0; _i3--) {
              var _card6 = this.handCards[_i3];
            ...
            //点击时,也会触发移动
            for (var _i4 = this.handCards.length - 1; _i4 >= 0; _i4--) {
              var _item = this.handCards[_i4].node;

              if (_item != null) {
                var posXL = _item.position.x - _item.getComponent(UITransform).contentSize.width / 2 * this.handScale; //左边界点

                var posXR = _item.position.x + _item.getComponent(UITransform).contentSize.width / 2 * this.handScale; //右边界点

                var _posY = _item.position.y + _item.getComponent(UITransform).contentSize.height / 2 * this.handScale; //上边界点


                var _posyYB = _item.position.y - _item.getComponent(UITransform).contentSize.height / 2 * this.handScale; //下边界点


                var tmpPosXR = this.handCards[_i4].getLastLine() ? posXR : posXL + this.handDistance_V; //右边界点

                var tmpPosY = this.handCards[_i4].getBottom() ? _posyYB : _posY - this.cardDistance_V; //下边界点
                //向左 向上  当前点纵坐标>下边界点  当前点横坐标>右边界点   牌的上边界点>起始点纵坐标    牌的左边界点<起始点横坐标  

                var leftUp = localStartPos.x >= localMovePos.x && localMovePos.y >= localStartPos.y && localMovePos.y >= tmpPosY && localMovePos.x <= tmpPosXR && _posY >= localStartPos.y && posXL <= localStartPos.x; //向左向下

                var leftDown = localStartPos.x >= localMovePos.x && localMovePos.y <= localStartPos.y && localMovePos.y <= _posY && localMovePos.x <= tmpPosXR && tmpPosY <= localStartPos.y && posXL <= localStartPos.x; //向右向上

                var rightUp = localStartPos.x <= localMovePos.x && localMovePos.y >= localStartPos.y && localMovePos.y >= tmpPosY && localMovePos.x >= posXL && _posY >= localStartPos.y && tmpPosXR >= localStartPos.x; //向右向下

                var rightDown = localStartPos.x <= localMovePos.x && localMovePos.y <= localStartPos.y && localMovePos.y <= _posY && localMovePos.x >= posXL && tmpPosY <= localStartPos.y && tmpPosXR >= localStartPos.x;

                if (leftUp || leftDown || rightUp || rightDown) {
                  this.handCards[_i4].setMask(true);
                } else {
                  this.handCards[_i4].setMask(false);
                }
              }
            } //第一张


            for (var _i5 = this.handCards.length - 1; _i5 >= 0; _i5--) {
              var _card7 = this.handCards[_i5];

              if (_card7.node && _card7.node.getComponent(UITransform).getBoundingBoxToWorld().contains(startPos)) {
                _card7.setMask(true);
            ...
            for (var _i6 = this.handCards.length - 1; _i6 >= 0; _i6--) {
              var _card8 = this.handCards[_i6];

              if (_card8.node && _card8.node.getComponent(UITransform).getBoundingBoxToWorld().contains(movePos)) {
                _card8.setMask(true);
        ...
        onScreenTouchEnd(event) {
          ...
          if ((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            ...
            this.doSelectCards(selectCards); //双击重置,不出牌
            ...
            for (var i in this.handCards) {
              ...
              if (item) {
                if (item.isMask()) {
                  ...
                  for (var _j4 in copeValue) {
                    if (copeValue[_j4] && copeValue[_j4] == item.getValue() && this.selectCardIndex.indexOf(item.getIndex()) == -1) {
                      this.selectCardIndex.push(item.getIndex());
                      item.setMask(true);
                      copeValue.splice(Number(_j4), 1);
                      break;
                    }
                  } //7788 7  牌值牌色一样
        ...
        onBtnCardDir() {
          ...
          this.showCardDir(true);
          this.init(false); // this.setHandCards(this.handCardsValue, false, false, false);
          //新优化部分
          ...
          if (cardType === (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            return;
          } // const remain = utils.deepCopy(this.handCardsValue);
          // const selectCopy = utils.deepCopy(selected);
          // let newHand: number[] = [];
          // for (let i = 0; i < remain.length; i++) {
          //     const card = remain[i];
          //     const idx = selectCopy.findIndex(v => v === card);
          //     if (idx !== -1) {
          //         selectCopy.splice(idx, 1); //删除匹配的选中牌
          //         continue;
          //     }
          //     newHand.push(card);
          // }
          // // 末尾加上选中的理牌牌组
          // newHand = newHand.concat(GameLogic.getSortCard(selected));
          // 更新显示
          // this.handCardsValue = newHand;


          this.setHandCards(this.handCardsValue); // 清除选择状态
        ...
        onBtnOneCardDir() {
          ...
          if (this.handCardsValue.length <= 0) {
            ...
            return;
          } // if(GlobalData.cardInfo.cardDir == false) {
          ...
          (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
            error: Error()
          }), GlobalData) : GlobalData).cardInfo.cardDir = false;
          this.init(false);
          this.setHandCards(this.handCardsValue, false, false, true); // const sorted = GameLogic.smartSortCards(this.handCardsValue);
          // GlobalData.cardInfo.cardDir = !GlobalData.cardInfo.cardDir;
          // this.showCardDir(true);
          // this.init(false);
          // this.smartSetHandCards(sorted);
        ...
        smartSetHandCards(sortedCards) {
          ...
          for (var _i7 = 0; _i7 < sameSizeList.length; _i7++) {
            var col = sameSizeList[_i7]; // 一列内的牌

            var posX = this.getHandCardPosX_V(_i7);

            for (var _j5 = col.length - 1; _j5 >= 0; _j5--) {
              var value = col[_j5];
              var card = this.getOneCard();
              card.node.setSiblingIndex(idx);
              var posY = this.getHandCardPosY_V(_j5);
              ...
              card.node.setPosition(v3(posX, posY));
              card.setIndex(idx);
              card.setBottom(_j5 === 0);
              card.setLastLine(_i7 === sameSizeList.length - 1);
        ...
        showCardDir(show) {
          ...
          if (show) {
            this.picCardDir.spriteFrame = this.spCardDirs[Number(!(_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.sortCard)];
          }
        }
        ...
        onGameFinishCards(datas) {
          ...
          this.hideAllTime();
          this.hideAllNoOut();
          this.showBtnLayer(false); //不包括自己的
        ...
        onOutCards(data) {
          ...
          if (Boolean(data.isSend)) {
            ...
            if (viewId == (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              ...
              // const cards = GameLogic.convertCardListS2C(data.cards);
              var remain = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
                error: Error()
              }), GameLogic) : GameLogic).getRemainCardsByDelete(this.handCardsValue, cards).cards; // ✅ 更新 handCardsValue 并用原来的 setHandCards 渲染

              this.handCardsValue = remain;
              this.setAndSortHandCards(this.handCardsValue);
              this.selectCardValue = [];
              (_crd && utils === void 0 ? (_reportPossibleCrUseOfutils({
                error: Error()
              }), utils) : utils).send((_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
        ...
        groupCardsBySizeKeepOrder(cards) {
          var result = [];
          var temp = [];
          ...
          for (var i = 0; i < cards.length; i++) {
            ...
            if (temp.length === 0 || (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).getCardSize(temp[0]) === (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).getCardSize(cur)) {
              temp.push(cur);
            } else {
              result.push(temp);
              temp = [cur];
        ...
        onUpCard(data) {
          ...
          var _loop10 = function _loop10(i) {
            ...
            var toPos = _this4.getOutCardByViewId(toViewId);

            var scale = _this4.getOutCardScaleByViewId(toViewId);

            var cards = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).convertCardListS2C([e.card]);

            _this4.onEventOutCards({
              viewid: fromViewId,
              cards: cards,
              cardCount: 1,
              isAuto: true
            ...
            if (_this4.outCards[fromViewId][0] != null) {
              //移动动画
              tween(_this4.outCards[fromViewId][0].node).delay(1).parallel(tween().to(moveTime, {
                position: toPos
              }), tween().to(moveTime, {
                scale: v3(scale, scale, scale)
              })).delay(2.5).call(() => {
                _this4.clearOutCards(fromViewId);
              }).start();
            }
          };
        ...
        onReviceCard(data, type) {
          if (type) {
            ...
            var _card9 = (_crd && GameLogic === void 0 ? (_reportPossibleCrUseOfGameLogic({
              error: Error()
            }), GameLogic) : GameLogic).convertCardListS2C([data.card])[0];
            (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.giveCard = _card9;
            this.setHandCards(this.handCardsValue.concat(_card9), false, true, (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.oneCard);
            this.showBtnLayer(false);
          }
      ...
      }), _descriptor14 = _applyDecoratedDescriptor(_class2.prototype, "spCardDirs", [_dec15], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
...
//# sourceMappingURL=08600025ad88c2af01e89c9e9a815dcfbdd017ae.js.map