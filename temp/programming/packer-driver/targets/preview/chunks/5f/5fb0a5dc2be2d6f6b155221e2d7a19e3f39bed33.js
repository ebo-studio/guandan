      ...
      (function (_GameLogic) {
        ...
        function getCardCountMap_I(cards) {
          ...
          for (var card of cards) {
            ...
            map.get(rank).push(card); // 判断红心级牌（这里假设 `levelCard` 是红心2等特殊牌）

            if (isHeartCard(card)) {
              heartCards.push(card); // 将红心级牌视为逢人配
            }
          } // 将红心级牌（逢人配）映射到 `map` 中


          for (var heartCard of heartCards) {
            var _rank = heartCard % 16; // 获取红心级牌的点数
            ...
            map.get(_rank).push(heartCard); // 将红心级牌添加到对应的点数分组中
          }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              if (hintList.length === 0) {
                for (var _i10 = groupedCards.length - 1; _i10 >= 0; _i10--) {
                  var _group6 = groupedCards[_i10]; // 只判断是否为炸弹（四张及以上，同点数）
...
//# sourceMappingURL=5fb0a5dc2be2d6f6b155221e2d7a19e3f39bed33.js.map