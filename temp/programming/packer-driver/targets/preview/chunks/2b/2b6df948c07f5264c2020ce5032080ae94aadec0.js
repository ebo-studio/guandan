    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected, prevGrouped) {
          ...
          var getColor = c => Math.floor(c / 16);

          var getSize = c => {
            var rank = c % 16;
            if (rank === 14) return 16; // 小王

            if (rank === 15) return 17; // 大王

            if (rank === (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
              error: Error()
            }), GlobalData) : GlobalData).cardInfo.levelCard) return 15; // 级牌
          ...
          var results = [];
          var usedIndex = new Set(); // 创建 rank -> [index] 映射

          var rankMap = new Map();

          for (var i = 0; i < cards.length; i++) {
            var rank = getSize(cards[i]);

            if (rank <= 14 && rank >= 3 || rank === 2 && getColor(cards[i]) === HONGTAO) {
              if (!rankMap.has(rank)) rankMap.set(rank, []);
              rankMap.get(rank).push(i);
            }
          }

          var allRanks = Array.from(rankMap.keys()).sort((a, b) => a - b); // 识别顺子（长度必须 == 5，不能复用）

          for (var _i7 = 0; _i7 <= allRanks.length - 5; _i7++) {
            var group = [];
            var valid = true;

            for (var j = 0; j < 5; j++) {
              var _rank = allRanks[_i7 + j];
              var candidates = rankMap.get(_rank);
              ...
              var idx = candidates.find(idx => !usedIndex.has(idx));
        ...
        function getOneCardSizeList(value) {
          ...
          for (var _i8 = 0; _i8 < pairs.length; _i8++) {
            if (temp.length === 0) {
              temp.push(pairs[_i8]);
            } else {
              var last = getCardSize(temp[temp.length - 1][0]);
              var current = getCardSize(pairs[_i8][0]);
          ...
          var findStraightGroups = cards => {
            ...
            for (var _i9 = 0; _i9 < unique.length; _i9++) {
              var cur = unique[_i9];
        ...
        function getGroupPower(cards) {
          ...
          if (size >= 4 && size % 2 === 0) {
            ...
            for (var _i10 = 1; _i10 < pairs.length; _i10++) {
              if (pairs[_i10] !== pairs[_i10 - 1] + 1 || pairs[_i10] >= 15) return 0;
            }
        ...
        function findBombsByCount(cards, count) {
          ...
          for (var [_rank2, group] of map.entries()) {
        ...
        function findFlushStraight(cards) {
          ...
          for (var [_color, colorCards] of colorMap.entries()) {
            ...
            for (var _card2 of colorCards) {
              var _rank3 = _card2 % 16;

              if (!rankMap.has(_rank3)) rankMap.set(_rank3, []);
              rankMap.get(_rank3).push(_card2);
            }
            ...
            for (var start = 3; start <= 10; start++) {
              ...
              if (missing === 0) {
                ...
                if (frp !== undefined) {
                  ...
                  // 恢复
                  for (var [_r2, _c2] of tempPopped) {
                    if (!rankMap.has(_r2)) rankMap.set(_r2, []);
                    rankMap.get(_r2).push(_c2);
                  }
                }
              } else {
                // 恢复
                for (var [_r3, _c3] of tempPopped) {
                  if (!rankMap.has(_r3)) rankMap.set(_r3, []);
                  rankMap.get(_r3).push(_c3);
                }
              }
            }
          }
        ...
        function findStraight_II(cards, usedIndex) {
          ...
          for (var _i11 = 0; _i11 < cards.length; _i11++) {
            if (usedIndex.has(_i11)) continue;
            var card = cards[_i11];
            var rank = card % 16;
            if (rank < 3 || rank > 13) continue; // 只允许 3~K 进入顺子

            if (!rankMap.has(rank)) rankMap.set(rank, []);
            rankMap.get(rank).push(_i11); // 保存的是索引，方便精确控制使用
          }
          ...
          while (i <= sortedRanks.length - 5) {
            ...
            if (isConsecutive) {
              ...
              for (var _rank5 of seq) {
                var available = rankMap.get(_rank5).filter(idx => !usedIndex.has(idx));
        ...
        function countThreeWithTwoAndStraight(unused) {
          ...
          for (var _i12 = 0; _i12 < unused.length; _i12++) {
            var _rank6 = getCardSize(unused[_i12]);

            if (_rank6 >= 3 && _rank6 <= 14 && !usedIndex.has(_i12)) {
              if (!rankToIndexes.has(_rank6)) rankToIndexes.set(_rank6, []);
              rankToIndexes.get(_rank6).push(_i12);
            }
          }
          ...
          for (var _i13 = 0; _i13 < sorted.length; _i13++) {
            var r = sorted[_i13];
        ...
        function findStraight(cards) {
          ...
          while (i <= ranks.length - 5) {
            ...
            if (length == 5) {
              ...
              for (var _rank7 of seq) {
                var cardsOfRank = map.get(_rank7);
                straight.push(cardsOfRank.pop());
          ...
          switch (type) {
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i15 = groupedCards.length - 1; _i15 >= 0; _i15--) {
                var _group2 = groupedCards[_i15];

                if (_group2.length === 3) {
                  var _card4 = _group2[0];
              ...
              for (var _i16 = groupedCards.length - 1; _i16 >= 0; _i16--) {
                var _group3 = groupedCards[_i16];

                if (_group3.length == 4) {
                  var bombs = GameLogic.newfindBombs(_group3);

                  if (bombs.length != 0) {
                    hintList.push(_group3);
                  }
                }
              } //五炸
              ...
              for (var _i17 = groupedCards.length - 1; _i17 >= 0; _i17--) {
                var _group4 = groupedCards[_i17];

                if (_group4.length == 5) {
                  var _bombs = GameLogic.newfindBombs(_group4);

                  if (_bombs.length != 0) {
                    hintList.push(_group4);
                  }
                }
              } //同花顺
              ...
              for (var _i18 = groupedCards.length - 1; _i18 >= 0; _i18--) {
                var _group5 = groupedCards[_i18];

                if (_group5.length == 5) {
                  var _bombs2 = GameLogic.findFlushStraight(_group5);

                  if (_bombs2.length != 0) {
                    hintList.push(_group5);
                  }
                }
              } //六炸
              ...
              for (var _i19 = groupedCards.length - 1; _i19 >= 0; _i19--) {
                var _group6 = groupedCards[_i19];

                if (_group6.length == 6) {
                  var _bombs3 = GameLogic.newfindBombs(_group6);
              ...
              for (var _i20 = groupedCards.length - 1; _i20 >= 0; _i20--) {
                var _group7 = groupedCards[_i20];

                if (_group7.length == 4) {
                  var _bombs4 = GameLogic.findRocket(_group7);

                  if (_bombs4.length != 0) {
                    hintList.push(_group7);
                  }
                }
              }
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i21 = groupedCards.length - 1; _i21 >= 0; _i21--) {
                var _group8 = groupedCards[_i21];

                if (_group8.length === 2) {
                  var _card5 = _group8[0];
              ...
              for (var _i22 = groupedCards.length - 1; _i22 >= 0; _i22--) {
                var _group9 = groupedCards[_i22];

                if (_group9.length === 3) {
                  var _card6 = _group9[0];
              ...
              for (var _i23 = groupedCards.length - 1; _i23 >= 0; _i23--) {
                var _group10 = groupedCards[_i23];

                if (_group10.length == 4) {
                  var _bombs5 = GameLogic.newfindBombs(_group10);
              ...
              for (var _i24 = groupedCards.length - 1; _i24 >= 0; _i24--) {
                var _group11 = groupedCards[_i24];

                if (_group11.length == 5) {
                  var _bombs6 = GameLogic.newfindBombs(_group11);
              ...
              for (var _i25 = groupedCards.length - 1; _i25 >= 0; _i25--) {
                var _group12 = groupedCards[_i25];

                if (_group12.length == 5) {
                  var _bombs7 = GameLogic.findFlushStraight(_group12);
              ...
              for (var _i26 = groupedCards.length - 1; _i26 >= 0; _i26--) {
                var _group13 = groupedCards[_i26];

                if (_group13.length == 6) {
                  var _bombs8 = GameLogic.newfindBombs(_group13);
              ...
              for (var _i27 = groupedCards.length - 1; _i27 >= 0; _i27--) {
                var _group14 = groupedCards[_i27];

                if (_group14.length == 4) {
                  var _bombs9 = GameLogic.findRocket(_group14);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_3:
              {
                ...
                for (var _i28 = groupedCards.length - 1; _i28 >= 0; _i28--) {
                  var _group15 = groupedCards[_i28];

                  if (_group15.length === 3) {
                    var size = GameLogic.getCardSize(_group15[0]);
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i30 = groupedCards.length - 1; _i30 >= 0; _i30--) {
                var _group16 = groupedCards[_i30];

                if (_group16.length >= 4) {
                  // ✅ 点数必须全部一致，才是炸弹
                  var isBomb = isSamePointWithFrp(_group16, (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
                  }), GlobalData) : GlobalData).cardInfo.levelCard);
                  if (!isBomb) continue;
                  var groupRank = GameLogic.getCardSize(_group16[0]);

                  if (_group16.length > targetCards.length || _group16.length === targetCards.length && groupRank > targetMainSize) {
            ...
            case (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
              ...
              for (var _i32 = groupedCards.length - 1; _i32 >= 0; _i32--) {
                var _group18 = groupedCards[_i32];

                if (_group18.length >= 6) {
                  if (!isSamePointWithFrp(_group18, (_crd && GlobalData === void 0 ? (_reportPossibleCrUseOfGlobalData({
                    error: Error()
                  }), GlobalData) : GlobalData).cardInfo.levelCard)) continue;

                  var _groupRank = GameLogic.getCardSize(_group18[0]);
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_SHUNZI_1:
              {
                ...
                var _loop3 = function _loop3(_i33) {
                  ...
                  if (isContinuous && seq[0] > targetStart) {
                    ...
                    for (var r of seq) {
                      _group19.push(rankMap.get(r).pop());
                    }
            ...
            }), GameDefine) : GameDefine).KIND_CARDS_COLOR:
              {
                ...
                var _loop4 = function _loop4(_i34) {
                  ...
                  if (isContinuous && seq[0] > _targetStart) {
                    ...
                    for (var r of seq) {
                      _group20.push(_rankMap.get(r).pop());
                    }
          ...
          if (targetType === (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            if (hintList.length === 0) {
              for (var _i37 = groupedCards.length - 1; _i37 >= 0; _i37--) {
                var _group23 = groupedCards[_i37];

                if (_group23.length === 2) {
                  var card = _group23[0];
            ...
            if (hintList.length === 0) {
              for (var _i38 = groupedCards.length - 1; _i38 >= 0; _i38--) {
                var _group24 = groupedCards[_i38];

                if (_group24.length === 3) {
                  var _card10 = _group24[0];
            ...
            if (hintList.length === 0) {
              for (var _i39 = groupedCards.length - 1; _i39 >= 0; _i39--) {
                var _group25 = groupedCards[_i39];

                if (_group25.length >= 4) {
                  var bombs = GameLogic.newfindBombs(_group25);
            ...
            for (var _group26 of groupedCards) {
              if (_group26.length === 2 && GameLogic.getCardSize(_group26[0]) > targetRank) {
            ...
            for (var _group27 of groupedCards) {
              if (_group27.length === 3 && GameLogic.getCardSize(_group27[0]) > targetRank) {
            ...
            var _loop5 = function _loop5(_group28) {
              if (_group28.length === 3 && GameLogic.getCardSize(_group28[0]) > targetRank) {
                // 找一个对子作为带牌
                var pair = groupedCards.find(g => g.length === 2 && g !== _group28);

                if (pair) {
                  hintList.push([..._group28, ...pair]);
                }
              }
            };

            for (var _group28 of groupedCards) {
              _loop5(_group28);
            }
          } // // ------------------ 顺子 ------------------
        ...
        function newfindBombs(cards, levelRank) {
          ...
          for (var _rank8 of sortedRanks) {
            var group = countMap.get(_rank8);
...
//# sourceMappingURL=2b6df948c07f5264c2020ce5032080ae94aadec0.js.map