    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function findThreeWithTwo(cards, usedIndex) {
          console.log("🔍 输入 cards:", cards);
          console.log("🔍 当前 usedIndex:", Array.from(usedIndex));

          var getCardSize = card => card % 16;

          var cardCountMap = new Map(); // 分组：点数 => 对应所有牌（不区分花色）

          for (var i = 0; i < cards.length; i++) {
            if (usedIndex.has(i)) continue;
            var rank = getCardSize(cards[i]);
            if (!cardCountMap.has(rank)) cardCountMap.set(rank, []);
            cardCountMap.get(rank).push(i); // 存索引而不是牌值，方便 later 标记
          }

          var triples = []; // 收集所有三张

          for (var [_rank2, indices] of cardCountMap.entries()) {
            if (indices.length >= 3) {
              triples.push(indices.slice(0, 3));
            }
          } // 排序三张（牌点小的优先）


          triples.sort((a, b) => getCardSize(cards[a[0]]) - getCardSize(cards[b[0]]));
          var result = [];

          for (var three of triples) {
            // 标记这三张的点数
            var tripleRank = getCardSize(cards[three[0]]); // 查找未使用、且不是同 rank 的对子

            var pair = null;

            for (var [_rank3, _indices] of cardCountMap.entries()) {
              if (_rank3 === tripleRank || _indices.length < 2) continue;
              var [i1, i2] = _indices;

              if (!usedIndex.has(i1) && !usedIndex.has(i2)) {
                pair = [i1, i2];
                break;
              }
            }
            ...
            if (pair) {
              // 合并索引并标记
              var allIndices = [...three, ...pair];
              var group = allIndices.map(idx => cards[idx]);
              allIndices.forEach(i => usedIndex.add(i));
              result.push(group);
            }
          }
        ...
        function smartSortCards(cards) {
          ...
          findThreeWithTwo(cards, usedIndex).forEach(g => pushAndMark(g, false)); // 三带二（最小三张）
          // ===== 剩余散牌，按点数从大到小排列 =====
            ...
            for (var i = 0; i <= ranks.length - 5; i++) {
              ...
              if (isConsecutive) {
                ...
                for (var _rank5 of seq) {
                  straight.push(rankMap.get(_rank5).pop());
                  if (rankMap.get(_rank5).length === 0) rankMap.delete(_rank5);
                }
          ...
          for (var i = 0; i <= ranks.length - 5; i++) {
            ...
            if (isConsecutive) {
              ...
              for (var _rank6 of seq) {
                straight.push(map.get(_rank6).pop());
                if (map.get(_rank6).length === 0) map.delete(_rank6);
              }
...
//# sourceMappingURL=64e85739faf69c198c18ea2407b0545c7ffa3d89.js.map