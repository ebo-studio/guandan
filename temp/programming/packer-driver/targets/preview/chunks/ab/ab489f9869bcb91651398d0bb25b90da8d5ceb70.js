    ...
    execute: function () {
      ...
      (function (_GameLogic) {
        ...
        function moveSelectedCardsToBack(cards, selected, prevSelected) {
          ...
          for (var c of newSelected) {
            if (!selectedHistory.includes(c)) {
              selectedHistory.push(c);
            }
          }

          var selectedSet = new Set(selectedHistory); // 2. 检查是否形成三带二组合

          var threeWithTwoSet = new Set();

          if (selectedHistory.length >= 5) {
            ...
            if (triple.length === 3 && pair.length === 2) {
              var group = [...triple, ...pair];
              group.forEach(c => threeWithTwoSet.add(c));
              moveToBack.push(group); // ✅ 整体推后
            }
          } // 3. 将其余牌按点数分组
          ...
          for (var j = 0; j < uniqueList.length; j++) {
            ...
            for (var i = idx; i < sortValue.length; i++) {
              if (getCardSize(d) === getCardSize(sortValue[i])) {
                if (threeWithTwoSet.has(sortValue[i])) {
                  // 已经放入组合组，不再加入
                  continue;
                }

                if (selectedSet.has(sortValue[i])) {
                  list1.push(sortValue[i]);
                } else if (!selectedSet.has(sortValue[i])) {
                  list1.push(sortValue[i]);
                }

                if (i === sortValue.length - 1) {
                  if (list1.length && !threeWithTwoSet.has(list1[0])) {
...
//# sourceMappingURL=ab489f9869bcb91651398d0bb25b90da8d5ceb70.js.map