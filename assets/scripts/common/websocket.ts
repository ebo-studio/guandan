...
export class websocket {
    ...
    _theSocket: WebSocket = null;
    _url: string = "";
    _connected: boolean = false;
    check_timeout_id: number = null;
    no_heartbeat_ms: number = 0;

    /**
     * connect
     */
    public connect(protocol: string, addr: string, port: number) {
        // this._url = protocol + '://' + addr + ':' + port;
        // console.log('Connect ' + this._url);
        this._url = protocol;
        this._theSocket = new WebSocket(this._url);
        // this._theSocket.binaryType = "arraybuffer";
    ...
    public sendData(arrayBuffer: ArrayBuffer) {
    ...
    protected sendHeartBeat(data: ArrayBuffer) {
    ...
    public onSocketError: (event: any) => void;

    public onSocketMessage: (data: ArrayBuffer) => void;

    public onSocketClose: (event: any) => void;
}
