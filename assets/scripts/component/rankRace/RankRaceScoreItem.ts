...
@ccclass('RankRaceScoreItem')
export class RankRaceScoreItem extends Component {
    //滚动列表
    ...
    setData() {
        this.nodeTip.active = GlobalData.gameResultInfo.scoreList.length == 0;
        this.personScroll.node.active = !this.nodeTip.active;
        if (this.personScroll.node.active) {
            this.personScroll.refreshData(GlobalData.gameResultInfo.scoreList);
        }
    }
}
