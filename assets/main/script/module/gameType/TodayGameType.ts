...
import { EUIState, IntervalTimeHelper, LevelHelper, Maths, PropHelper, RandomSeed, TResoucesUrl, Times, _config_, _gameType, _logic, _main, _platform, _prop, _timer, _ui, winCenterPostion } from "../../Main";
...
export class TodayGameType implements IGameTypeLogic {
    ...
    constructor() {
        ...
        _ui.on(_ui.EventType.OPEN, (url: string) => {
            ...
            if (_gameType.type == EGameType.today)
                if (url == CGameData.SettingUrl
                    || url == CGameData.PropGetUrl) {
    ...
    public run() {
        ...
        this.intervalTimeCountDown.interval = 100
    ...
    public checkComplete() {
        ...
        if (!(state == EUIState.Open
            || state == EUIState.Load))
            if (this.intervalTimeCountDown.sub <= 0) {
                let state = _ui.getModule(CGameData.PropGetUrl).state
